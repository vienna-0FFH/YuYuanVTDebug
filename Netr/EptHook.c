...
{
    InterlockedIncrement64((volatile LONG64*)&g_EptVersion);
}
...
EptInveptIpiTarget(_In_ ULONG_PTR Context)
{
    ...
    if (g_InVmxRootModePerCpu[cpu]) {
        ...
        UCHAR result = AsmInveptAllContexts();
        (void)result;
    } else if (g_HypervisorContext.IsActive) {
        __try {
            AsmVmCall(VMCALL_EPT_INVEPT);
        } __except (EXCEPTION_EXECUTE_HANDLER) { /* swallow,IPI 上下文不能 DbgPrint */ }
    }
    g_LastEptVersion[cpu] = g_EptVersion;
    return 0;
}
...
EptInveptAllContexts(VOID)
{
    ...
    // ========================================
    if (g_InVmxRootModePerCpu[cpuIndex]) {
        UCHAR result = AsmInveptAllContexts();
        (void)result;
        g_LastEptVersion[cpuIndex] = g_EptVersion;
        return;
    }
    ...
    if (irql <= APC_LEVEL && g_HypervisorContext.IsActive) {
        KeIpiGenericCall(EptInveptIpiTarget, 0);
        return;
    }
...
VOID EptSetVmxRootMode(BOOLEAN InRootMode)
{
    ULONG cpuIndex = KeGetCurrentProcessorNumber();
    if (cpuIndex < 64) {
        g_InVmxRootModePerCpu[cpuIndex] = InRootMode;
    }
}
...
static PEPT_HOOK_ENTRY
EptHookFindByPhysicalAddressNoLock(
    _In_ ULONG64 PhysicalAddress
);
...
)
{
    ...
    hookEntry->State = EptHookStateInactive;
    
    // 缓存目标页的 EPT 条目 (P1-3: per-CPU 数组,每核独立 PT)
    if (!EptSetupPerCpuPtesForHook(hookEntry->TargetPhysicalAddress, hookEntry)) {
        DbgPrint("[EPT-Hook] Failed to setup per-CPU PTEs for PA 0x%llx\n",
    ...
    // 激活 Hook (P1-3: 遍历所有 CPU,各自激活自己的 PTE)
    // Execute-Only 模式: 指向 FakePage, R=0, W=0, X=1
    //   执行时:正常执行 FakePage (含 JMP 到 Hook 函数)
    //   读取时:触发 EPT Violation,临时切换显示原始代码
    {
        ULONG activatedCount = 0;
        ULONG cpuIdx;
        for (cpuIdx = 0; cpuIdx < g_HypervisorContext.ProcessorCount &&
                         cpuIdx < RTL_NUMBER_OF(hookEntry->TargetPte); cpuIdx++) {
        ...
        if (activatedCount == 0) {
            DbgPrint("[EPT-Hook] ERROR: No per-CPU PTE available, cannot activate\n");
            EptHookRemove(hookEntry);
            return STATUS_UNSUCCESSFUL;
        }

        DbgPrint("[EPT-Hook] Activated on %u CPUs: Execute-Only (R=0,W=0,X=1) -> FakePage\n",
                 activatedCount);

        // 2026-05-31 修复同 EptSetupPerCpuPtesForHook: 不用 KeIpiGenericCall,
        // 改 g_EptVersion 递增让每核下次 vmexit 自动 INVEPT。避免装第二个 hook
        // 时 12 CPU 在 root 处理 NtQSI EPT violation 风暴 → IPI 死锁卡死。
        EptIncrementVersion();
    ...
    DbgPrint("[EPT-Hook] Hook installed successfully\n");
...
)
{
    ...
    // 先将 Hook 标记为 Inactive，防止新的 MTF 引用
    HookEntry->State = EptHookStateInactive;
    MemoryBarrier();
    
    // 恢复 EPT 权限和 PFN (P1-3: 遍历所有 CPU,各自恢复)
    // 关键:不仅要恢复 R/W/X,还要把 PFN 从 FakePage 切回原始物理页 ——
    // 否则 hook 被"标记移除"但 GPA 仍指向 trampoline,等于没卸。
    {
        ...
        for (cpuIdx = 0; cpuIdx < g_HypervisorContext.ProcessorCount &&
            ...
            if (HookEntry->TargetPte[cpuIdx]) {
                EptSetPteAtomic(HookEntry->TargetPte[cpuIdx],
                                HookEntry->TargetPhysicalAddress >> 12, 1, 1, 1);
                restoredCount++;
            }
        }
        if (restoredCount > 0) {
            EptInveptAllContexts();
            DbgPrint("[EPT-Hook] Restored on %u CPUs: PFN=0x%llx R=W=X=1\n",
                     restoredCount, HookEntry->TargetPhysicalAddress >> 12);
        }
    }
    ...
    if (retryCount >= 100) {
        DbgPrint("[EPT-Hook] Warning: MTF still pending after timeout\n");
    }
    ...
    DbgPrint("[EPT-Hook] Hook removed\n");
...
)
{
    ...
    hookEntry = EptHookFindByVirtualAddress(TargetAddress);
    if (!hookEntry) {
        return STATUS_NOT_FOUND;
    }
    
    return EptHookRemove(hookEntry);
}
...
EptHookRemoveAll(VOID)
{
    ...
    // 先将所有条目移到临时链表
    KeAcquireSpinLock(&g_EptHookManager.Lock, &oldIrql);
    while (!IsListEmpty(&g_EptHookManager.HookList)) {
        entry = RemoveHeadList(&g_EptHookManager.HookList);
    ...
    while (!IsListEmpty(&tempList)) {
        ...
        // 恢复 EPT 权限 (P1-3: 遍历所有 CPU)
        // 必须切回 OriginalPage —— 后面立即 MmFreeContiguousMemory 释放
        // FakePage,若 PTE 仍指向 FakePage 物理地址,等于野指针。
    ...
    // 批量刷新 EPT TLB（只在最后刷新一次）
    EptInveptAllContexts();
    
    DbgPrint("[EPT-Hook] All hooks removed\n");
}
...
)
{
    ...
    __vmx_vmread(VMCS_CTRL_PROCESSOR_BASED_VM_EXECUTION_CONTROLS, &cpuCtls);
    ...
    __vmx_vmwrite(VMCS_CTRL_PROCESSOR_BASED_VM_EXECUTION_CONTROLS, cpuCtls);
}
...
)
{
    ...
    if (hookEntry->State != EptHookStateActive || !hookEntry->TargetPte[cpuIndex]) {
    ...
    // 检查是否已有 pending 的 MTF（避免重复设置）
    // 使用内存屏障确保读取最新值
    MemoryBarrier();
    if (InterlockedCompareExchange(&mtfCtx->MtfActive, 1, 1) == 1) {
        // 已有 MTF pending —— 但 PTE 此时可能已被前一次 MTF exit 改回
        // R=0/W=0/X=1 (Execute-Only 状态)。如果当前是 read/write/execute
        // violation,**必须开 RWX**,否则 guest 重试同一指令还是 violation,
        // 我们又走到这里,又 return TRUE 不改 PTE → 死循环在 handler 内部,
        // 加载几秒后所有 CPU 卡死,且不走 fallback 切断 (2026-05-21 第六轮真元凶)。
        //
        // 历史:旧代码只在 isExecute 时开 RWX,isRead/isWrite 直接 return TRUE,
        // 是经典的 MTF 重叠死循环 bug。
        //
        // 2026-05-21 第七轮:旧实现 hookEntry->TargetPte->Read=1; ->Write=1;
        // ->Execute=1 是 3 次独立 RMW,多核 race 会看到中间态(R=1/W=0/X=0)
        // 触发 EPT_MISCONFIG 或权限循环。改用 cmpxchg64 一次原子写,保留当前 PFN。
        EptOpenRwxKeepPfnAtomic(hookEntry->TargetPte[cpuIndex]);
        EptInveptAllContexts();
        return TRUE;
    }
    ...
    if (g_EptHookManager.ExecuteOnlySupported) {
        ...
        if (isRead || isWrite) {
            ...
            if (ripOnSamePage) {
                ...
                // =====================================================
                EptSetPteAtomic(hookEntry->TargetPte[cpuIndex],
                                hookEntry->FakePagePhysical >> 12, 1, 1, 1);
                EptInveptAllContexts();
                ...
                // =====================================================
                EptSetPteAtomic(hookEntry->TargetPte[cpuIndex],
                                hookEntry->TargetPhysicalAddress >> 12, 1, 1, 0);
                EptInveptAllContexts();

                // 注意：VMX root 模式下不要调用 DbgPrint
            }
            
            // 设置 MTF，单步执行后恢复到 Execute-Only 状态
            // 使用原子操作和内存屏障确保顺序
            mtfCtx->PendingHookEntry = hookEntry;
            MemoryBarrier();
            InterlockedExchange(&mtfCtx->MtfActive, 1);
            EptSetMonitorTrapFlag(TRUE);
        }
        else if (isExecute) {
...
EptHookHandleMtfExit(VOID)
{
    ...
    SIZE_T guestRip = 0;
    ULONG64 ripPageVa;
    ULONG64 targetPageVa;
    BOOLEAN ripOnSamePage;
    static volatile LONG mtfCount = 0;
    LONG currentMtf;
    ...
    // 检查是否是我们的 MTF（使用原子操作）
    if (InterlockedCompareExchange(&mtfCtx->MtfActive, 0, 1) != 1) {
        // 不是我们设置的 MTF，但仍然需要禁用 MTF
        EptSetMonitorTrapFlag(FALSE);
        return FALSE;
    }
    
    // 获取 hookEntry（MtfActive 已被清除）
    MemoryBarrier();
    hookEntry = (PEPT_HOOK_ENTRY)mtfCtx->PendingHookEntry;
    
    // MTF 计数
    currentMtf = InterlockedIncrement(&mtfCount);
    
    // 读取当前 Guest RIP
    __vmx_vmread(GUEST_RIP, &guestRip);
    
    // 注意：VMX root 模式下不要调用 DbgPrint，会导致蓝屏
    
    // 清除 PendingHookEntry（MtfActive 已在上面被原子清除）
    // 使用内存屏障确保顺序
    mtfCtx->PendingHookEntry = NULL;
    MemoryBarrier();
    EptSetMonitorTrapFlag(FALSE);
    
    // 安全检查：验证 hookEntry 仍然有效 (P1-3: 检查当前 CPU 的 PTE)
    if (!hookEntry || hookEntry->State != EptHookStateActive ||
        !hookEntry->TargetPte[cpuIndex]) {
        return TRUE;
    }

    // 检查当前 RIP 是否仍在被 Hook 的页面上
    // 如果是，需要特殊处理以避免连续的 Execute Violation
    ripPageVa = guestRip & PAGE_MASK;
    targetPageVa = (ULONG64)hookEntry->TargetVirtualAddress & PAGE_MASK;
    ripOnSamePage = (ripPageVa == targetPageVa);

    // =====================================================
    // Execute-Only 模式：恢复到 FakePage (R=0, W=0, X=1)
    // =====================================================
    //
    // 无论 RIP 在哪里，都恢复到 Execute-Only 状态
    // 如果后续还有 Read Violation，会再次触发并处理
    // 注意：VMX root 模式下不要调用 DbgPrint

    // 恢复到 FakePage (R=0, W=0, X=1) — P1-3: 只动当前 CPU 的 PTE
    EptSetPteAtomic(hookEntry->TargetPte[cpuIndex],
                    hookEntry->FakePagePhysical >> 12, 0, 0, 1);
    EptInveptAllContexts();
...
)
{
    ...
    PEPT_HOOK_ENTRY hookEntry = EptHookFindByPhysicalAddressNoLock(pagePhysical);
    ULONG defensiveCpuIndex = KeGetCurrentProcessorNumber();
    if (defensiveCpuIndex >= 64) defensiveCpuIndex = 0;
    if (!hookEntry || !hookEntry->TargetPte[defensiveCpuIndex]) {
        return FALSE;
    }

    // 一律先切回原始页 + 开 RWX —— 这样 Guest 这一指令能成功访问,
    // 不会泄漏 FakePage 的 trampoline 字节给非 hook 路径的读取者。
    // P1-3: 只动当前 CPU 的 PTE
    EptSetPteAtomic(hookEntry->TargetPte[defensiveCpuIndex],
                    hookEntry->TargetPhysicalAddress >> 12, 1, 1, 1);
    EptInveptAllContexts();

    if (hookEntry->State != EptHookStateActive) {
        // hook 正在卸载中,EptHookRemove 会把 PFN 永久切回原始页,无需 MTF。
        return TRUE;
    }

    // hook 仍活跃 —— 单步本指令后由 EptHookHandleMtfExit 把 PTE 恢复回
    // FakePage + R=0/W=0/X=1。如果当前 CPU 上已经有 MTF pending,不重复武装。
    ULONG cpuIndex = KeGetCurrentProcessorNumber();
    if (cpuIndex >= 64) cpuIndex = 0;
    PEPT_MTF_CONTEXT mtfCtx = &g_EptHookManager.MtfContext[cpuIndex];

    if (InterlockedCompareExchange(&mtfCtx->MtfActive, 1, 0) == 0) {
        // 我们成功 0→1 抢到武装权
        mtfCtx->PendingHookEntry = hookEntry;
        MemoryBarrier();
        EptSetMonitorTrapFlag(TRUE);
    }
    // else: 已有 MTF pending,本次复用其单步,handler 会恢复 PTE。
...
)
{
    ...
    // 分配新的 PT 表（512 个条目，4KB 对齐）
    newPtTable = (PEPT_ENTRY)MmAllocateContiguousMemory(PAGE_SIZE_4KB, maxAddr);
    if (!newPtTable) {
    ...
    for (i = 0; i < 512; i++) {
        ...
        newPtTable[i].MemoryType = 6;  // Write-Back
    ...
    DbgPrint("[EPT-Hook] New PT table at VA %p, PA 0x%llx\n", 
        newPtTable, MmGetPhysicalAddress(newPtTable).QuadPart);
...
)
{
    ...
    // hypervisor 未激活 → 退化到单指针路径,只填 [0]
    if (!(g_HypervisorContext.IsActive && g_HypervisorContext.VcpuData)) {
        PEPT_PTE_ENTRY fallback = EptGetPteForPhysicalAddress(PhysicalAddress);
        if (!fallback) return FALSE;
        HookEntry->TargetPte[0] = fallback;
        return TRUE;
    }
    ...
    if (processorCount > RTL_NUMBER_OF(HookEntry->TargetPte)) {
        processorCount = RTL_NUMBER_OF(HookEntry->TargetPte);  // clamp 到 64
    }
    ...
    for (cpuIndex = 0; cpuIndex < processorCount; cpuIndex++) {
        ...
        if (pde->Value & (1ULL << 7)) {
            ...
            newPtTable = EptSplit2MBPage((PEPT_ENTRY)pde, PhysicalAddress & ~0x1FFFFFULL);
            if (!newPtTable) {
            ...
            if (eptTables->SplitPtCount < RTL_NUMBER_OF(eptTables->SplitPt)) {
                eptTables->SplitPt[eptTables->SplitPtCount] = (PEPT_PTE)newPtTable;
                eptTables->SplitPtPhysical[eptTables->SplitPtCount].QuadPart = newPtPhysical;
            ...
            PEPT_PTE existingPt = NULL;
            for (ULONG i = 0; i < eptTables->SplitPtCount; i++) {
                if ((ULONG64)(eptTables->SplitPtPhysical[i].QuadPart >> 12) == ptPfn) {
                    existingPt = eptTables->SplitPt[i];
                    break;
                }
            }
            if (existingPt) {
                ...
                ptPhys.QuadPart = ((ULONG64)pde->PageFrameNumber) << 12;
                newPtTable = (PEPT_ENTRY)EptMapPhysicalToVirtual(ptPhys.QuadPart);
                if (!newPtTable) {
                    continue;
                }
                HookEntry->TargetPte[cpuIndex] = (PEPT_PTE_ENTRY)&newPtTable[ptIndex];
    ...
    if (successCount == 0) {
        DbgPrint("[EPT-Hook] EptSetupPerCpuPtesForHook: 0 CPUs succeeded for PA 0x%llx\n",
                 PhysicalAddress);
        return FALSE;
    }
...
)
{
    ...
    // 写入 jmp [rip+0]; address
    // 2026-06-16: 与 EptCreateTrampoline 同款保护 — 本地栈缓冲拼好 14 字节再单次写入,
    // 消除"jmp + 全 0 占位"中间态(csrss 0x3B 蓝屏根因之一)。
    {
...
)
{
    ...
    // P1-3: 仅在 root mode 调用 (EptHookHandleViolation 内),所以只动当前 CPU
    ...
    if (HookEntry->FakePagePhysical == 0) {
        return;
    }
...
)
{
    ...
    // P1-3: root mode 调用,只动当前 CPU 的 PTE
    cpuIdx = KeGetCurrentProcessorNumber();
    if (cpuIdx >= 64) cpuIdx = 0;
    ...
    if (HookEntry->TargetPhysicalAddress == 0) {
        return;
    }
...
)
{
    ...
    if (g_NtQuerySystemInformationHook != NULL && 
        g_NtGetNextProcessHook != NULL) {
...
EptHookInstallFileHideHook(VOID)
{
    ...
    if (g_NtQueryDirectoryFileHook != NULL) {
        DbgPrint("[EPT-Hook] NtQueryDirectoryFile hook already installed\n");
        return STATUS_SUCCESS;
    }
    ...
    DbgPrint("[EPT-Hook] NtQueryDirectoryFile at %p\n", ntQueryDirectoryFile);
...
EptHookRemoveDriverHideHook(VOID)
{
    ...
    if (g_ObReferenceObjectByNameHook) {
        ...
        g_OriginalObReferenceObjectByName = NULL;
    }
