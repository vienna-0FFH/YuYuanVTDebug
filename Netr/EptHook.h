...
typedef enum _EPT_HOOK_STATE {
    EptHookStateInactive,   // 未激活
    EptHookStateActive,     // 已激活
    EptHookStatePaused,     // 暂停（用于读取原始代码）
} EPT_HOOK_STATE;
...
typedef struct _EPT_HOOK_ENTRY {
    ...
    // 安装时遍历填充 (PASSIVE_LEVEL); root mode 中用 [KeGetCurrentProcessorNumber()]。
...
typedef struct _EPT_MTF_CONTEXT {
    volatile PEPT_HOOK_ENTRY PendingHookEntry;   // 需要恢复的 Hook 条目
    volatile LONG MtfActive;                      // MTF 是否激活 (0=否, 1=是)
 ...
 * @param OutHookEntry   返回 Hook 条目（可选）
 * @return NTSTATUS
 */
NTSTATUS
EptHookInstall(
 ...
 */
VOID EptCheckAndInvalidateTlb(VOID);
