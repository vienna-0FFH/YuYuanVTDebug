...
// EPT页表
#define HV_EPT_MAX_SPLIT_PTS 512
...
typedef struct _EPT_TABLES {
    ...
    PEPT_PTE SplitPt[HV_EPT_MAX_SPLIT_PTS];
    PHYSICAL_ADDRESS SplitPtPhysical[HV_EPT_MAX_SPLIT_PTS];  // 与 SplitPt 同步:每个 PT 的 HPA,供 root 模式无 Mm* 查找
    BOOLEAN SplitPtOwned[HV_EPT_MAX_SPLIT_PTS];
    ULONG SplitPtCount;
...
typedef struct _VCPU_DATA {
    ...
    BOOLEAN IsVirtualized;
    BOOLEAN IsVmxOn;                       // VMXON succeeded; permits partial-start rollback
    PVOID VmExitStack;
