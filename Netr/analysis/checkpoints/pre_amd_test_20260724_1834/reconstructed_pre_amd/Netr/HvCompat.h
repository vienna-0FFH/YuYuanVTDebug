...
//
// 默认 ON: AMD 路径使用 ASID 池、扩展 NPT、vGIF 门控及合并 IOPM/MSRPM。
// 该路径已通过静态审计和编译，仍需 AMD 真机覆盖；失败路径必须保持 fail-closed。
#ifndef HV_ENABLE_SVM_HARDENING
#define HV_ENABLE_SVM_HARDENING 1
#endif

// AMD NPT cloaking gate (HvCloak 镜像到 NPT)
// 默认 0:Intel VMX 路径下的 overlay EPT 可用；AMD 仍走传统 NPT identity。
// 当前 NPT overlay 生命周期尚未实现，不能只翻 gate；需先补齐对称的
// page ownership、MTF/rundown、rollback 与卸载路径。
...
// Compile the nested VMX/SVM implementation.  The runtime gate still starts
// closed and is published only after HvVtRoot plus all fixed per-VCPU pools
// have been initialized successfully.  Compilation support therefore does not
// imply that a partially initialized nested path can execute.
#ifndef HV_ENABLE_NESTED_VIRTUALIZATION
#define HV_ENABLE_NESTED_VIRTUALIZATION 1
#endif
