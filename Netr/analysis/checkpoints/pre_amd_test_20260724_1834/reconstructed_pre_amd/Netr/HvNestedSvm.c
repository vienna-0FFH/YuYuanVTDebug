 ...
 */
#if 0
// Historical Windows-context mapper retained for reference only. Nested SVM
// VM-exit handlers use the per-vCPU preallocated buffer and VtRoot scratch.
...
static BOOLEAN NestedSvmTranslateGpaToHpa(PVCPU_DATA VcpuData, ULONG64 Gpa, PULONG64 Hpa)
{
    ...
    return NestedNptTranslateL1GpaToHpa(
        VcpuData, Gpa, Hpa, &permissions);
#if 0
...
NTSTATUS NestedSvmInitialize(VOID)
{
    ...
    for (ULONG i = 0; i < g_HypervisorContext.ProcessorCount; ++i) {
        status = NestedSvmInitializeState(&g_HypervisorContext.VcpuData[i]);
        if (!NT_SUCCESS(status)) {
            for (ULONG j = 0; j < i; ++j) {
                NestedSvmCleanupState(&g_HypervisorContext.VcpuData[j]);
            }
            return status;
        }
    }
...
VOID NestedSvmCleanup(VOID)
{
    if (g_HypervisorContext.VcpuData) {
        for (ULONG i = 0; i < g_HypervisorContext.ProcessorCount; ++i) {
            NestedSvmCleanupState(&g_HypervisorContext.VcpuData[i]);
        }
    }
}
...
NTSTATUS NestedSvmInitializeState(PVCPU_DATA VcpuData)
{
    ...
    if (!VcpuData) return STATUS_INVALID_PARAMETER;

    nested = &VcpuData->NestedSvm;
    RtlZeroMemory(nested, sizeof(*nested));
    maxAddress.QuadPart = -1LL;

    nested->Vmcb12 = MmAllocateContiguousMemory(PAGE_SIZE, maxAddress);
    nested->MergedMsrpm = MmAllocateContiguousMemory(PAGE_SIZE * 2, maxAddress);
    nested->MergedIopm = MmAllocateContiguousMemory(PAGE_SIZE * 3, maxAddress);
    if (!nested->Vmcb12 || !nested->MergedMsrpm || !nested->MergedIopm) {
        NestedSvmCleanupState(VcpuData);
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    nested->Vmcb12Physical = MmGetPhysicalAddress(nested->Vmcb12);
    ...
    if (nested->Vmcb12Physical.QuadPart == 0 ||
    ...
    RtlZeroMemory(nested->MergedIopm, PAGE_SIZE * 3);
...
VOID NestedSvmCleanupState(PVCPU_DATA VcpuData)
{
    ...
    if (nested->NestedNptTables) {
        NestedNptDestroyContext(
            (PNESTED_NPT_CONTEXT)nested->NestedNptTables);
        nested->NestedNptTables = NULL;
    }

    // These buffers are provisioned for every AMD vCPU at initialization,
    // independent of the hardening gate, so their lifetime is unconditional.
    ...
    PNESTED_VMCB_CACHE Cache)
{
    ...
    vmcb = (PVMCB)nested->Vmcb12;
    if (!vmcb || !HvNestedRootReadPhysical(
            VcpuData, vmcbHpa, vmcb, PAGE_SIZE)) {
        return FALSE;
    }
    ...
    PNESTED_VMCB_CACHE Cache)
{
    ...
    return HvNestedRootWritePhysical(
        VcpuData, vmcbHpa, vmcb, PAGE_SIZE);
}
    ...
    PNESTED_VMCB_MERGED_CONTROLS Merged)
{
...
#if HV_ENABLE_SVM_HARDENING
    {
        PUCHAR scratch = (PUCHAR)nested->Vmcb12;

        if (!nested->MergedMsrpm || !nested->MergedIopm || !scratch) {
            ...
            HvNestedRecordEvent(
                VcpuData, HvNestedEventPoolExhausted, 'MVMS', 0, 0);
        } else {
            if (VcpuData->MsrPermissionMap) {
                __movsb(
                    (PUCHAR)nested->MergedMsrpm,
                    (const PUCHAR)VcpuData->MsrPermissionMap,
                    PAGE_SIZE * 2);
            } else {
                __stosb((PUCHAR)nested->MergedMsrpm, 0, PAGE_SIZE * 2);
            }

            if (Vmcb12->MsrpmBasePa != 0) {
                for (ULONG page = 0; page < 2; ++page) {
                    ULONG64 hpa = 0;
                    PUCHAR merged =
                        (PUCHAR)nested->MergedMsrpm + page * PAGE_SIZE;
                    if (!NestedSvmTranslateGpaToHpa(
                            VcpuData,
                            Vmcb12->MsrpmBasePa + page * PAGE_SIZE,
                            &hpa) ||
                        !HvNestedRootReadPhysical(
                            VcpuData, hpa, scratch, PAGE_SIZE)) {
                        __stosb(merged, 0xFF, PAGE_SIZE);
                        continue;
                    }
                ...
                Merged->MsrpmBasePa =
                    (ULONG64)nested->MergedMsrpmPhysical.QuadPart;
            ...
            if (Vmcb12->IopmBasePa != 0) {
                for (ULONG page = 0; page < 3; ++page) {
                    ULONG64 hpa = 0;
                    PUCHAR merged =
                        (PUCHAR)nested->MergedIopm + page * PAGE_SIZE;
                    if (!NestedSvmTranslateGpaToHpa(
                            VcpuData,
                            Vmcb12->IopmBasePa + page * PAGE_SIZE,
                            &hpa) ||
                        !HvNestedRootReadPhysical(
                            VcpuData, hpa, merged, PAGE_SIZE)) {
                        __stosb(merged, 0xFF, PAGE_SIZE);
                    }
                }
                Merged->IopmBasePa =
                    (ULONG64)nested->MergedIopmPhysical.QuadPart;
    ...
    nested->Vmcb12Physical.QuadPart = 0;
    VcpuData->IsInL2 = FALSE;
}
    ...
    PNESTED_VMCB_CACHE Vmcb12)
{
    ...
    if (merged.NpEnabled) {
        ...
        nptContext = (PNESTED_NPT_CONTEXT)nested->NestedNptTables;
        if (nptContext && nptContext->L1NCr3 != Vmcb12->NCr3) {
            NestedNptReleaseContext(nptContext);
            nested->NestedNptTables = NULL;
            nptContext = NULL;
        }
        if (!nptContext) {
            nptContext = NestedNptGetOrCreateContext(
                VcpuData, Vmcb12->NCr3);
        }
        if (nptContext) {
            ...
            DbgPrint("[HV-NESTED-SVM] Failed to create nested NPT context; entry rejected\n");
            return STATUS_INSUFFICIENT_RESOURCES;
        }
    }
    ...
    HvNestedRecordEvent(
        VcpuData, HvNestedEventSvmVmrun, 0,
        nested->VmcbGpa, nested->L1NCr3);
    ...
    ULONG64 ExitInfo2)
{
    ...
    // 更新 VMCB12 控制区的退出信息
    {
        ...
        vmcb12 = (PVMCB)nested->Vmcb12;
        if (vmcb12 && HvNestedRootReadPhysical(
                VcpuData, vmcb12Hpa, vmcb12, PAGE_SIZE)) {
            ...
            (VOID)HvNestedRootWritePhysical(
                VcpuData, vmcb12Hpa, vmcb12, PAGE_SIZE);
        }
    }
}
    ...
    ULONG64 ExitInfo2)
{
    ...
    HvNestedRecordEvent(
        VcpuData, HvNestedEventSvmExit, ExitCode,
        ExitInfo1, ExitInfo2);
}
...
static VOID NestedSvmSaveToHostSaveArea(PVCPU_DATA VcpuData, ULONG64 HostSaveAreaGpa)
{
    ...
    // 映射 Host Save Area
    hostSave = nested->Vmcb12;
    if (!hostSave || !HvNestedRootReadPhysical(
            VcpuData, hpa, hostSave, PAGE_SIZE)) {
        return;
    }
...
static VOID NestedSvmLoadFromHostSaveArea(PVCPU_DATA VcpuData)
{
    ...
    UNREFERENCED_PARAMETER(hostSave);
}
    ...
    PGUEST_CONTEXT GuestContext)
{
    ...
    if (!HvNestedCanEnterL2()) {
        NestedSvmInjectEvent(
            VcpuData, 6, SVM_EVENT_TYPE_EXCEPTION, FALSE, 0);
        return TRUE;
    }
    ...
    hostSaveGpa = nested->HostSaveGpa;
    ...
    PGUEST_CONTEXT GuestContext)
{
    ...
    // 映射目标 VMCB
    targetVmcb = (PVMCB)VcpuData->NestedSvm.Vmcb12;
    if (!targetVmcb || !HvNestedRootReadPhysical(
            VcpuData, hpa, targetVmcb, PAGE_SIZE)) {
        NestedSvmInjectEvent(VcpuData, 14, SVM_EVENT_TYPE_EXCEPTION, TRUE, 0);
        return TRUE;
    }
    ...
    UNREFERENCED_PARAMETER(targetVmcb);
    ...
    PGUEST_CONTEXT GuestContext)
{
    ...
    if (!HvNestedRootWritePhysical(
            VcpuData, hpa, targetVmcb, PAGE_SIZE)) {
        NestedSvmInjectEvent(
            VcpuData, 14, SVM_EVENT_TYPE_EXCEPTION, TRUE, 0);
        return TRUE;
    }
