    ...
    if (ctx) {
        ...
        __try {
            ...
            if (HvGetCpuVendor() == CPU_VENDOR_AMD) {
                result = AsmVmmcallWithResult(ctx->VmcallNum);
            } else {
                result = AsmVmCallWithResult(ctx->VmcallNum);
            }
            if (ctx->VmcallNum == 1 && result != 0xDEADBEEF) {
...
)
{
    ...
    if ((ULONG)ctx.SuccessCount == g_HypervisorContext.ProcessorCount &&
        ctx.FailureCount == 0) {
