...
// Keep driver startup passive. The debugger bridge registers explicitly when
// it is injected; polling must never arm kernel EPT hooks as a side effect.
export const AUTO_REGISTER_GUI_WITH_DRIVER = false;
...
interface DriverStore {
  ...
  /** GUI 自身是否已注册到驱动保护；被动模式下保持 false */
...
export const useDriver = create<DriverStore>((set, get) => ({
  ...
  refreshService: async () => {
    try {
      ...
      // device 打开时持续重试 license；自我注册只在显式开启主动模式时执行。
      if (deviceOpen) {
        ...
        if (AUTO_REGISTER_GUI_WITH_DRIVER && licensed && !get().selfProtected) {
        ...
        // 边沿 true→false:复位
        set({ status: null, statusError: null, selfProtected: false, selfProtectError: null });
        void invoke("dbg_set_self_protected", { value: false }).catch(() => undefined);
      } else if (!deviceOpen && get().status) {
        set({ status: null, statusError: null });
      }
    } catch {
      set({ service: null, deviceOpen: false, status: null });
    }
  },
  ...
  startPolling: (ms = 1500) => {
    ...
    const tick = async () => {
      ...
      running = true;
      try {
        // 状态查询依赖 deviceOpen，必须等服务/设备查询完成后再执行。
        await get().refreshService();
