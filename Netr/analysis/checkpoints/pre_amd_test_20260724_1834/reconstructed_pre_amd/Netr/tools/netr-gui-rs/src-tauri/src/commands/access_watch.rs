...
const HWBP_ALLOW_VT: u32 = 0x0000_0001;
const HWBP_MODE_VT: u64 = 1;

const _: [(); 32] = [(); std::mem::size_of::<HwbpReq>()];
const _: [(); 16] = [(); std::mem::size_of::<PullReq>()];
const _: [(); 16] = [(); std::mem::size_of::<PullResHdr>()];
const _: [(); 248] = [(); std::mem::size_of::<DbgEvtRaw>()];
...
fn pull_dbgevt(device: &DeviceState, since: u64) -> Result<(u64, Vec<DbgEvtRaw>), String> {
    ...
    let hdr: PullResHdr = unsafe { std::ptr::read_unaligned(out.as_ptr() as *const PullResHdr) };
    let count = { let c = hdr.count; c };
    let next_seq = { let n = hdr.next_seq; n };
    let items_off = std::mem::size_of::<PullResHdr>();
    let available = (written as usize).saturating_sub(items_off) / std::mem::size_of::<DbgEvtRaw>();
    let safe_count = (count as usize).min(PULL_MAX as usize).min(available);
    let mut evts = Vec::with_capacity(safe_count);
    for i in 0..safe_count {
        let p = unsafe {
            out.as_ptr().add(items_off + i * std::mem::size_of::<DbgEvtRaw>()) as *const DbgEvtRaw
        };
        evts.push(unsafe { std::ptr::read_unaligned(p) });
    }
    Ok((next_seq, evts))
}
...
pub async fn dbg_find_who_accesses(
    ...
    let device_ref = device.inner();

    // 1) 拉光当前 ring, 拿 baseline next_seq
    let (baseline_seq, _) = pull_dbgevt(device_ref, 0)
    ...
    let mut out = [0u8; 32];
    let set_written = device_ref.ioctl(IOCTL_HV_DBG_SET_HWBP, hwbp_buf, &mut out)
        .map_err(|e| AppError::Internal(format!("装 HWBP: {e:?}")))?;
    if set_written < 16 {
        return Err(AppError::Internal(format!("SET_HWBP returned {set_written} bytes")));
    }
    let set_status = u32::from_le_bytes(out[0..4].try_into().unwrap());
    let set_mode = u64::from_le_bytes(out[8..16].try_into().unwrap());
    if set_status != 0 || set_mode != HWBP_MODE_VT {
        return Err(AppError::Internal(format!(
            "SET_HWBP did not install a VT watchpoint (status={set_status}, mode={set_mode})"
        )));
    }
    ...
    while elapsed < duration_ms {
        ...
        elapsed += 100;
        match pull_dbgevt(device_ref, seq) {
            Ok((next, evts)) => {
                seq = next;
                for e in evts {
    ...
    let clear_written = device_ref
        .ioctl(IOCTL_HV_DBG_CLEAR_HWBP, clear_buf, &mut out)
        .map_err(|e| AppError::Internal(format!("clear VT HWBP: {e:?}")))?;
    if clear_written < 16 {
        return Err(AppError::Internal(format!("CLEAR_HWBP returned {clear_written} bytes")));
    }
    let clear_status = u32::from_le_bytes(out[0..4].try_into().unwrap());
    let clear_mode = u64::from_le_bytes(out[8..16].try_into().unwrap());
    if clear_status != 0 || clear_mode != HWBP_MODE_VT {
        return Err(AppError::Internal(format!(
            "CLEAR_HWBP did not remove the VT watchpoint (status={clear_status}, mode={clear_mode})"
        )));
    }
