...
export function StackPanel() {
  const { pid, ctx, setAddress, setMainTab, openBottomHexAt } = useSession();
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const r = await dbgIpc.readMemory(pid, ctx.rsp, 256 * 8);
        if (cancelled) return;
    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex h-9 shrink-0 items-center gap-2 border-b border-border bg-card/60 px-3 text-xs">
        ...
        {ctx ? (
          <>
            ...
            <span className="text-muted-foreground/50">256 QWORD</span>
          </>
        ) : (
      ...
      <div className="min-h-0 flex-1 overflow-auto font-mono text-[11px]">
        {qwords.length === 0 ? (
          ...
          qwords.map((q, i) => (
            <ContextMenu key={q.addr.toString()}>
              ...
              <ContextMenuContent>
                <ContextMenuItem onClick={() => openBottomHexAt(q.value)}>
                  Hex跳转到栈中值
                </ContextMenuItem>
                <ContextMenuItem onClick={() => openBottomHexAt(q.addr)}>
                  Hex跳转到栈址
                </ContextMenuItem>
              </ContextMenuContent>
            </ContextMenu>
