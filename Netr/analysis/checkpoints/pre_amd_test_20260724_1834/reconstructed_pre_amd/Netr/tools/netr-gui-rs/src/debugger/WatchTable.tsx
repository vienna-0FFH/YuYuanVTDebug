...
export function WatchTable() {
  const {
    pid, watches, addWatch, removeWatch, updateWatch, clearWatches, setAddress, setMainTab,
    debugMode, capabilities,
  } = useSession();
  const hwbpReason = hwBreakpointUnavailableReason(debugMode, capabilities);
              ...
              {watches.map((w) => (
                ...
                <tr className={cn("border-b border-border/40 hover:bg-accent/40", w.frozen && "bg-warning/5")}>
                  ...
                  <ContextMenuContent>
                    ...
                    <ContextMenuSeparator />
                    <ContextMenuItem
                      onClick={() => setAccessDialog({ addr: w.address, mode: "write" })}
                      disabled={Boolean(hwbpReason)}
                      title={hwbpReason ?? undefined}
                    >
                      🔍 找出谁改写了此地址
                    </ContextMenuItem>
                    <ContextMenuItem
                      onClick={() => setAccessDialog({ addr: w.address, mode: "rw" })}
                      disabled={Boolean(hwbpReason)}
                      title={hwbpReason ?? undefined}
                    >
                      🔍 找出谁访问了此地址
