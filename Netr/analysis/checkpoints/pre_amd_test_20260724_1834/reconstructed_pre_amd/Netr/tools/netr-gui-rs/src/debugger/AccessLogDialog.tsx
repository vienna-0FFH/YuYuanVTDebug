...
export function AccessLogDialog({
  ...
  }, [open, address, mode, length, pid, unavailableReason]);
      ...
      <div className="flex flex-col gap-2 px-3 py-2 text-[11px]">
        <div className="flex items-center gap-2">
          <span className={cn(
            ...
            {installed ? "HWBP 监听中" : unavailableReason ? "当前会话不可用" : "未启用"}
          </span>
          {unavailableReason && (
            <span className="max-w-md truncate text-muted-foreground" title={unavailableReason}>
              {unavailableReason}
            </span>
          )}
          ...
          {[1, 2, 4, 8].map((n) => (
            <button
              ...
              className={cn(
                ...
                unavailableReason && "cursor-not-allowed opacity-45",
