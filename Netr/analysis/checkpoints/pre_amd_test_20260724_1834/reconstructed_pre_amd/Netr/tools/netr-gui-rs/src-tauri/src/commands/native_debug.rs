...
use crate::ioctl::codes::{
    IOCTL_HV_BRIDGE_BIND_TARGET, IOCTL_HV_BRIDGE_UNBIND_TARGET,
};
...
impl NativeEngine {
    ...
    fn cancel_step_over_for_thread(&mut self, tid: u32) -> Result<(), String> {
    ...
    match command {
        ...
        LoopCommand::ContinueEntry { tid, reply } => {
            ...
            let result = if was_initial_system_breakpoint {
                unsafe { ContinueDebugEvent(event.pid, event.tid, DBG_CONTINUE) }
                    .map_err(|error| {
                        string_error("ContinueDebugEvent(initial system breakpoint)", error)
                    })
            } else {
                engine.prepare_resume(event, ResumeAction::Continue).and_then(|_| {
                    unsafe { ContinueDebugEvent(event.pid, event.tid, event.continue_status) }
                        .map_err(|error| string_error("ContinueDebugEvent(native)", error))
                })
            };
            if result.is_ok() {
                *pending = None;
        ...
        LoopCommand::ContinueInitialSystemBreakpoint { tid, reply } => {
            ...
            let result = unsafe { ContinueDebugEvent(event.pid, event.tid, DBG_CONTINUE) }
                .map_err(|error| {
                    string_error("ContinueDebugEvent(initial system breakpoint)", error)
                });
            if result.is_ok() {
        ...
        LoopCommand::StepInto { tid, reply } => {
            if let Some(event) = pending.as_mut() {
                ...
                let result = engine.prepare_resume(event, ResumeAction::StepInto).and_then(|_| {
                    unsafe { ContinueDebugEvent(event.pid, event.tid, event.continue_status) }
                        .map_err(|error| string_error("ContinueDebugEvent(step-into)", error))
                });
                if result.is_ok() {
                    *pending = None;
                    pending_tid.store(0, Ordering::Release);
                    ...
                    .and_then(|_| unsafe {
                        ContinueDebugEvent(event.pid, event.tid, event.continue_status)
                            .map_err(|error| {
                                ...
                                string_error("ContinueDebugEvent(VT step-into)", error)
                            })
                    });
                if result.is_ok() {
                    *pending = None;
        ...
        LoopCommand::StepOver {
            ...
            if let Some(event) = pending.as_mut() {
                ...
                let result = decoded.and_then(|(action, mut result)| {
                    ...
                    unsafe { ContinueDebugEvent(event.pid, event.tid, event.continue_status) }
                        .map_err(|error| string_error("ContinueDebugEvent(step-over)", error))?;
        ...
        LoopCommand::StepOut { tid, reply } => {
            ...
            let result = engine.return_address(tid).and_then(|return_address| {
                if let Some(event) = pending.as_mut() {
                    ...
                    unsafe { ContinueDebugEvent(event.pid, event.tid, event.continue_status) }
                        .map_err(|error| string_error("ContinueDebugEvent(step-out)", error))?;
                    *pending = None;
                    pending_tid.store(0, Ordering::Release);
                    stage.store(STAGE_ENTRY_CONTINUED, Ordering::Release);
                } else {
...
fn handle_single_step_event(
    ...
    let reference_vt_hardware_hit = vt_hardware_mask != 0
        && !has_rearm
        && !native_tf_step
        && debug_state.dr6 & (DR6_BREAKPOINT_MASK | DR6_SINGLE_STEP) == 0;
    if vt_hardware_mask != 0 {
        private_dbgk_entry_diag(
            engine.pid,
            "hwbp.single_step.classify",
            format!(
                "tid={} rip=0x{rip:X} dr6=0x{:X} configured=0x{configured_mask:X} vt=0x{vt_hardware_mask:X} hit=0x{hardware_mask:X} reference_zero_dr6={reference_vt_hardware_hit}",
                event.dwThreadId,
                debug_state.dr6,
            ),
        );
    ...
    } else if reference_vt_hardware_hit && vt_hardware_mask.count_ones() == 1 {
        Some(vt_hardware_mask.trailing_zeros())
    } else {
        None
    };
    ...
    if hardware_mask != 0 || reference_vt_hardware_hit {
        stage.store(STAGE_ENTRY_PENDING, Ordering::Release);
        pending_tid.store(event.dwThreadId, Ordering::Release);
