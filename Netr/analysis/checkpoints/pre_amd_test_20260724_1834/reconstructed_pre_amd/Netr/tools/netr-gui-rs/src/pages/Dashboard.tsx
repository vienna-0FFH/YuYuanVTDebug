    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <KpiCard
          icon={Activity}
          label="CPU"
          value={status ? formatCpuCount(status.cpu_count) : "—"}
          tone="info"
        ...
        <KpiCard
          ...
          tone="info"
          hint={
            status?.debugger_proxy_enabled
              ? "Proxy 已开"
              : status && status.debuggers > 0
              ? "核心 Hook 未就绪"
              : "待注册调试器"
          }
        />
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          <DetailCard
            ...
            items={[
              ...
              ["VT-root", status.vt_root_enabled ? pill("ON", "success") : pill("编译关闭", "muted")],
              ["Hook 已装", boolPill(status.hook_initialized, "ON", "OFF")],
              ["AntiVMP 扩展", status.anti_anti_debug ? pill("ON", "success") : pill("按需关闭", "muted")],
            ]}
          />
          <DetailCard
            icon={ShieldCheck}
            ...
            items={[
              ...
              ["访问绕过", status.access_bypass_enabled ? pill("ON", "success") : pill("未启用", "muted")],
              [
                "调试器代理",
                status.debugger_proxy_enabled
                  ? pill("ON", "success")
                  : status.debuggers > 0
                  ? pill("Hook 未就绪", "warning")
                  : pill("待注册", "muted"),
              ],
            ]}
          />
          <DetailCard
            ...
            items={[
              ...
              ["逻辑核", pill(formatCpuCount(status.cpu_count), "info")],
...
function formatCpuCount(value: number): string {
  ...
  return browserValue > 0 ? `${browserValue} 核` : "—";
}
