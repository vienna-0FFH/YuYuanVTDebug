...
export function MainViewPanel() {
  const { address, mainTab, setMainTab, pid, ctx, requestDisasmNavigation } = useSession();
  ...
  async function go() {
    ...
    try {
      ...
      requestDisasmNavigation(v, "force");
      setGotoText("");
    } catch (e) {
    ...
    <div className="flex h-full flex-col bg-background">
      <div className="flex h-9 shrink-0 items-center gap-2 border-b border-border bg-card/60 px-3 text-xs">
        ...
        <div className="flex h-6 rounded-md border border-border bg-background p-0.5">
          {(["disasm", "hex", "bps"] as MainTab[]).map((t) => (
            <button
              key={t}
              type="button"
...
function labelOf(t: MainTab): string {
  return ({ disasm: "反汇编", hex: "Hex", bps: "软断点" } as const)[t];
}
