...
| **注入** | Shellcode/APC 与 DLL 映射骨架；DLL TLS/DllMain 完成语义仍阻断 | `HvInjection` |
...
| API | IOCTL | 说明 |
|---|---|---|
| `HvInjectDllFromFile(...)` | `IOCTL_HV_INJECT_DLL` (0x60) | 当前可解析/映射 PE，但 TLS callback 与 DllMain 尚未执行，不能把返回成功视为模块已初始化 |

当前 driver 请求结构为固定路径 ABI：`TargetPid + DllPath[520] + Flags`。在补齐 TLS/DllMain、失败回滚和目标退出生命周期前，GUI/Bridge 不应把 DLL 注入显示为 production-ready。
