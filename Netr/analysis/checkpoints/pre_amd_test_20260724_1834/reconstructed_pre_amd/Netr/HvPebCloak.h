...
#define HV_PEB_CLOAK_MAX_PATCHES_PER_PAGE 4

#define HV_PEB_TARGET_FREE       0
#define HV_PEB_TARGET_BUILDING   1
#define HV_PEB_TARGET_ACTIVE     2
#define HV_PEB_TARGET_RETIRING   3
#define HV_PEB_TARGET_QUIESCED   4

#define HV_PEB_PATCH_SET_BYTE    1
#define HV_PEB_PATCH_AND_DWORD   2
#define HV_PEB_PATCH_SET_DWORD   3

#define HV_PEB_MTF_IDLE          0
#define HV_PEB_MTF_PUBLISHING    1
#define HV_PEB_MTF_ARMED         2
#define HV_PEB_MTF_COMPLETING    3

// Per-page VM-exit rundown.  The high bit closes the page against new root
// users while the low bits count users that may retain the page until MTF.
#define HV_PEB_PAGE_ROOT_CLOSING 0x40000000L
#define HV_PEB_PAGE_ROOT_COUNT_MASK 0x3FFFFFFFL

typedef struct _HV_PEB_PATCH_RULE {
    ULONG Offset;
    ULONG Operation;
    ULONG Value;
} HV_PEB_PATCH_RULE, *PHV_PEB_PATCH_RULE;
...
#define HV_PEB_OFF_NT_GLOBAL_FLAG   0x0BC
...
typedef struct _HV_PEB_CLOAK_PAGE {
    ...
    PVOID          PatchVaAlt;
    ULONG64        PatchPfnAlt;
    volatile LONG  ActivePatchIndex;
    volatile LONG  RefreshBusy;
    volatile LONG  Stale;
    volatile LONG  RootGate;
    volatile LONG  PatchReaders[2];
    ULONG          PatchCount;
    HV_PEB_PATCH_RULE PatchRules[HV_PEB_CLOAK_MAX_PATCHES_PER_PAGE];
    PEPT_PTE_ENTRY CloakedPte[HV_PEB_CLOAK_MAX_CPUS];
    volatile LONG  Installed;       // 1=已在 EPT 上安装 R=0
    volatile LONG  Retiring;        // completion-time no-rearm gate
...
typedef struct _HV_PEB_CLOAK_TARGET {
    ...
    ULONG              PageCount;
    volatile LONG      Active;       // HV_PEB_TARGET_* state
    volatile LONG      PrivateDbgkTarget;
    volatile LONG      RebindFailures;
    volatile LONG64    RebindDueTime;
...
typedef struct _HV_PEB_MTF_CONTEXT {
    ...
    volatile LONG               Active;       // HV_PEB_MTF_* state
    UCHAR                       PassOriginal;
    UCHAR                       Retiring;
    UCHAR                       PatchIndex;
    UCHAR                       Reserved;
...
typedef struct _HV_PEB_CLOAK_MANAGER {
    ...
    volatile LONG          GloballyEnabled;     // 至少一个 target 注册时 = TRUE
    volatile LONG          ShuttingDown;
    volatile LONG          RootReaders;
    KEVENT                 RebindStopEvent;
    HANDLE                 RebindThreadHandle;
    EX_PUSH_LOCK           ControlLock;          // PASSIVE control-plane only
    volatile LONG          Generation;
    volatile LONG          LastClassGeneration[HV_PEB_CLOAK_MAX_CPUS];
    volatile LONG          LastOverlayTargetIsPrivateDbgk[HV_PEB_CLOAK_MAX_CPUS];
    ...
    volatile LONG64        PrivateDbgkKernelMainSwitches;
} HV_PEB_CLOAK_MANAGER, *PHV_PEB_CLOAK_MANAGER;

extern HV_PEB_CLOAK_MANAGER g_PebCloakManager;

// Shared PASSIVE_LEVEL serialization for all control-plane mutations of
// VcpuData->EptPebSpoof (PEB cloak, vwatch and private software breakpoints).
// VM-exit handlers must never acquire this lock.
...
/* Close new target/rebind admission without freeing overlay state. This is
 * the first unload phase; full teardown remains in HvPebCloakShutdown after
 * hook-owned asynchronous workers have drained. IRQL = PASSIVE_LEVEL. */
...
/*
 * PASSIVE_LEVEL query used while the caller holds OverlayMutation.  It keeps
 * PEB and Vwatch from independently programming the same overlay PTE until a
 * composed per-GPA owner exists.
 */
