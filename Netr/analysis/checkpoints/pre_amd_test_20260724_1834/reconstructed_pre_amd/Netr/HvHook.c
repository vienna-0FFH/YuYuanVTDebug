...
static volatile LONG g_HvHookClosing = FALSE;
static EX_RUNDOWN_REF g_HvHookCallbackRundown;
static BOOLEAN g_HvHookCallbackRundownEverInitialized = FALSE;
static volatile LONG g_HvHookCallbackRundownActive = FALSE;

/* NtReadFile may remain inside the original synchronous filesystem request
 * for an unbounded interval.  Publishing that global hook therefore lets an
 * unrelated read hold the driver-wide callback rundown during unload.  Keep
 * the fallback implementation available for focused work, but do not publish
 * it until it has a cancellation/lifetime design independent of the global
 * hook rundown.  NtCreateFile/NtOpenFile remain the primary image guard. */
#ifndef HV_ENABLE_BLOCKING_NTREADFILE_IMAGE_FALLBACK
#define HV_ENABLE_BLOCKING_NTREADFILE_IMAGE_FALLBACK 0
#endif

#define HV_HOOK_CALLBACK_BEGIN(_Handle, _FailureReturn)                      \
...
static BOOLEAN g_BridgeAntiDebugEnabled = FALSE;
...
static HV_HOOK_HANDLE g_HookNtQueryInformationThread = NULL;
static HV_HOOK_HANDLE g_HookNtSystemDebugControl = NULL;
...
#define HV_HOOK_OWNER_DEBUGGER_PROXY 0x00000001UL
#define HV_HOOK_OWNER_ANTIANTIDEBUG  0x00000002UL
#define HV_HOOK_OWNER_BRIDGE         0x00000004UL
#define HV_HOOK_OWNER_DRIVER_HIDE    0x00000008UL
#define HV_HOOK_OWNER_PRIVATE_DBGK   0x00000010UL

// Shared hook ownership is mutated only while g_DebuggerMutationLock is held.
// A handle may stay installed after one feature is disabled as long as another
// explicit owner remains.
static ULONG g_HookOwnersNtQueryInformationProcess = 0;
static ULONG g_HookOwnersNtQuerySystemInformation = 0;
static ULONG g_HookOwnersNtGetContextThread = 0;
static ULONG g_HookOwnersNtSetContextThread = 0;
static ULONG g_HookOwnersNtCreateThreadEx = 0;
static ULONG g_HookOwnersNtQueryObject = 0;
static ULONG g_HookOwnersNtDebugActiveProcess = 0;
static volatile LONG g_DriverHideEnabled = FALSE;
...
static PVOID g_PrivateDbgkIngressAddress = NULL;
static PVOID g_SymbolNtSetContextThread = NULL;
static PVOID g_SymbolNtReadVirtualMemory = NULL;
static PVOID g_SymbolNtWriteVirtualMemory = NULL;
...
static BOOLEAN HvHookpIsAntiAntiDebugTargetPid(_In_opt_ HANDLE TargetPid)
{
    if (!g_AntiAntiDebugEnabled || !TargetPid) {
        return FALSE;
    }
    if (g_AntiAntiDebugConfig.TargetPid != 0) {
        return (ULONG)(ULONG_PTR)TargetPid == g_AntiAntiDebugConfig.TargetPid;
    }
    return HvHookIsProtectedProcessPid(TargetPid);
}

static BOOLEAN HvHookpIsAntiAntiDebugCaller(VOID)
{
    HANDLE caller;

    if (!g_AntiAntiDebugEnabled || ExGetPreviousMode() != UserMode) {
        return FALSE;
    }
    caller = PsGetCurrentProcessId();
    if (!caller || HvHookIsDebuggerPid(caller) ||
        HvHookIsTrustedSystemCaller(caller)) {
        return FALSE;
    }
    return HvHookpIsAntiAntiDebugTargetPid(caller);
}
...
// Serializes 0->1 / 1->0 debugger transitions while hook sets are installed
// or removed. The list spin lock alone cannot cover those blocking operations.
static EX_PUSH_LOCK g_DebuggerMutationLock = 0;
static BOOLEAN g_DebuggerMutationLockInitialized = FALSE;
static EX_RUNDOWN_REF g_PebCloakWorkerRundown;
static BOOLEAN g_PebCloakWorkerRundownReady = FALSE;

static NTSTATUS HvHookpEnableDriverHideLocked(VOID);
static NTSTATUS HvHookpDisableDriverHideLocked(VOID);
static NTSTATUS HvHookpRegisterObCallback(VOID);
static VOID HvHookpUnregisterObCallback(VOID);
static NTSTATUS HvHookpAcquireDebuggerProtectOwnerLocked(VOID);
static VOID HvHookpReleaseDebuggerProtectOwnerLocked(VOID);
static NTSTATUS HvHookpAcquireAccessBypassObOwnerLocked(VOID);
static VOID HvHookpReleaseAccessBypassObOwnerLocked(VOID);
...
HvHookInitialize(VOID)
{
    ...
    InterlockedExchange(&g_HvHookClosing, FALSE);
    if (!g_HvHookCallbackRundownEverInitialized) {
        ExInitializeRundownProtection(&g_HvHookCallbackRundown);
        g_HvHookCallbackRundownEverInitialized = TRUE;
    } else if (InterlockedCompareExchange(
                   &g_HvHookCallbackRundownActive, 0, 0) == 0) {
        ExReInitializeRundownProtection(&g_HvHookCallbackRundown);
    }
    InterlockedExchange(&g_HvHookCallbackRundownActive, TRUE);

    // Initialize debugger/target bookkeeping before publishing
    // g_HvHookInitialized. Registration can then be safely serialized without
    // racing the former lazy list initialization path.
    if (!g_DebuggerMutationLockInitialized) {
        ExInitializePushLock(&g_DebuggerMutationLock);
        g_DebuggerMutationLockInitialized = TRUE;
    }
    ...
    if (!g_PebCloakWorkerRundownReady) {
        ExInitializeRundownProtection(&g_PebCloakWorkerRundown);
        g_PebCloakWorkerRundownReady = TRUE;
    }
...
HvHookCleanup(VOID)
{
    ...
    InterlockedExchange(&g_HvHookClosing, TRUE);

    // 阶段 7.9: 显式关闭 NtOpenProcess bypass,确保状态标志和句柄被清掉
    // (HvHookRemoveAll 会移除底层 hook,但不会清这一层的 g_AccessBypassEnabled)
    (void)HvHookDisableAccessBypass();

    /* Wait out any debugger/Bridge mutation that entered before Closing was
     * published, then keep list and object-callback ownership serialized for
     * the rest of teardown. */
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    ...
    if ((g_HvHookBackend == CPU_VENDOR_INTEL &&
        ...
        DbgPrint("[HvHook] Cleanup retained root-owned hook/page entries; manager stays published for retry\n");
        ExReleasePushLockExclusive(&g_DebuggerMutationLock);
        KeLeaveCriticalRegion();
        return;
    }

    /* AccessBypass disable can retain its owner state when an individual
     * inline remove fails.  RemoveAll above is the backend safety net, but an
     * Ob callback is not an EPT/NPT entry and must always be detached before
     * the driver image can be released. */
    HvHookpUnregisterObCallback();

    if (InterlockedCompareExchange(
            &g_HvHookCallbackRundownActive, FALSE, TRUE) == TRUE) {
        ExWaitForRundownProtectionRelease(&g_HvHookCallbackRundown);
    }
    HvHookDrainAsyncWorkers();

    /* Release lifetime references even if a debugger refused termination or
     * its process-exit callback did not complete before unload cleanup. */
    if (g_ProtectedProcessInitialized) {
        for (;;) {
            PPROTECTED_PROCESS process = NULL;
            KeAcquireSpinLock(&g_ProtectedProcessLock, &oldIrql);
            if (!IsListEmpty(&g_ProtectedProcessList)) {
                entry = RemoveHeadList(&g_ProtectedProcessList);
                process = CONTAINING_RECORD(
                    entry, PROTECTED_PROCESS, ListEntry);
            }
            KeReleaseSpinLock(&g_ProtectedProcessLock, oldIrql);
            if (!process) break;
            if (process->ProcessObject) {
                ObDereferenceObject(process->ProcessObject);
            }
            ExFreePoolWithTag(process, HV_HOOK_TAG);
        }
    }
    if (g_ProtectedDebuggerInitialized) {
        for (;;) {
            ...
            if (!IsListEmpty(&g_ProtectedDebuggerList)) {
                entry = RemoveHeadList(&g_ProtectedDebuggerList);
                debugger = CONTAINING_RECORD(
                    entry, PROTECTED_DEBUGGER, ListEntry);
            }
            KeReleaseSpinLock(&g_ProtectedDebuggerLock, oldIrql);
            if (!debugger) break;
            if (debugger->ProcessObject) {
                ObDereferenceObject(debugger->ProcessObject);
            }
            ExFreePoolWithTag(debugger, HV_HOOK_TAG);
        }
    }
    g_BridgeAntiDebugEnabled = FALSE;
    g_HookNtQueryInformationProcess = NULL;
    g_HookOwnersNtQueryInformationProcess = 0;
    g_HookOwnersNtQuerySystemInformation = 0;
    g_HookOwnersNtGetContextThread = 0;
    g_HookOwnersNtSetContextThread = 0;
    g_HookOwnersNtCreateThreadEx = 0;
    g_HookOwnersNtQueryObject = 0;
    g_HookOwnersNtDebugActiveProcess = 0;
    g_HookNtDebugActiveProcess = NULL;
    g_PrivateDbgkIngressAddress = NULL;
    g_SymbolNtSetContextThread = NULL;
    g_SymbolNtReadVirtualMemory = NULL;
    g_SymbolNtWriteVirtualMemory = NULL;
    InterlockedExchange(&g_DriverHideEnabled, FALSE);
...
HvHookDrainAsyncWorkers(VOID)
{
    ...
    DbgPrint("[HvHook] Draining asynchronous PEB cloak workers\n");
    ExWaitForRundownProtectionRelease(&g_PebCloakWorkerRundown);
    g_PebCloakWorkerRundownReady = FALSE;
    DbgPrint("[HvHook] Asynchronous PEB cloak workers drained\n");
}
...
)
{
    ...
    if (!OutHandle || !TargetAddress || !HookFunction) {
        return STATUS_INVALID_PARAMETER;
    }
    *OutHandle = NULL;
    if (!g_HvHookInitialized ||
        InterlockedCompareExchange(&g_HvHookClosing, 0, 0) != 0) {
    ...
    if (g_HvHookBackend == CPU_VENDOR_INTEL) {
        ...
        status = EptHookPrepare(
            TargetAddress, HookFunction, &entry, &trampoline);
        handle = (HV_HOOK_HANDLE)entry;
    } else if (g_HvHookBackend == CPU_VENDOR_AMD) {
    ...
    if (!handle || !trampoline || HvHookGetTrampoline(handle) != trampoline) {
        NTSTATUS removeStatus = handle
            ? HvHookRemove(handle) : STATUS_SUCCESS;
...
{
    if (!g_HvHookInitialized || !Handle) {
        return STATUS_INVALID_PARAMETER;
    }
    if (InterlockedCompareExchange(&g_HvHookClosing, 0, 0) != 0) {
        return STATUS_DELETE_PENDING;
    }
    if (g_HvHookBackend == CPU_VENDOR_INTEL) {
        return EptHookSetOverlayVisiblePrepared(
            (PEPT_HOOK_ENTRY)Handle, Visible);
    }
...
{
    if (!g_HvHookInitialized || !Handle || !HvHookGetTrampoline(Handle)) {
        return STATUS_INVALID_PARAMETER;
    }
    if (InterlockedCompareExchange(&g_HvHookClosing, 0, 0) != 0) {
        return STATUS_DELETE_PENDING;
    ...
    if (g_HvHookBackend == CPU_VENDOR_INTEL) {
        return EptHookActivate((PEPT_HOOK_ENTRY)Handle);
    }
    if (g_HvHookBackend == CPU_VENDOR_AMD) {
        return NptHookActivate((PNPT_HOOK_ENTRY)Handle);
    }
    return STATUS_NOT_SUPPORTED;
}
...
{
    NTSTATUS status;
    HV_HOOK_HANDLE installedHandle = NULL;
    ...
    if (g_HvHookBackend == CPU_VENDOR_INTEL) {
        ...
        installedHandle = (HV_HOOK_HANDLE)entry;
    } 
    else if (g_HvHookBackend == CPU_VENDOR_AMD) {
        PNPT_HOOK_ENTRY entry = NULL;
...
)
{
    ...
    return ExAcquireRundownProtection(&g_HvHookCallbackRundown);
}
...
{
    UNREFERENCED_PARAMETER(Handle);
    ExReleaseRundownProtection(&g_HvHookCallbackRundown);
}
...
static BOOLEAN HvHookpDebuggerHideSharedHooksPublished(VOID);
static BOOLEAN HvHookpAnySharedHideHookPublished(VOID);

static NTSTATUS
HvHookpAddHiddenProcessState(
    _In_ ULONG ProcessId
)
{
    if (g_HvHookBackend == CPU_VENDOR_INTEL) {
        return EptHookAddHiddenProcessState(ProcessId);
    }
    if (g_HvHookBackend == CPU_VENDOR_AMD) {
        return NptHookAddHiddenProcessState(ProcessId);
    }
    return STATUS_NOT_SUPPORTED;
}

static NTSTATUS
HvHookpRemoveHiddenProcessState(
    _In_ ULONG ProcessId
)
{
    if (g_HvHookBackend == CPU_VENDOR_INTEL) {
...
{
    if (g_HvHookBackend == CPU_VENDOR_INTEL) {
        return EptHookGetHiddenProcessCount();
    }
    if (g_HvHookBackend == CPU_VENDOR_AMD) {
        return NptHookGetHiddenProcessCount();
    }
    return 0;
}
...
)
{
    ...
    if (HvHookpDebuggerHideSharedHooksPublished()) {
        return HvHookpAddHiddenProcessState(ProcessId);
    }

    /* Never fall into a private backend installer while any common syscall
     * hook is already published: that recreates the same-page collision. */
    if (HvHookpAnySharedHideHookPublished()) {
...
)
{
    ...
    return HvHookpRemoveHiddenProcessState(ProcessId);
}
...
HvHookInstallFileHideHook(VOID)
{
    ...
    target = HvHookResolveNtRoutine(L"NtQueryDirectoryFile");
    if (!target) {
        return STATUS_PROCEDURE_NOT_FOUND;
    }

    if (g_HvHookBackend == CPU_VENDOR_INTEL) {
        return EptHookInstallFileHideHookAtAddress(target);
    } 
    else if (g_HvHookBackend == CPU_VENDOR_AMD) {
        return NptHookInstallFileHideHookAtAddress(target);
    }
...
)
{
    ...
    for (PLIST_ENTRY entry = g_HiddenMemoryList.Flink;
         ...
         entry = entry->Flink) {
        PHIDDEN_MEMORY_REGION existing = CONTAINING_RECORD(
            entry, HIDDEN_MEMORY_REGION, ListEntry);
...
)
{
    ...
    {
        NTSTATUS status = HvRemoveHiddenMemoryRegion(ProcessId, Address);
        if (!NT_SUCCESS(status)) {
            return status;
        }
    }
...
HvHookInstallDriverHideHook(VOID)
{
    ...
    DbgPrint("[HvHook] Installing Driver Hide Hooks (shared NtQSI owner)...\n");
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    status = HvHookpEnableDriverHideLocked();
    ExReleasePushLockExclusive(&g_DebuggerMutationLock);
    KeLeaveCriticalRegion();
    return status;
}
...
HvHookRemoveDriverHideHook(VOID)
{
    ...
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    status = HvHookpDisableDriverHideLocked();
    ExReleasePushLockExclusive(&g_DebuggerMutationLock);
    KeLeaveCriticalRegion();
    return status;
}
...
{
    HV_HOOK_CALLBACK_BEGIN(
        g_HookNtQueryInformationProcess, STATUS_DELETE_PENDING)
    ...
    // P121: AAD 反调试 spoof 启用条件:
    //   (a) g_AntiAntiDebugEnabled 显式 IOCTL 开 — 老路径
    //   (b) 查询目标已经绑定到某个 debugger — Bridge 自动路径
    //
    ...
    BOOLEAN autoTargetScope =
        (g_BridgeAntiDebugEnabled && targetPid != NULL &&
         (HvHookIsBridgeOwnedTargetPid(targetPid) ||
          HvPrivateDebugObjectIsTarget(targetPid)));
    BOOLEAN explicitTargetScope =
        (g_AntiAntiDebugEnabled &&
         g_AntiAntiDebugConfig.HookNtQueryInformationProcess &&
         HvHookpIsAntiAntiDebugTargetPid(targetPid));
    BOOLEAN doAntiDebugSpoof = explicitTargetScope || autoTargetScope;
    if (!doAntiDebugSpoof) {
        return status;
    }
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtQuerySystemInformation, STATUS_DELETE_PENDING)
    NTSTATUS status;
    ...
    // 隐藏内核调试器信息。扩展 AntiVMP 只作用于显式 PID 或 Bridge 绑定目标，
    // 不再把该结果对整台机器全局改写。
    if (g_AntiAntiDebugEnabled &&
        g_AntiAntiDebugConfig.HookNtQuerySystemInformation &&
        HvHookpIsAntiAntiDebugCaller() &&
        (SystemInformationClass == SystemKernelDebuggerInformation ||
         SystemInformationClass == 0x95)) {
    ...
    // Driver hiding owns this same hook explicitly.  Keep class 11 separate
    // from the process-hide count: self-hide must work before any process is
    // registered, and disabling the last debugger must not release this owner.
    if (SystemInformationClass == 11 &&
        SystemInformation &&
        SystemInformationLength >= sizeof(ULONG) &&
        InterlockedCompareExchange(&g_DriverHideEnabled, FALSE, FALSE) != 0)
    {
        ...
        __try {
            ...
            /* Filtering failure must not corrupt the original query result. */
        }
    }
...
{
    HV_HOOK_CALLBACK_BEGIN(
        g_HookNtQueryVirtualMemory, STATUS_DELETE_PENDING)
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtUserQueryWindow, NULL)
    PFN_NtUserQueryWindow Original =
        (PFN_NtUserQueryWindow)HvHookGetTrampoline(g_HookNtUserQueryWindow);
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtUserInternalGetWindowText, 0)
...
static HANDLE NTAPI HookedNtUserFindWindowEx(
    ...
    DWORD Type)
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtUserFindWindowEx, NULL)
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtUserGetClassName, 0)
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtUserWindowFromPoint, NULL)
...
static HANDLE NTAPI HookedNtUserChildWindowFromPointEx(
    HANDLE HwndParent, HV_POINT Point, ULONG Flags)
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtUserChildWindowFromPointEx, NULL)
...
{
    HV_HOOK_CALLBACK_BEGIN(
        g_HookNtSetInformationThread, STATUS_DELETE_PENDING)
    ...
    if (ThreadInformationClass == ThreadHideFromDebugger &&
        g_AntiAntiDebugConfig.HookNtSetInformationThread &&
        HvHookpIsAntiAntiDebugCaller()) {
        PETHREAD thread = NULL;
        NTSTATUS validateStatus = ObReferenceObjectByHandle(
            ThreadHandle,
            ...
            *PsThreadType,
            ...
            (PVOID*)&thread,
            NULL);
        if (NT_SUCCESS(validateStatus) && thread) {
            ObDereferenceObject(thread);
            HV_HOT_DBG("[HvHook-AAD] ThreadHideFromDebugger swallowed\n");
        }
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtClose, STATUS_DELETE_PENDING)
    ...
    // 先用对象管理器验证句柄，坏句柄直接返回，不进入 NtClose 的
    // debugger-only exception 路径。伪句柄同样不可关闭。
    ...
    if (handleInfo.HandleAttributes & 0x1UL) {
...
typedef struct _HV_AAD_OBJECT_TYPE_INFORMATION {
    ...
    USHORT TypeIndex;
    ULONG ReservedByte;
} HV_AAD_OBJECT_TYPE_INFORMATION, *PHV_AAD_OBJECT_TYPE_INFORMATION;

static PUCHAR HvHookpAlignUp8(_In_ PUCHAR Pointer)
{
    return (PUCHAR)(((ULONG_PTR)Pointer + 7ULL) & ~7ULL);
}
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtQueryObject, STATUS_DELETE_PENDING)
    NTSTATUS status;
    ...
    BOOLEAN privateObjectCaller = HvPrivateDebugObjectIsTarget(
        PsGetCurrentProcessId());
    if (!NT_SUCCESS(status) ||
        ObjectInformationClass != ObjectTypesInformation ||
        !ObjectInformation ||
        ObjectInformationLength < sizeof(ULONG) ||
        ((!g_AntiAntiDebugConfig.HookNtQueryObject ||
          !HvHookpIsAntiAntiDebugCaller()) &&
         !privateObjectCaller)) {
        return status;
    }
    ...
    __try {
        ...
        PUCHAR base = (PUCHAR)ObjectInformation;
        PUCHAR limit = base + ObjectInformationLength;
        ULONG count = *(PULONG)base;
        if (count > 0x1000) {
            return status;
        }

        PUCHAR cursor = HvHookpAlignUp8(base + sizeof(ULONG));
        static const WCHAR debugObjectName[] = L"DebugObject";
        const USHORT debugObjectBytes =
            (USHORT)(sizeof(debugObjectName) - sizeof(WCHAR));

        for (ULONG index = 0; index < count; ++index) {
            if (cursor > limit ||
                (SIZE_T)(limit - cursor) < sizeof(HV_AAD_OBJECT_TYPE_INFORMATION)) {
                break;
            }

            PHV_AAD_OBJECT_TYPE_INFORMATION row =
                (PHV_AAD_OBJECT_TYPE_INFORMATION)cursor;
            PUCHAR stringEnd = (PUCHAR)(row + 1) + row->TypeName.MaximumLength;
            PUCHAR next = HvHookpAlignUp8(stringEnd);
            if (next <= cursor || next > limit) {
                break;
            }

            if (row->TypeName.Buffer &&
                row->TypeName.Length == debugObjectBytes &&
                (PUCHAR)row->TypeName.Buffer >= base &&
                (PUCHAR)row->TypeName.Buffer + row->TypeName.Length <= limit &&
                RtlCompareMemory(
                    row->TypeName.Buffer,
                    debugObjectName,
                    debugObjectBytes) == debugObjectBytes) {
                row->TotalNumberOfHandles = 0;
                row->TotalNumberOfObjects = 0;
                row->HighWaterNumberOfHandles = 0;
                row->HighWaterNumberOfObjects = 0;
                for (USHORT i = 0;
                     i < row->TypeName.Length / sizeof(WCHAR);
                     ++i) {
                    row->TypeName.Buffer[i] = L'_';
                }
                HV_HOT_DBG("[HvHook-AAD] DebugObject type row scrubbed\n");
                break;
            }
            cursor = next;
        }
...
static NTSTATUS NTAPI HookedNtQueryInformationThread(
    HANDLE ThreadHandle,
    ULONG ThreadInformationClass,
    PVOID ThreadInformation,
    ULONG ThreadInformationLength,
...
{
    HV_HOOK_CALLBACK_BEGIN(
        g_HookNtQueryInformationThread, STATUS_DELETE_PENDING)
    PFN_NtQueryInformationThread Original =
        (PFN_NtQueryInformationThread)HvHookGetTrampoline(
            g_HookNtQueryInformationThread);
    if (!Original) {
        return STATUS_UNSUCCESSFUL;
    }
    ...
    if (ThreadInformationClass != ThreadHideFromDebugger ||
        !ThreadInformation || ThreadInformationLength < sizeof(BOOLEAN) ||
        !g_AntiAntiDebugConfig.HookNtQueryInformationThread ||
        !HvHookpIsAntiAntiDebugCaller()) {
        return status;
    }
    ...
    __try {
        if (ExGetPreviousMode() == UserMode) {
            ProbeForWrite(ThreadInformation, sizeof(BOOLEAN), sizeof(UCHAR));
            if (ReturnLength) {
...
static NTSTATUS NTAPI HookedNtSystemDebugControl(
    ULONG Command,
    PVOID InputBuffer,
    ULONG InputBufferLength,
    PVOID OutputBuffer,
...
{
    HV_HOOK_CALLBACK_BEGIN(
        g_HookNtSystemDebugControl, STATUS_DELETE_PENDING)
    PFN_NtSystemDebugControl Original =
        (PFN_NtSystemDebugControl)HvHookGetTrampoline(
            g_HookNtSystemDebugControl);
    if (!Original) {
        return STATUS_UNSUCCESSFUL;
    }

    if (g_AntiAntiDebugConfig.HookNtSystemDebugControl &&
        HvHookpIsAntiAntiDebugCaller() &&
        Command != 29 && Command != 37) {
...
static NTSTATUS
HvHookpInstallTrackedHookLocked(
    _In_ PCWSTR Name,
    _In_opt_ ULONG CallerPid,
    _In_ PVOID HookFunction,
    _Inout_ HV_HOOK_HANDLE* Handle
)
{
    HV_HOOK_HANDLE candidate = NULL;
    PVOID target;
    NTSTATUS status;
    ...
    target = HvHookpGetNtRoutine(Name, CallerPid);
    if (!target) {
        return STATUS_PROCEDURE_NOT_FOUND;
    }

    status = HvHookInstall(target, HookFunction, &candidate);
    if (status == STATUS_ALREADY_REGISTERED) {
        HV_HOOK_INFO info;

        candidate = HvHookFindByVirtualAddress(target);
        if (candidate &&
            NT_SUCCESS(HvHookGetInfo(candidate, &info)) &&
            info.TargetVirtualAddress == target &&
            info.HookFunction == HookFunction) {
            status = STATUS_SUCCESS;
            DbgPrint("[HvHook] %ws adopted existing compatible hook %p\n",
                     Name, candidate);
        } else {
            candidate = NULL;
        }
    ...
    if (!candidate) {
        DbgPrint("[HvHook] HvHookInstall(%ws) returned no handle\n", Name);
        return STATUS_DEVICE_NOT_READY;
    }

    *Handle = candidate;
    DbgPrint("[HvHook] %ws hooked at %p, handle=%p\n",
             Name, target, candidate);
    return STATUS_SUCCESS;
}

static NTSTATUS
HvHookpRemoveTrackedHookLocked(
    _In_ PCWSTR Name,
    _Inout_ HV_HOOK_HANDLE* Handle
)
{
    ...
    if (NT_SUCCESS(status)) {
        *Handle = NULL;
        ...
        DbgPrint("[HvHook] HvHookRemove(%ws, %p) failed: 0x%X\n",
                 Name, *Handle, status);
    }
    return status;
}
...
static NTSTATUS
HvHookpAcquireSharedHookLocked(
    _In_ PCWSTR Name,
    _In_opt_ ULONG CallerPid,
    _In_ PVOID HookFunction,
    _Inout_ HV_HOOK_HANDLE* Handle,
    _Inout_ PULONG Owners,
    _In_ ULONG Owner
)
{
    NTSTATUS status;

    if (((*Owners & Owner) != 0) && *Handle) {
        return STATUS_SUCCESS;
    }

    *Owners &= ~Owner;
    status = HvHookpInstallTrackedHookLocked(
        Name, CallerPid, HookFunction, Handle);
    if (NT_SUCCESS(status)) {
        *Owners |= Owner;
    }
    return status;
...
static NTSTATUS
HvHookpReleaseSharedHookLocked(
    _In_ PCWSTR Name,
    _Inout_ HV_HOOK_HANDLE* Handle,
    _Inout_ PULONG Owners,
    _In_ ULONG Owner
)
{
    ...
    remainingOwners = *Owners & ~Owner;
    if (remainingOwners != 0) {
        *Owners = remainingOwners;
        return STATUS_SUCCESS;
    }

    status = HvHookpRemoveTrackedHookLocked(Name, Handle);
    if (NT_SUCCESS(status)) {
        *Owners = 0;
    }
    return status;
...
static NTSTATUS
HvHookpEnableDriverHideLocked(VOID)
{
    NTSTATUS status;
    NTSTATUS backendStatus;

    if ((InterlockedCompareExchange(&g_DriverHideEnabled, FALSE, FALSE) != 0) &&
        g_HookNtQuerySystemInformation &&
        (g_HookOwnersNtQuerySystemInformation &
         HV_HOOK_OWNER_DRIVER_HIDE) != 0) {
        return STATUS_SUCCESS;
    }

    InterlockedExchange(&g_DriverHideEnabled, FALSE);
    status = HvHookpAcquireSharedHookLocked(
        L"NtQuerySystemInformation",
        0,
        HookedNtQuerySystemInformation,
        &g_HookNtQuerySystemInformation,
        &g_HookOwnersNtQuerySystemInformation,
        HV_HOOK_OWNER_DRIVER_HIDE);
    if (!NT_SUCCESS(status)) {
        return status;
    }

    if (g_HvHookBackend == CPU_VENDOR_INTEL) {
        backendStatus = EptHookInstallDriverHideHook();
    } else if (g_HvHookBackend == CPU_VENDOR_AMD) {
        backendStatus = NptHookInstallDriverHideHook();
    } else {
        backendStatus = STATUS_NOT_SUPPORTED;
    }

    if (!NT_SUCCESS(backendStatus)) {
        (void)HvHookpReleaseSharedHookLocked(
            L"NtQuerySystemInformation",
            &g_HookNtQuerySystemInformation,
            &g_HookOwnersNtQuerySystemInformation,
            HV_HOOK_OWNER_DRIVER_HIDE);
    ...
    MemoryBarrier();
    InterlockedExchange(&g_DriverHideEnabled, TRUE);
    return STATUS_SUCCESS;
}

static NTSTATUS
HvHookpDisableDriverHideLocked(VOID)
{
    NTSTATUS backendStatus = STATUS_SUCCESS;
    NTSTATUS ownerStatus;

    /* Stop new filtering before dismantling either backend state. */
    InterlockedExchange(&g_DriverHideEnabled, FALSE);
    MemoryBarrier();

    if (g_HvHookBackend == CPU_VENDOR_INTEL) {
        backendStatus = EptHookRemoveDriverHideHook();
    } else if (g_HvHookBackend == CPU_VENDOR_AMD) {
        backendStatus = NptHookRemoveDriverHideHook();
    }

    ownerStatus = HvHookpReleaseSharedHookLocked(
        L"NtQuerySystemInformation",
        &g_HookNtQuerySystemInformation,
        &g_HookOwnersNtQuerySystemInformation,
        HV_HOOK_OWNER_DRIVER_HIDE);

    return !NT_SUCCESS(backendStatus) ? backendStatus : ownerStatus;
}

static NTSTATUS
HvHookpEnableAntiAntiDebugLocked(
    _In_opt_ PHV_ANTIANTIDEBUG_CONFIG Config
);

static NTSTATUS
HvHookpDisableAntiAntiDebugLocked(VOID);
...
)
{
    ...
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    status = HvHookpEnableAntiAntiDebugLocked(Config);
    ExReleasePushLockExclusive(&g_DebuggerMutationLock);
    KeLeaveCriticalRegion();
    return status;
}
...
)
{
    ...
    PVOID targetFunc;
    ULONG resolverPid = (ULONG)(ULONG_PTR)PsGetCurrentProcessId();
    ...
    if (Config) {
        ...
        g_AntiAntiDebugConfig.HookNtGetContextThread = TRUE;
        g_AntiAntiDebugConfig.HookNtSetContextThread = TRUE;
        g_AntiAntiDebugConfig.HookNtQueryInformationThread = TRUE;
        g_AntiAntiDebugConfig.HookNtSystemDebugControl = TRUE;
    }
    ...
    if (g_AntiAntiDebugConfig.HookNtQueryInformationProcess) {
        status = HvHookpAcquireSharedHookLocked(
            L"NtQueryInformationProcess",
            resolverPid,
            HookedNtQueryInformationProcess,
            &g_HookNtQueryInformationProcess,
            &g_HookOwnersNtQueryInformationProcess,
            HV_HOOK_OWNER_ANTIANTIDEBUG);
        if (!NT_SUCCESS(status)) {
            DbgPrint("[HvHook] AAD NtQueryInformationProcess unavailable: 0x%X\n",
                     status);
        }
    }
    ...
    if (g_AntiAntiDebugConfig.HookNtQuerySystemInformation) {
        status = HvHookpAcquireSharedHookLocked(
            ...
            HV_HOOK_OWNER_ANTIANTIDEBUG);
        if (!NT_SUCCESS(status)) {
            DbgPrint("[HvHook] AAD NtQuerySystemInformation unavailable: 0x%X\n",
                     status);
        }
    }
    ...
    // Hook NtClose
    if (g_AntiAntiDebugConfig.HookNtClose && !g_HookNtClose) {
    ...
    if (g_AntiAntiDebugConfig.HookNtClose &&
        !g_HookNtDuplicateObject) {
        targetFunc = HvHookpGetNtRoutine(L"NtDuplicateObject", resolverPid);
        if (targetFunc) {
    ...
    if (g_AntiAntiDebugConfig.HookNtQueryInformationThread &&
        !g_HookNtQueryInformationThread) {
        targetFunc = HvHookpGetNtRoutine(
            L"NtQueryInformationThread", resolverPid);
        if (targetFunc) {
            status = HvHookInstall(
                targetFunc,
                ...
                &g_HookNtQueryInformationThread);
            if (!NT_SUCCESS(status)) {
                g_HookNtQueryInformationThread = NULL;
            }
        }
    }
    ...
    if (g_AntiAntiDebugConfig.HookNtSetInformationThread) {
        status = HvHookpAcquireSharedHookLocked(
            ...
            &g_HookNtCreateThreadEx,
            &g_HookOwnersNtCreateThreadEx,
            HV_HOOK_OWNER_ANTIANTIDEBUG);
        if (!NT_SUCCESS(status)) {
            DbgPrint("[HvHook] AAD NtCreateThreadEx unavailable: 0x%X\n",
                     status);
        }
    }
    ...
    if (g_AntiAntiDebugConfig.HookNtSystemDebugControl &&
        !g_HookNtSystemDebugControl) {
        targetFunc = HvHookpGetNtRoutine(
            L"NtSystemDebugControl", resolverPid);
        if (targetFunc) {
            status = HvHookInstall(
                targetFunc,
                HookedNtSystemDebugControl,
                &g_HookNtSystemDebugControl);
            if (!NT_SUCCESS(status)) {
                g_HookNtSystemDebugControl = NULL;
            }
        }
    }

    if (g_AntiAntiDebugConfig.HookNtGetContextThread) {
        status = HvHookpAcquireSharedHookLocked(
            L"NtGetContextThread",
            resolverPid,
            HookedNtGetContextThread,
            &g_HookNtGetContextThread,
            &g_HookOwnersNtGetContextThread,
            HV_HOOK_OWNER_ANTIANTIDEBUG);
        if (!NT_SUCCESS(status)) {
            DbgPrint("[HvHook] AAD NtGetContextThread unavailable: 0x%X\n",
                     status);
        }
    }

    if (g_AntiAntiDebugConfig.HookNtSetContextThread) {
        status = HvHookpAcquireSharedHookLocked(
            L"NtSetContextThread",
            resolverPid,
            HookedNtSetContextThread,
            &g_HookNtSetContextThread,
            &g_HookOwnersNtSetContextThread,
            HV_HOOK_OWNER_ANTIANTIDEBUG);
        if (!NT_SUCCESS(status)) {
            DbgPrint("[HvHook] AAD NtSetContextThread unavailable: 0x%X\n",
                     status);
        }
    }
    ...
    {
        ULONG configured = HvHookGetAntiAntiDebugConfiguredMask();
        ULONG installed = HvHookGetAntiAntiDebugInstalledMask();
        DbgPrint("[HvHook] Anti-anti-debug enable result: configured=0x%X installed=0x%X\n",
                 configured, installed);
        if ((installed & configured) != configured) {
            NTSTATUS cleanupStatus = HvHookpDisableAntiAntiDebugLocked();
            return NT_SUCCESS(cleanupStatus)
...
HvHookDisableAntiAntiDebug(VOID)
{
    ...
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    status = HvHookpDisableAntiAntiDebugLocked();
    ExReleasePushLockExclusive(&g_DebuggerMutationLock);
    KeLeaveCriticalRegion();
    return status;
}
...
HvHookpDisableAntiAntiDebugLocked(VOID)
{
    ...
    // Shared hooks release only AAD's explicit owner bit. No configuration
    // field is used to infer whether another feature still owns the handle.
    status = HvHookpReleaseSharedHookLocked(
        L"NtQueryInformationProcess",
        &g_HookNtQueryInformationProcess,
        &g_HookOwnersNtQueryInformationProcess,
        HV_HOOK_OWNER_ANTIANTIDEBUG);
    if (!NT_SUCCESS(status)) finalStatus = status;
    status = HvHookpReleaseSharedHookLocked(
        L"NtQuerySystemInformation",
        &g_HookNtQuerySystemInformation,
        &g_HookOwnersNtQuerySystemInformation,
        HV_HOOK_OWNER_ANTIANTIDEBUG);
    if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;
    status = HvHookpRemoveTrackedHookLocked(
        L"NtSetInformationThread", &g_HookNtSetInformationThread);
    if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;
    status = HvHookpRemoveTrackedHookLocked(
        L"NtClose", &g_HookNtClose);
    if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;
    if (g_HookNtDuplicateObject && !g_AccessBypassEnabled) {
        status = HvHookpRemoveTrackedHookLocked(
            L"NtDuplicateObject", &g_HookNtDuplicateObject);
        if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;
    }
    status = HvHookpReleaseSharedHookLocked(
        L"NtQueryObject",
    ...
    if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;
    status = HvHookpRemoveTrackedHookLocked(
        L"NtQueryInformationThread", &g_HookNtQueryInformationThread);
    if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;
    status = HvHookpReleaseSharedHookLocked(
        L"NtCreateThreadEx",
        &g_HookNtCreateThreadEx,
    ...
    if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;
    status = HvHookpRemoveTrackedHookLocked(
        L"NtSystemDebugControl", &g_HookNtSystemDebugControl);
    if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;
    status = HvHookpReleaseSharedHookLocked(
        L"NtGetContextThread",
        &g_HookNtGetContextThread,
    ...
    if (!NT_SUCCESS(finalStatus)) {
        DbgPrint("[HvHook] Anti-anti-debug disable retained hook ownership: 0x%X\n",
                 finalStatus);
        return finalStatus;
    }
...
{
    return g_AntiAntiDebugEnabled ? g_AntiAntiDebugConfig.TargetPid : 0;
}

ULONG HvHookGetAntiAntiDebugConfiguredMask(VOID)
{
    ULONG mask = 0;
    if (!g_AntiAntiDebugEnabled) return 0;
    if (g_AntiAntiDebugConfig.HookNtQueryInformationProcess)
        mask |= HV_AAD_FEATURE_PROCESS_DEBUG_QUERY;
    if (g_AntiAntiDebugConfig.HookNtQuerySystemInformation)
        mask |= HV_AAD_FEATURE_KERNEL_DEBUG_QUERY;
    if (g_AntiAntiDebugConfig.HookNtSetInformationThread ||
        g_AntiAntiDebugConfig.HookNtQueryInformationThread)
        mask |= HV_AAD_FEATURE_THREAD_HIDE;
    if (g_AntiAntiDebugConfig.HookNtClose)
        mask |= HV_AAD_FEATURE_INVALID_HANDLE;
    if (g_AntiAntiDebugConfig.HookNtQueryObject)
        mask |= HV_AAD_FEATURE_DEBUG_OBJECT;
    if (g_AntiAntiDebugConfig.HookNtGetContextThread ||
        g_AntiAntiDebugConfig.HookNtSetContextThread)
        mask |= HV_AAD_FEATURE_DEBUG_REGISTERS;
    if (g_AntiAntiDebugConfig.HookNtSystemDebugControl)
        mask |= HV_AAD_FEATURE_SYSTEM_DEBUG_CONTROL;
    return mask;
}
...
ULONG HvHookGetAntiAntiDebugInstalledMask(VOID)
{
    ULONG configured = HvHookGetAntiAntiDebugConfiguredMask();
    ULONG installed = 0;
    if ((configured & HV_AAD_FEATURE_PROCESS_DEBUG_QUERY) &&
        g_HookNtQueryInformationProcess)
        installed |= HV_AAD_FEATURE_PROCESS_DEBUG_QUERY;
    if ((configured & HV_AAD_FEATURE_KERNEL_DEBUG_QUERY) &&
        g_HookNtQuerySystemInformation)
        installed |= HV_AAD_FEATURE_KERNEL_DEBUG_QUERY;
    if ((configured & HV_AAD_FEATURE_THREAD_HIDE) &&
        g_HookNtSetInformationThread && g_HookNtQueryInformationThread &&
        g_HookNtCreateThreadEx)
        installed |= HV_AAD_FEATURE_THREAD_HIDE;
    if ((configured & HV_AAD_FEATURE_INVALID_HANDLE) &&
        g_HookNtClose && g_HookNtDuplicateObject)
        installed |= HV_AAD_FEATURE_INVALID_HANDLE;
    if ((configured & HV_AAD_FEATURE_DEBUG_OBJECT) && g_HookNtQueryObject)
        installed |= HV_AAD_FEATURE_DEBUG_OBJECT;
    if ((configured & HV_AAD_FEATURE_DEBUG_REGISTERS) &&
        g_HookNtGetContextThread && g_HookNtSetContextThread)
        installed |= HV_AAD_FEATURE_DEBUG_REGISTERS;
    if ((configured & HV_AAD_FEATURE_SYSTEM_DEBUG_CONTROL) &&
        g_HookNtSystemDebugControl)
        installed |= HV_AAD_FEATURE_SYSTEM_DEBUG_CONTROL;
    return installed;
}

BOOLEAN HvHookIsBridgeAntiDebugEnabled(VOID)
{
    return g_BridgeAntiDebugEnabled;
}
...
static NTSTATUS
HvHookpAddDebuggerLocked(
    _In_ PHV_DEBUGGER_CONFIG Config
);

static NTSTATUS
HvHookpRemoveDebuggerLocked(
    _In_ ULONG ProcessId
);

static NTSTATUS
HvHookpEnableDebuggerProxyLocked(
    _In_ ULONG CallerPid
);

static VOID
HvHookpDisableDebuggerProxyLocked(VOID);

static VOID
HvHookpDisableBridgeAntiDebugLocked(VOID);

static NTSTATUS
HvHookpEnableBridgeAntiDebugLocked(_In_ ULONG CallerPid);

static NTSTATUS
HvHookpEnableAccessBypassLocked(VOID);

static NTSTATUS
HvHookpDisableAccessBypassLocked(VOID);

static NTSTATUS
HvHookpProtectProcessLocked(
    _In_ PHV_PROTECT_CONFIG Config,
    _In_ PEPROCESS TargetProcess,
    _In_ BOOLEAN BridgeOwned,
    _In_ ULONG BridgeBindFlags
);

static NTSTATUS
HvHookpUnbindBridgeTargetLocked(
    _In_ ULONG ProcessId,
    _In_ ULONG DebuggerPid
);

static BOOLEAN
HvHookpHasBridgeBindings(VOID);

static ULONG
HvHookpUnprotectAllForDebuggerLocked(
    _In_ ULONG DebuggerPid
);

static NTSTATUS
HvHookpEnsureDebuggerCoreLocked(
    _In_ ULONG CallerPid
)
{
    NTSTATUS status;

    status = HvHookpEnableDebuggerProxyLocked(CallerPid);
    if (NT_SUCCESS(status) && !HvHookIsDebuggerProxyEnabled()) {
    ...
    status = HvHookpEnableAccessBypassLocked();
    if (NT_SUCCESS(status) && !HvHookIsAccessBypassEnabled()) {
        status = STATUS_DEVICE_NOT_READY;
    }
    return status;
}
...
)
{
    ...
    if (!g_DebuggerMutationLockInitialized || KeGetCurrentIrql() > APC_LEVEL) {
        return STATUS_INVALID_DEVICE_STATE;
    }

    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    status = HvHookpAddDebuggerLocked(Config);
    ExReleasePushLockExclusive(&g_DebuggerMutationLock);
    KeLeaveCriticalRegion();
    return status;
}
...
)
{
    ...
    PPROTECTED_DEBUGGER existing;
    PEPROCESS liveProcess = NULL;
    UINT64 liveCreateTime = 0;
    PLIST_ENTRY entry;
    KIRQL oldIrql;
    BOOLEAN hadCoreDebugger = FALSE;
    BOOLEAN newProtectionOwnerAcquired = FALSE;
    ...
    // Bridge registration and the GUI fallback can race. Merge by PID.
    KeAcquireSpinLock(&g_ProtectedDebuggerLock, &oldIrql);
    for (entry = g_ProtectedDebuggerList.Flink;
         entry != &g_ProtectedDebuggerList;
         entry = entry->Flink)
    {
        existing = CONTAINING_RECORD(entry, PROTECTED_DEBUGGER, ListEntry);
        if (!existing->IdentityOnly) {
            hadCoreDebugger = TRUE;
        }
        if (existing->ProcessId == Config->ProcessId) {
            BOOLEAN wasHidden = existing->HideFromList;
            BOOLEAN wasIdentityOnly = existing->IdentityOnly;
            BOOLEAN wasProtected = existing->ProtectFromTerminate;
            BOOLEAN acquiredProtectionOwner = FALSE;
            NTSTATUS updateStatus;

            for (PLIST_ENTRY coreEntry = g_ProtectedDebuggerList.Flink;
                 coreEntry != &g_ProtectedDebuggerList;
                 coreEntry = coreEntry->Flink)
            {
                PPROTECTED_DEBUGGER coreDebugger =
                    CONTAINING_RECORD(coreEntry, PROTECTED_DEBUGGER, ListEntry);
                if (!coreDebugger->IdentityOnly) {
                    hadCoreDebugger = TRUE;
                    break;
                }
            }
            ...
            updateStatus = PsLookupProcessByProcessId(
                (HANDLE)(ULONG_PTR)Config->ProcessId, &liveProcess);
            if (!NT_SUCCESS(updateStatus)) {
                return updateStatus;
            }
            liveCreateTime =
                (UINT64)PsGetProcessCreateTimeQuadPart(liveProcess);
            if (PsGetProcessExitStatus(liveProcess) != STATUS_PENDING ||
                liveCreateTime == 0 ||
                existing->ProcessObject != liveProcess ||
                existing->ProcessCreateTime != liveCreateTime) {
                ObDereferenceObject(liveProcess);
                return STATUS_INVALID_CID;
            }

            if (Config->ProtectFromTerminate && !wasProtected) {
                updateStatus =
                    HvHookpAcquireDebuggerProtectOwnerLocked();
                if (!NT_SUCCESS(updateStatus)) {
                    ObDereferenceObject(liveProcess);
                    return updateStatus;
                }
                acquiredProtectionOwner = TRUE;
            }

            updateStatus = STATUS_SUCCESS;
            if (!Config->BridgeIdentityOnly) {
                updateStatus = HvHookpEnsureDebuggerCoreLocked(Config->ProcessId);
                if (!NT_SUCCESS(updateStatus)) {
                    if (!hadCoreDebugger) {
                        (void)HvHookpDisableAccessBypassLocked();
                        HvHookpDisableDebuggerProxyLocked();
                    }
                    if (acquiredProtectionOwner) {
                        HvHookpReleaseDebuggerProtectOwnerLocked();
                    }
                    ObDereferenceObject(liveProcess);
                    return updateStatus;
                }
            ...
            if (Config->HideFromList) {
                updateStatus = HvHookHideProcess(Config->ProcessId);
            } else if (wasHidden) {
                updateStatus = HvHookUnhideProcess(Config->ProcessId);
            }
            if (!NT_SUCCESS(updateStatus)) {
                if (!Config->BridgeIdentityOnly &&
                    wasIdentityOnly &&
            ...
            /* Publish the explicit configuration only after its hide-state
             * transition succeeded; failure leaves the old record intact. */
            KeAcquireSpinLock(&g_ProtectedDebuggerLock, &oldIrql);
            existing->EnablePrivilege = Config->EnablePrivilege;
            existing->ProtectFromTerminate = Config->ProtectFromTerminate;
            existing->HideFromList = Config->HideFromList;
            existing->IdentityOnly =
                wasIdentityOnly && Config->BridgeIdentityOnly;
            if (Config->ProcessName[0] != L'\0') {
                RtlStringCbCopyNW(existing->ProcessName,
                                  sizeof(existing->ProcessName),
                                  Config->ProcessName,
                                  sizeof(Config->ProcessName));
            }
            KeReleaseSpinLock(&g_ProtectedDebuggerLock, oldIrql);
            if (wasProtected && !Config->ProtectFromTerminate) {
                HvHookpReleaseDebuggerProtectOwnerLocked();
            }
            ObDereferenceObject(liveProcess);
            return STATUS_SUCCESS;
    ...
    // Do not retain a PID that has already exited.
    {
        NTSTATUS liveStatus = PsLookupProcessByProcessId(
            (HANDLE)(ULONG_PTR)Config->ProcessId, &liveProcess);
        if (!NT_SUCCESS(liveStatus)) {
            return liveStatus;
        }
        liveCreateTime =
            (UINT64)PsGetProcessCreateTimeQuadPart(liveProcess);
    ...
    debugger->ProcessObject = liveProcess;
    debugger->ProcessCreateTime = liveCreateTime;
    if (Config->ProcessName[0] != L'\0') {
    ...
    debugger->IdentityOnly = Config->BridgeIdentityOnly;
    ...
    {
        PEPROCESS proc = liveProcess;
        NTSTATUS lkSt = proc ? STATUS_SUCCESS : STATUS_INVALID_CID;
    ...
    for (entry = g_ProtectedDebuggerList.Flink;
         ...
         entry = entry->Flink)
    {
        PPROTECTED_DEBUGGER candidate =
            CONTAINING_RECORD(entry, PROTECTED_DEBUGGER, ListEntry);
        if (!candidate->IdentityOnly) {
            hadCoreDebugger = TRUE;
        }
        if (candidate->ProcessId == Config->ProcessId) {
            existing = candidate;
            break;
        }
    }
    if (existing) {
        ...
        if (existing->ProcessObject != debugger->ProcessObject ||
            existing->ProcessCreateTime != debugger->ProcessCreateTime) {
            ObDereferenceObject(debugger->ProcessObject);
            ExFreePoolWithTag(debugger, HV_HOOK_TAG);
            return STATUS_INVALID_CID;
        }
        ...
        if (Config->ProtectFromTerminate && !wasProtected) {
            updateStatus = HvHookpAcquireDebuggerProtectOwnerLocked();
            if (!NT_SUCCESS(updateStatus)) {
                ObDereferenceObject(debugger->ProcessObject);
                ExFreePoolWithTag(debugger, HV_HOOK_TAG);
                return updateStatus;
    ...
    KeReleaseSpinLock(&g_ProtectedDebuggerLock, oldIrql);
    if (Config->ProtectFromTerminate) {
        ...
        newProtectionOwnerAcquired = TRUE;
    }
    KeAcquireSpinLock(&g_ProtectedDebuggerLock, &oldIrql);
    ...
    // EPT-violation 风暴。
    {
        ...
        if (!Config->BridgeIdentityOnly) {
            coreStatus = HvHookpEnsureDebuggerCoreLocked(Config->ProcessId);
        }
        if (!NT_SUCCESS(coreStatus)) {
            KeAcquireSpinLock(&g_ProtectedDebuggerLock, &oldIrql);
            RemoveEntryList(&debugger->ListEntry);
            KeReleaseSpinLock(&g_ProtectedDebuggerLock, oldIrql);
            if (!hadCoreDebugger) {
                (void)HvHookpDisableAccessBypassLocked();
                HvHookpDisableDebuggerProxyLocked();
            }
            if (newProtectionOwnerAcquired) {
                HvHookpReleaseDebuggerProtectOwnerLocked();
            }
            ObDereferenceObject(debugger->ProcessObject);
            ExFreePoolWithTag(debugger, HV_HOOK_TAG);
            HvDbgEvtPost(HV_DBGEVT_SEV_ERROR, HV_DBGEVT_CAT_ADD_DEBUGGER,
                         coreStatus, Config->ProcessId, 0,
                         (UINT64)hadCoreDebugger, 0,
                         "Debugger registration rolled back: core hooks unavailable");
            return coreStatus;
        }

        HvDbgEvtPost(HV_DBGEVT_SEV_INFO, HV_DBGEVT_CAT_ADD_DEBUGGER,
                     STATUS_SUCCESS, Config->ProcessId, 0,
                     (UINT64)hadCoreDebugger,
                     (UINT64)Config->BridgeIdentityOnly,
                     Config->BridgeIdentityOnly
                         ? "Bridge debugger identity published; generic syscall core hooks unchanged"
                         : (hadCoreDebugger
                             ? "Additional debugger registered; core hooks verified"
                             : "DebuggerProxy + AccessBypass enabled (0->1)"));
    }

    // 如果需要隐藏调试器进程
    if (Config->HideFromList) {
        NTSTATUS hideStatus = HvHookHideProcess(Config->ProcessId);
        if (!NT_SUCCESS(hideStatus)) {
            KeAcquireSpinLock(&g_ProtectedDebuggerLock, &oldIrql);
            RemoveEntryList(&debugger->ListEntry);
            KeReleaseSpinLock(&g_ProtectedDebuggerLock, oldIrql);
            if (!Config->BridgeIdentityOnly && !hadCoreDebugger) {
                (void)HvHookpDisableAccessBypassLocked();
                HvHookpDisableDebuggerProxyLocked();
            }
            if (newProtectionOwnerAcquired) {
            ...
            ExFreePoolWithTag(debugger, HV_HOOK_TAG);
            HvDbgEvtPost(HV_DBGEVT_SEV_ERROR, HV_DBGEVT_CAT_ADD_DEBUGGER,
                         hideStatus, Config->ProcessId, 0,
                         (UINT64)wasEmpty, 0,
                         "Debugger registration rolled back: hide state unavailable");
            return hideStatus;
        }
    }
...
)
{
    ...
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    status = HvHookpRemoveDebuggerLocked(ProcessId);
    ExReleasePushLockExclusive(&g_DebuggerMutationLock);
    KeLeaveCriticalRegion();
    return status;
}
...
)
{
    ...
    KIRQL oldIrql;
    BOOLEAN found = FALSE;
    BOOLEAN removedRequiredCore = FALSE;
    BOOLEAN removedProtectionOwner = FALSE;
    BOOLEAN hasCoreDebugger = FALSE;
         ...
         entry = entry->Flink)
    {
        ...
        if (debugger->ProcessId == ProcessId) {
            removedRequiredCore = !debugger->IdentityOnly;
            removedProtectionOwner = debugger->ProtectFromTerminate;
            RemoveEntryList(entry);
            found = TRUE;
            break;
        }
    ...
    if (!nowEmpty) {
        for (entry = g_ProtectedDebuggerList.Flink;
             ...
             entry = entry->Flink)
        {
            PPROTECTED_DEBUGGER remaining =
                CONTAINING_RECORD(entry, PROTECTED_DEBUGGER, ListEntry);
            if (!remaining->IdentityOnly) {
                hasCoreDebugger = TRUE;
                break;
            }
        }
    }
    ...
    if (found && debugger) {
        ...
        // 如果之前隐藏了，取消隐藏
        // A debugger owns its target bindings. Drop them before PID reuse.
        (void)HvHookpUnprotectAllForDebuggerLocked(ProcessId);
        (void)HvDbgClearAllForDebugger((HANDLE)(ULONG_PTR)ProcessId);
        HvDbgClearAllSwBpForDebugger((HANDLE)(ULONG_PTR)ProcessId);
        HvDbgClearAllStepForDebugger((HANDLE)(ULONG_PTR)ProcessId);
        HvVwatchClearAllForDebugger((HANDLE)(ULONG_PTR)ProcessId);

        if (debugger->HideFromList) {
            HvHookUnhideProcess(ProcessId);
        }
        if (removedProtectionOwner) {
            HvHookpReleaseDebuggerProtectOwnerLocked();
        }
        if (debugger->ProcessObject) {
            ObDereferenceObject(debugger->ProcessObject);
            debugger->ProcessObject = NULL;
        }
        ExFreePoolWithTag(debugger, HV_HOOK_TAG);

        if (nowEmpty) {
            HvHookpDisableBridgeAntiDebugLocked();
        }
        if (removedRequiredCore && !hasCoreDebugger) {
            (void)HvHookpDisableAccessBypassLocked();
            HvHookpDisableDebuggerProxyLocked();
            HvDbgEvtPost(HV_DBGEVT_SEV_INFO, HV_DBGEVT_CAT_REMOVE_DEBUGGER,
                         STATUS_SUCCESS, ProcessId, 0, 0, 0,
                         "Last core-hook debugger removed; DebuggerProxy + AccessBypass disabled");
        }
...
static NTSTATUS
HvHookpProtectProcessLegacyLocked(
    _In_ PHV_PROTECT_CONFIG Config
)
{
    PPROTECTED_PROCESS proc;
    PPROTECTED_PROCESS existing;
    PLIST_ENTRY entry;
    KIRQL oldIrql;
    ...
    // A target PID has one owner record. Rebinding updates that record.
    KeAcquireSpinLock(&g_ProtectedProcessLock, &oldIrql);
    for (entry = g_ProtectedProcessList.Flink;
         entry != &g_ProtectedProcessList;
         entry = entry->Flink)
    {
        existing = CONTAINING_RECORD(entry, PROTECTED_PROCESS, ListEntry);
        if (existing->ProcessId == Config->ProcessId) {
            if (existing->DebuggerPid != Config->DebuggerPid) {
                KeReleaseSpinLock(&g_ProtectedProcessLock, oldIrql);
                return STATUS_SHARING_VIOLATION;
            }
            existing->DebuggerPid = Config->DebuggerPid;
            existing->PreventTerminate = Config->PreventTerminate;
            existing->PreventSuspend = Config->PreventSuspend;
            existing->PreventMemoryAccess = Config->PreventMemoryAccess;
            if (Config->ProcessName[0] != L'\0') {
                RtlStringCbCopyNW(existing->ProcessName,
                                  sizeof(existing->ProcessName),
                                  Config->ProcessName,
    ...
    for (entry = g_ProtectedProcessList.Flink;
         ...
         entry = entry->Flink)
    {
        PPROTECTED_PROCESS candidate =
            CONTAINING_RECORD(entry, PROTECTED_PROCESS, ListEntry);
        if (candidate->ProcessId == Config->ProcessId) {
            existing = candidate;
            break;
        }
...
)
{
    ...
    if (!Config || Config->ProcessId == 0) return STATUS_INVALID_PARAMETER;
    if (!g_DebuggerMutationLockInitialized || KeGetCurrentIrql() > APC_LEVEL) {
        return STATUS_INVALID_DEVICE_STATE;
    }
    status = PsLookupProcessByProcessId(
        (HANDLE)(ULONG_PTR)Config->ProcessId, &targetProcess);
    if (!NT_SUCCESS(status)) return status;

    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    status = HvHookpProtectProcessLocked(Config, targetProcess, FALSE, 0);
    ExReleasePushLockExclusive(&g_DebuggerMutationLock);
    KeLeaveCriticalRegion();
...
{
    PPROTECTED_PROCESS record;
    PPROTECTED_PROCESS existing = NULL;
    PLIST_ENTRY entry;
    KIRQL oldIrql;
    UINT64 createTime;
    NTSTATUS status = STATUS_SUCCESS;
    ...
    if (!Config || !TargetProcess || Config->ProcessId == 0) {
        return STATUS_INVALID_PARAMETER;
    }
    createTime = (UINT64)PsGetProcessCreateTimeQuadPart(TargetProcess);
    if (createTime == 0 || PsGetProcessExitStatus(TargetProcess) != STATUS_PENDING) {
        return STATUS_PROCESS_IS_TERMINATING;
    }

    record = (PPROTECTED_PROCESS)HvAllocateNonPagedZeroed(
        sizeof(PROTECTED_PROCESS), HV_HOOK_TAG);
    if (!record) return STATUS_INSUFFICIENT_RESOURCES;

    record->ProcessId = Config->ProcessId;
    record->ProcessObject = TargetProcess;
    record->ProcessCreateTime = createTime;
    ObReferenceObject(TargetProcess);
    record->DebuggerPid = Config->DebuggerPid;
    record->BridgeBindFlags = BridgeOwned ? BridgeBindFlags : 0;
    record->BridgeOwned = BridgeOwned;
    record->PreventTerminate = Config->PreventTerminate;
    record->PreventSuspend = Config->PreventSuspend;
    record->PreventMemoryAccess = Config->PreventMemoryAccess;
    if (Config->ProcessName[0] != L'\0') {
        RtlStringCbCopyNW(record->ProcessName, sizeof(record->ProcessName),
                          Config->ProcessName, sizeof(Config->ProcessName));
    }
    ...
    if (PsGetProcessExitStatus(TargetProcess) != STATUS_PENDING) {
        ...
        if (existing) {
            if ((existing->ProcessObject &&
                 (existing->ProcessObject != TargetProcess ||
                  existing->ProcessCreateTime != createTime))) {
                status = STATUS_INVALID_CID;
            } else if (existing->DebuggerPid != Config->DebuggerPid) {
                status = STATUS_SHARING_VIOLATION;
            } else if (existing->BridgeOwned != BridgeOwned) {
                /* A manual protection record and a Bridge binding have
                 * different teardown owners.  Sharing one mutable record
                 * would let UNBIND silently delete manual policy (or let a
                 * generic update corrupt Bridge bookkeeping). */
                status = STATUS_SHARING_VIOLATION;
            } else {
                if (!existing->ProcessObject) {
                    existing->ProcessObject = TargetProcess;
                ...
                existing->PreventSuspend = Config->PreventSuspend;
                existing->PreventMemoryAccess = Config->PreventMemoryAccess;
                existing->BridgeBindFlags = BridgeOwned ? BridgeBindFlags : 0;
                if (Config->ProcessName[0] != L'\0') {
                    RtlStringCbCopyNW(existing->ProcessName,
                                      sizeof(existing->ProcessName),
                                      Config->ProcessName,
            ...
            InsertTailList(&g_ProtectedProcessList, &record->ListEntry);
            inserted = TRUE;
        }
    }
    KeReleaseSpinLock(&g_ProtectedProcessLock, oldIrql);
    ...
    if (!inserted) {
        ...
        ExFreePoolWithTag(record, HV_HOOK_TAG);
    }
    return status;
}
...
HvHookpHasBridgeBindings(VOID)
{
    ...
    if (!g_ProtectedProcessInitialized) return FALSE;
    KeAcquireSpinLock(&g_ProtectedProcessLock, &oldIrql);
    for (entry = g_ProtectedProcessList.Flink;
         entry != &g_ProtectedProcessList;
         entry = entry->Flink)
    {
        PPROTECTED_PROCESS process =
            CONTAINING_RECORD(entry, PROTECTED_PROCESS, ListEntry);
        if (process->BridgeOwned) {
            found = TRUE;
            break;
        }
    }
...
static NTSTATUS
HvHookpUnprotectManualProcessLocked(
    _In_ ULONG ProcessId
)
{
    PLIST_ENTRY entry;
         ...
         entry = entry->Flink)
    {
        ...
        if (proc->ProcessId == ProcessId && !proc->BridgeOwned) {
            RemoveEntryList(entry);
            found = TRUE;
            break;
        }
    ...
    if (found && proc) {
        if (proc->ProcessObject) {
            ObDereferenceObject(proc->ProcessObject);
        }
        ExFreePoolWithTag(proc, HV_HOOK_TAG);
        return STATUS_SUCCESS;
    }
...
)
{
    ...
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    status = HvHookpUnprotectManualProcessLocked(ProcessId);
    ExReleasePushLockExclusive(&g_DebuggerMutationLock);
    KeLeaveCriticalRegion();
    return status;
}
...
)
{
    ...
    if (!Process || !ProcessId) return STATUS_INVALID_PARAMETER;
    if (!g_HvHookInitialized || !g_ProtectedProcessInitialized) {
        return STATUS_NOT_INITIALIZED;
    }
    if (!g_DebuggerMutationLockInitialized || KeGetCurrentIrql() > APC_LEVEL) {
    ...
    if ((ULONG)(ULONG_PTR)PsGetProcessId(Process) != pid || createTime == 0) {
        return STATUS_INVALID_CID;
    }
         ...
         entry = entry->Flink)
    {
        PPROTECTED_PROCESS candidate =
            CONTAINING_RECORD(entry, PROTECTED_PROCESS, ListEntry);
        if (candidate->ProcessId != pid) continue;

        sawPid = TRUE;
        if (candidate->ProcessObject == Process &&
            candidate->ProcessCreateTime == createTime) {
            RemoveEntryList(entry);
            proc = candidate;
        }
        break;
    ...
    if (proc) {
        BOOLEAN bridgeOwned = proc->BridgeOwned;
        if (proc->ProcessObject) ObDereferenceObject(proc->ProcessObject);
        ExFreePoolWithTag(proc, HV_HOOK_TAG);
        if (bridgeOwned && !HvHookpHasBridgeBindings()) {
            HvHookpDisableBridgeAntiDebugLocked();
        }
        status = STATUS_SUCCESS;
    } else {
        status = sawPid ? STATUS_INVALID_CID : STATUS_NOT_FOUND;
    }
...
)
{
    ...
    (void)HvPrivateDebugObjectUnbind(
        (HANDLE)(ULONG_PTR)DebuggerPid,
        (HANDLE)(ULONG_PTR)ProcessId);
    HvPebCloakUnregisterTarget((HANDLE)(ULONG_PTR)ProcessId);
    if (proc->ProcessObject) ObDereferenceObject(proc->ProcessObject);
    ExFreePoolWithTag(proc, HV_HOOK_TAG);
    if (!HvHookpHasBridgeBindings()) HvHookpDisableBridgeAntiDebugLocked();
    return STATUS_SUCCESS;
}
...
)
{
    ...
    if (!g_ProtectedProcessInitialized || DebuggerPid == 0) {
        return 0;
    }
    ...
    for (;;) {
        ...
        (void)HvPrivateDebugObjectUnbind(
            (HANDLE)(ULONG_PTR)DebuggerPid,
            (HANDLE)(ULONG_PTR)proc->ProcessId);
        HvPebCloakUnregisterTarget(
            (HANDLE)(ULONG_PTR)proc->ProcessId);
        if (proc->ProcessObject) {
            ObDereferenceObject(proc->ProcessObject);
        }
    ...
    HvPrivateDebugObjectUnbindAllForDebugger(
        (HANDLE)(ULONG_PTR)DebuggerPid);

    if (!HvHookpHasBridgeBindings()) {
        HvHookpDisableBridgeAntiDebugLocked();
    }
    return removed;
}
...
)
{
    ...
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    removed = HvHookpUnprotectAllForDebuggerLocked(DebuggerPid);
    ExReleasePushLockExclusive(&g_DebuggerMutationLock);
    KeLeaveCriticalRegion();
    return removed;
}
...
)
{
         ...
         entry = entry->Flink)
    {
        PPROTECTED_PROCESS proc =
            CONTAINING_RECORD(entry, PROTECTED_PROCESS, ListEntry);
        if (proc->BridgeOwned && proc->DebuggerPid == DebuggerPid) {
            count++;
        }
    }
    KeReleaseSpinLock(&g_ProtectedProcessLock, oldIrql);
...
)
{
    ...
    if (!g_ProtectedProcessInitialized || ProcessId == 0 || DebuggerPid == 0) {
        return FALSE;
    }
...
static BOOLEAN
HvHookpBridgeTargetHasBindFlag(
    _In_ ULONG ProcessId,
    _In_ ULONG DebuggerPid,
    _In_ ULONG Flag
)
{
    ...
    if (!g_ProtectedProcessInitialized || ProcessId == 0 ||
        DebuggerPid == 0 || Flag == 0) {
        return FALSE;
    }
         ...
         entry = entry->Flink)
    {
        ...
        if (process->BridgeOwned &&
            ...
            process->DebuggerPid == DebuggerPid &&
            (process->BridgeBindFlags & Flag) != 0) {
            found = TRUE;
            break;
        }
    }
...
{
    PPROTECTED_PROCESS binding = NULL;
    PLIST_ENTRY entry;
    KIRQL oldIrql;
    ...
    if (!g_HvHookInitialized || !g_ProtectedProcessInitialized ||
        !g_DebuggerMutationLockInitialized ||
        KeGetCurrentIrql() != PASSIVE_LEVEL) {
        return STATUS_INVALID_DEVICE_STATE;
    }
    ...
    if (!binding) {
        status = STATUS_NOT_FOUND;
    } else if (!binding->ProcessObject ||
               binding->ProcessCreateTime == 0 ||
               PsGetProcessExitStatus(binding->ProcessObject) != STATUS_PENDING) {
        status = STATUS_PROCESS_IS_TERMINATING;
    } else if ((binding->BridgeBindFlags &
                HV_BRIDGE_BIND_DEFER_PEB_CLOAK) == 0) {
        status = STATUS_SUCCESS;
    } else {
    ...
    if (status == STATUS_PENDING) {
        status = HvPebCloakRegisterTarget(
            (HANDLE)(ULONG_PTR)ProcessId);
        if (NT_SUCCESS(status)) {
            KeAcquireSpinLock(&g_ProtectedProcessLock, &oldIrql);
            binding->BridgeBindFlags &= ~HV_BRIDGE_BIND_DEFER_PEB_CLOAK;
            KeReleaseSpinLock(&g_ProtectedProcessLock, oldIrql);
        }
    }
...
static BOOLEAN
HvHookpShouldProtectDebuggerPid(_In_ HANDLE Pid)
{
    PLIST_ENTRY entry;
    KIRQL oldIrql;
         ...
         entry = entry->Flink)
    {
        PPROTECTED_DEBUGGER debugger =
            CONTAINING_RECORD(entry, PROTECTED_DEBUGGER, ListEntry);
        if (debugger->ProcessId == targetPid) {
            protect = debugger->ProtectFromTerminate;
            break;
        }
    }
    KeReleaseSpinLock(&g_ProtectedDebuggerLock, oldIrql);
...
)
{
    ...
    if ((ULONG)(ULONG_PTR)PsGetCurrentProcessId() == targetPid) {
        liveProcess = PsGetCurrentProcess();
    } else {
        if (!NT_SUCCESS(PsLookupProcessByProcessId(
                (HANDLE)(ULONG_PTR)targetPid, &liveProcess))) {
            return FALSE;
        }
        referenced = TRUE;
    }

    liveCreateTime = (UINT64)PsGetProcessCreateTimeQuadPart(liveProcess);
    if (liveCreateTime != 0 &&
        PsGetProcessExitStatus(liveProcess) == STATUS_PENDING) {
        KeAcquireSpinLock(&g_ProtectedProcessLock, &oldIrql);
        for (entry = g_ProtectedProcessList.Flink;
             entry != &g_ProtectedProcessList;
             entry = entry->Flink)
        {
            PPROTECTED_PROCESS proc =
                CONTAINING_RECORD(entry, PROTECTED_PROCESS, ListEntry);
            if (proc->BridgeOwned &&
                proc->ProcessId == targetPid &&
                proc->ProcessObject == liveProcess &&
                proc->ProcessCreateTime == liveCreateTime) {
                found = TRUE;
                break;
            }
        }
    ...
    if (referenced) ObDereferenceObject(liveProcess);
    return found;
}
...
static PETHREAD HvHookpReferenceThreadHandle(HANDLE ThreadHandle)
{
    PETHREAD ethread = NULL;
    NTSTATUS s = ObReferenceObjectByHandle(
        ThreadHandle, 0,
        *PsThreadType, KernelMode, (PVOID*)&ethread, NULL);
    return NT_SUCCESS(s) ? ethread : NULL;
}

static HANDLE HvHookpThreadHandleToProcessId(HANDLE ThreadHandle)
{
    PETHREAD ethread = HvHookpReferenceThreadHandle(ThreadHandle);
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtSetContextThread, STATUS_DELETE_PENDING)
    ...
    // 目标进程自写 DR 时不让真实硬件断点落地。Bridge 基线或显式 AntiVMP
    // 任一路径命中即可；调试器进程写目标 DR 仍走下面的 vwatch 虚拟化。
    BOOLEAN scrubSelfDr =
        (caller == targetPid && targetPid != NULL) &&
        ((g_BridgeAntiDebugEnabled && HvHookIsBridgeOwnedTargetPid(caller)) ||
         (g_AntiAntiDebugConfig.HookNtSetContextThread &&
          HvHookpIsAntiAntiDebugCaller()));
    if (scrubSelfDr && ThreadContext) {
        __try {
            if (ExGetPreviousMode() == UserMode) {
                ProbeForWrite(ThreadContext, HV_CTX_OFF_DR7 + 8, sizeof(ULONG));
            }
            UCHAR* base = (UCHAR*)ThreadContext;
            ULONG selfFlags = *(ULONG*)(base + HV_CTX_OFF_FLAGS);
            if (selfFlags & 0x10) {
    ...
    KPROCESSOR_MODE prevMode = ExGetPreviousMode();
    ULONG flags = 0;
    UINT64 dr0 = 0, dr1 = 0, dr2 = 0, dr3 = 0, dr6 = 0, dr7 = 0;
    BOOLEAN hasDr = FALSE;
    ...
    __try {
        ...
        if (hasDr) {
            ...
            dr6 = *(UINT64*)(base + HV_CTX_OFF_DR6);
    ...
    PETHREAD targetThread = HvHookpReferenceThreadHandle(ThreadHandle);
    if (!targetThread) {
        return Original(ThreadHandle, ThreadContext);
    }
    ...
    NTSTATUS originalStatus = Original(ThreadHandle, ThreadContext);
...
static PCSTR
HvHookpReadCopyStageDetail(_In_ ULONG Stage)
{
    switch (Stage) {
    case HV_PHYS_COPY_STAGE_VALIDATE:
        return "ReadDirect: stage=1 validate; Original fallback";
    case HV_PHYS_COPY_STAGE_BUFFER_LOCK:
        return "ReadDirect: stage=2 buffer-lock; Original fallback";
    case HV_PHYS_COPY_STAGE_PROCESS_LOOKUP:
        return "ReadDirect: stage=3 process-lookup; Original fallback";
    case HV_PHYS_COPY_STAGE_TARGET_LOCK:
        return "ReadDirect: stage=4 target-lock; Original fallback";
    case HV_PHYS_COPY_STAGE_CR3_RESOLVE:
        return "ReadDirect: stage=5 cr3-resolve; Original fallback";
    case HV_PHYS_COPY_STAGE_VT_ROOT_COPY:
        return "ReadDirect: stage=6 vt-root-copy; Original fallback";
    case HV_PHYS_COPY_STAGE_COMPLETE:
        return "ReadDirect: stage=7 partial-copy; Original fallback";
    case HV_PHYS_COPY_STAGE_MDL_FALLBACK:
        return "ReadDirect: stage=8 mdl-fallback; Original fallback";
    default:
        return "ReadDirect: stage=0 unknown; Original fallback";
    }
}
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtReadVirtualMemory, STATUS_DELETE_PENDING)
    ...
    __try {
        ...
        st = HvPhysReadProcessMemoryDiagnosed(
            (ULONG)(ULONG_PTR)targetPid,
            (UINT64)(ULONG_PTR)BaseAddress,
            Buffer, BufferSize, &done, &copyDiag);
        if (NumberOfBytesRead) {
    ...
    if (!NT_SUCCESS(st) || done < BufferSize) {
        NTSTATUS detailStatus = copyDiag.DetailStatus;
        if (NT_SUCCESS(detailStatus)) {
            detailStatus = NT_SUCCESS(st) ? STATUS_PARTIAL_COPY : st;
        }
        HvDbgEvtPost(!NT_SUCCESS(st) ? HV_DBGEVT_SEV_ERROR : HV_DBGEVT_SEV_WARN,
                     HV_DBGEVT_CAT_READ_MEMORY, detailStatus,
                     (ULONG)(ULONG_PTR)caller,
                     (ULONG)(ULONG_PTR)targetPid,
                     (UINT64)(ULONG_PTR)BaseAddress, BufferSize,
                     HvHookpReadCopyStageDetail(copyDiag.Stage));
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtWriteVirtualMemory, STATUS_DELETE_PENDING)
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtReadVirtualMemoryEx, STATUS_DELETE_PENDING)
...
static NTSTATUS NTAPI HookedNtMapViewOfSection(
    ...
    ULONG Win32Protect)
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtMapViewOfSection, STATUS_DELETE_PENDING)
...
static NTSTATUS NTAPI HookedNtMapViewOfSectionEx(
    ...
    ULONG PageProtection, PVOID Parameters)
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtMapViewOfSectionEx, STATUS_DELETE_PENDING)
...
static NTSTATUS NTAPI HookedNtCreateSection(
    ...
    ULONG AllocationAttributes, HANDLE FileHandle)
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtCreateSection, STATUS_DELETE_PENDING)
...
static NTSTATUS NTAPI HookedNtCreateThreadEx(
    ...
    SIZE_T MaximumStackSize, PVOID AttributeList)
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtCreateThreadEx, STATUS_DELETE_PENDING)
    ...
    HANDLE caller = PsGetCurrentProcessId();
    if ((CreateFlags & 0x4UL) != 0 &&
        g_AntiAntiDebugConfig.HookNtSetInformationThread &&
        HvHookpIsAntiAntiDebugCaller()) {
        CreateFlags &= ~0x4UL;
        HV_HOT_DBG("[HvHook-AAD] NtCreateThreadEx HIDE_FROM_DEBUGGER cleared\n");
    }
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtSuspendThread, STATUS_DELETE_PENDING)
...
static NTSTATUS NTAPI HookedNtResumeThread(
    HANDLE ThreadHandle, PULONG PreviousSuspendCount)
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtResumeThread, STATUS_DELETE_PENDING)
...
static NTSTATUS NTAPI HookedNtOpenThread(
    ...
    PVOID ObjectAttributes, PVOID ClientId)
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtOpenThread, STATUS_DELETE_PENDING)
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtGetContextThread, STATUS_DELETE_PENDING)
    ...
    // caller 是 debugger → P128: 用 vwatch 表重组虚拟 DR 地址/控制状态.
    // Dr6 belongs to the stopped thread's native trap frame and must remain
    // untouched so the debugger can distinguish HWBP B0..B3 from TF/MTF.
    if (HvHookIsDebuggerPid(caller)) {
        if (targetPid) {
            PETHREAD targetThread = HvHookpReferenceThreadHandle(ThreadHandle);
            if (targetThread) {
            ...
            if (targetThread) {
                KPROCESSOR_MODE pendingMode = ExGetPreviousMode();
                __try {
                    if (pendingMode == UserMode) {
                        ProbeForWrite(
                            ThreadContext,
                            HV_CTX_OFF_DR7 + 8,
                            sizeof(ULONG));
                    ...
                    UINT64 pendingDr6 = 0;
                    if ((flags & 0x10UL) != 0 &&
                        HvVwatchQueryPendingHardwareHit(
                            targetThread,
                            targetPid,
                            &pendingDr6)) {
                        *(UINT64*)(base + HV_CTX_OFF_DR6) |= pendingDr6;
                    }
                } __except (EXCEPTION_EXECUTE_HANDLER) {
                }
                ObDereferenceObject(targetThread);
    ...
    if (!((g_BridgeAntiDebugEnabled && HvHookIsBridgeOwnedTargetPid(caller)) ||
          (g_AntiAntiDebugConfig.HookNtGetContextThread &&
           HvHookpIsAntiAntiDebugCaller()))) {
        return s;
    }
...
static NTSTATUS
HvHookpAcquirePrivateDbgkIngressLocked(_In_opt_ PVOID ResolvedAddress)
{
    HV_HOOK_HANDLE candidate = NULL;
    HV_HOOK_INFO info;
    PVOID target = ResolvedAddress
        ? ResolvedAddress : g_PrivateDbgkIngressAddress;
    NTSTATUS status;
    ...
    if (g_HookNtDebugActiveProcess) {
        status = HvHookGetInfo(g_HookNtDebugActiveProcess, &info);
        if (!NT_SUCCESS(status) ||
            info.TargetVirtualAddress != target ||
            info.HookFunction != HookedNtDebugActiveProcess) {
            return STATUS_CONFLICTING_ADDRESSES;
        }
        g_HookOwnersNtDebugActiveProcess |= HV_HOOK_OWNER_PRIVATE_DBGK;
        return STATUS_SUCCESS;
    }
    ...
    g_HookNtDebugActiveProcess = candidate;
    g_HookOwnersNtDebugActiveProcess |= HV_HOOK_OWNER_PRIVATE_DBGK;
    return STATUS_SUCCESS;
}

static NTSTATUS
HvHookpReleasePrivateDbgkIngressLocked(VOID)
{
    return HvHookpReleaseSharedHookLocked(
        L"NtDebugActiveProcess",
        &g_HookNtDebugActiveProcess,
        &g_HookOwnersNtDebugActiveProcess,
        HV_HOOK_OWNER_PRIVATE_DBGK);
}
...
typedef struct _HV_PEB_CLOAK_REG_CTX {
    ...
    BOOLEAN         RequirePrivateDbgk;
} HV_PEB_CLOAK_REG_CTX, *PHV_PEB_CLOAK_REG_CTX;

static BOOLEAN
HvHookpPebCloakRegistrationIsCurrent(
    _In_ PHV_PEB_CLOAK_REG_CTX Context,
    _In_ PEPROCESS CurrentTargetProcess)
{
    PEPROCESS debuggerProcess = NULL;
    NTSTATUS status;
    BOOLEAN current = FALSE;

    if (!Context || !CurrentTargetProcess || !Context->TargetPid ||
        !Context->DebuggerPid ||
        CurrentTargetProcess != Context->TargetProcess ||
        (UINT64)PsGetProcessCreateTimeQuadPart(CurrentTargetProcess) !=
            Context->TargetCreateTime ||
        PsGetProcessExitStatus(CurrentTargetProcess) != STATUS_PENDING ||
        InterlockedCompareExchange(&g_HvHookClosing, 0, 0) != 0 ||
        !HvHookIsBoundTargetForDebugger(
            (ULONG)(ULONG_PTR)Context->TargetPid,
            (ULONG)(ULONG_PTR)Context->DebuggerPid) ||
        (Context->RequirePrivateDbgk &&
    ...
    status = PsLookupProcessByProcessId(
        Context->DebuggerPid,
        &debuggerProcess);
    if (NT_SUCCESS(status) && debuggerProcess) {
        current =
            (UINT64)PsGetProcessCreateTimeQuadPart(debuggerProcess) ==
                Context->DebuggerCreateTime &&
            PsGetProcessExitStatus(debuggerProcess) == STATUS_PENDING;
...
static VOID HvHookpPebCloakRegisterWorker(_In_ PVOID Context)
{
    ...
    NTSTATUS scrubStatus = STATUS_INVALID_CID;
    NTSTATUS cloakStatus = STATUS_INVALID_CID;
    PEPROCESS currentProcess = NULL;
    NTSTATUS identityStatus =
        InterlockedCompareExchange(&g_HvHookClosing, 0, 0) == 0
            ? PsLookupProcessByProcessId(targetPid, &currentProcess)
            : STATUS_DELETE_PENDING;

    if (NT_SUCCESS(identityStatus) && currentProcess &&
        HvHookpPebCloakRegistrationIsCurrent(ctx, currentProcess)) {
        scrubStatus = HvDbgScrubPebDebugState(
            (ULONG)(ULONG_PTR)targetPid);
        if (InterlockedCompareExchange(&g_HvHookClosing, 0, 0) == 0 &&
            HvHookpPebCloakRegistrationIsCurrent(ctx, currentProcess)) {
            cloakStatus = HvPebCloakRegisterTarget(targetPid);
            if (NT_SUCCESS(cloakStatus) &&
                !HvHookpPebCloakRegistrationIsCurrent(ctx, currentProcess)) {
                HvPebCloakUnregisterTarget(targetPid);
                cloakStatus = STATUS_DELETE_PENDING;
            }
        } else {
            cloakStatus = STATUS_DELETE_PENDING;
        }
    ...
    HvDbgEvtPost(NT_SUCCESS(scrubStatus) ? HV_DBGEVT_SEV_INFO : HV_DBGEVT_SEV_ERROR,
                 HV_DBGEVT_CAT_AAD, scrubStatus, 0,
                 (ULONG)(ULONG_PTR)targetPid, 0, 0,
                 "NtDebugActiveProcess PEB scrub");
    DbgPrint("[HvHook-AAD] PEB scrub target=%u status=0x%X\n",
             (ULONG)(ULONG_PTR)targetPid, scrubStatus);
    HvDbgEvtPost(NT_SUCCESS(cloakStatus) ? HV_DBGEVT_SEV_INFO : HV_DBGEVT_SEV_ERROR,
                 HV_DBGEVT_CAT_AAD, cloakStatus, 0,
                 (ULONG)(ULONG_PTR)targetPid, 0, 0,
                 "NtDebugActiveProcess VT PEB cloak");
    DbgPrint("[HvHook-AAD] VT PEB cloak target=%u status=0x%X\n",
             (ULONG)(ULONG_PTR)targetPid, cloakStatus);

    if (ctx->TargetProcess) {
        ObDereferenceObject(ctx->TargetProcess);
        ctx->TargetProcess = NULL;
    }
    ExFreePoolWithTag(ctx, 'HwAD');
    ExReleaseRundownProtection(&g_PebCloakWorkerRundown);
}

static NTSTATUS
HvHookpQueuePebCloakRegistration(
    _In_ HANDLE ProcessHandle,
    _In_opt_ HANDLE ExpectedTargetPid,
    _In_ HANDLE ExpectedDebuggerPid,
    _In_ BOOLEAN RequirePrivateDbgk)
{
    PEPROCESS targetProcess = NULL;
    PEPROCESS debuggerProcess;
    PHV_PEB_CLOAK_REG_CTX context;
    HANDLE targetPid;
    UINT64 targetCreateTime;
    UINT64 debuggerCreateTime;
    NTSTATUS status;
    ...
    debuggerProcess = PsGetCurrentProcess();
    if (!ExpectedDebuggerPid ||
        PsGetProcessId(debuggerProcess) != ExpectedDebuggerPid ||
        PsGetProcessExitStatus(debuggerProcess) != STATUS_PENDING) {
        return STATUS_INVALID_CID;
    }
    debuggerCreateTime =
        (UINT64)PsGetProcessCreateTimeQuadPart(debuggerProcess);
    if (debuggerCreateTime == 0) {
        return STATUS_INVALID_CID;
    }
    ...
    targetCreateTime = (UINT64)PsGetProcessCreateTimeQuadPart(targetProcess);
    if (!targetPid || targetCreateTime == 0 ||
        (ExpectedTargetPid && targetPid != ExpectedTargetPid)) {
        ObDereferenceObject(targetProcess);
        return STATUS_INVALID_CID;
    }
    if (!g_PebCloakWorkerRundownReady ||
        !ExAcquireRundownProtection(&g_PebCloakWorkerRundown)) {
        ObDereferenceObject(targetProcess);
        return STATUS_DELETE_PENDING;
    }
    if (InterlockedCompareExchange(&g_HvHookClosing, 0, 0) != 0) {
    ...
    context = (PHV_PEB_CLOAK_REG_CTX)ExAllocatePool2(
        POOL_FLAG_NON_PAGED,
        sizeof(HV_PEB_CLOAK_REG_CTX),
        'HwAD');
    if (!context) {
        ExReleaseRundownProtection(&g_PebCloakWorkerRundown);
        ObDereferenceObject(targetProcess);
    ...
    context->TargetCreateTime = targetCreateTime;
    context->DebuggerPid = ExpectedDebuggerPid;
    context->DebuggerCreateTime = debuggerCreateTime;
    context->RequirePrivateDbgk = RequirePrivateDbgk;
    ExInitializeWorkItem(
        &context->WorkItem,
        HvHookpPebCloakRegisterWorker,
        context);
...
static NTSTATUS NTAPI HookedNtDebugActiveProcess(
    ...
    HANDLE DebugObjectHandle)
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtDebugActiveProcess, STATUS_DELETE_PENDING)
    PFN_NtDebugActiveProcess Original =
        (PFN_NtDebugActiveProcess)HvHookGetTrampoline(g_HookNtDebugActiveProcess);
    if (!Original) return STATUS_UNSUCCESSFUL;

    /* Private Dbgk mode must never enter DbgkpSetProcessDebugObject: that
     * would publish a native EPROCESS.DebugPort and recreate the very
     * detection surface this mode is meant to remove.  The private session
     * owns attach, initial CREATE_PROCESS delivery, wait/continue and detach. */
    if (HvPrivateDebugObjectIsEnabled() &&
        ExGetPreviousMode() != KernelMode &&
        HvHookIsDebuggerPid(PsGetCurrentProcessId())) {
        HANDLE targetPid = NULL;
        NTSTATUS privateStatus = HvPrivateDebugObjectAttach(
            PsGetCurrentProcessId(), ProcessHandle, &targetPid);
        if (NT_SUCCESS(privateStatus) && targetPid) {
            HV_PROTECT_CONFIG protectConfig = { 0 };
            NTSTATUS scrubStatus;
            NTSTATUS cloakQueueStatus;
            BOOLEAN deferPebCloak;
            protectConfig.ProcessId = (ULONG)(ULONG_PTR)targetPid;
            protectConfig.DebuggerPid =
                (ULONG)(ULONG_PTR)PsGetCurrentProcessId();
            protectConfig.PreventTerminate = TRUE;
            protectConfig.PreventSuspend = TRUE;
            protectConfig.PreventMemoryAccess = TRUE;
            (void)HvHookProtectProcess(&protectConfig);
            scrubStatus = HvDbgScrubPebDebugState(
                (ULONG)(ULONG_PTR)targetPid);
            deferPebCloak = HvHookpBridgeTargetHasBindFlag(
                (ULONG)(ULONG_PTR)targetPid,
                (ULONG)(ULONG_PTR)PsGetCurrentProcessId(),
                HV_BRIDGE_BIND_DEFER_PEB_CLOAK);
            cloakQueueStatus = deferPebCloak
                ? STATUS_PENDING
                : HvHookpQueuePebCloakRegistration(
                    ProcessHandle,
                    targetPid,
                    ...
                    TRUE);
            HvDbgEvtPost(
                NT_SUCCESS(scrubStatus)
                    ? HV_DBGEVT_SEV_INFO : HV_DBGEVT_SEV_ERROR,
                HV_DBGEVT_CAT_AAD,
                scrubStatus,
                (ULONG)(ULONG_PTR)PsGetCurrentProcessId(),
                (ULONG)(ULONG_PTR)targetPid,
                0,
                0,
                "Private Dbgk PEB scrub");
            HvDbgEvtPost(
                NT_SUCCESS(cloakQueueStatus)
                    ? HV_DBGEVT_SEV_INFO : HV_DBGEVT_SEV_ERROR,
                HV_DBGEVT_CAT_AAD,
                cloakQueueStatus,
                (ULONG)(ULONG_PTR)PsGetCurrentProcessId(),
                ...
                0,
                deferPebCloak
                    ? "Private Dbgk VT PEB cloak deferred to PE entry"
                    : "Private Dbgk VT PEB cloak queued");
            return STATUS_SUCCESS;
        }
        if (privateStatus != STATUS_NOT_SUPPORTED &&
            privateStatus != STATUS_PORT_NOT_SET) {
    ...
    UINT64 targetCreateTime =
        (UINT64)PsGetProcessCreateTimeQuadPart(tmpProc);
    if (!targetPid || targetCreateTime == 0) {
        ObDereferenceObject(tmpProc);
        return s;
    }

    // Native debuggers may bypass kernel32!DebugActiveProcess entirely. Bind
    // the successful kernel attach here so the driver proxy has the target
    // before control returns to the debugger; the injected Bridge remains the
    // fallback for debug-create and debug-event paths.
    {
        HV_PROTECT_CONFIG protectConfig = { 0 };
        protectConfig.ProcessId = (ULONG)(ULONG_PTR)targetPid;
        protectConfig.DebuggerPid = (ULONG)(ULONG_PTR)caller;
        protectConfig.PreventTerminate = TRUE;
        protectConfig.PreventSuspend = TRUE;
        protectConfig.PreventMemoryAccess = TRUE;

        NTSTATUS bindStatus = HvHookProtectProcess(&protectConfig);
        HvDbgEvtPost(NT_SUCCESS(bindStatus) ? HV_DBGEVT_SEV_INFO : HV_DBGEVT_SEV_ERROR,
                     HV_DBGEVT_CAT_AAD, bindStatus,
                     (ULONG)(ULONG_PTR)caller,
                     (ULONG)(ULONG_PTR)targetPid,
                      0, 0, "NtDebugActiveProcess target bind");
    }

    if (HvHookpBridgeTargetHasBindFlag(
            (ULONG)(ULONG_PTR)targetPid,
            (ULONG)(ULONG_PTR)caller,
            HV_BRIDGE_BIND_DEFER_PEB_CLOAK)) {
        ObDereferenceObject(tmpProc);
        HvDbgEvtPost(
            ...
            0,
            0,
            "Windows DebugObject VT PEB cloak deferred to PE entry");
        return s;
    }
    ...
    ctx->TargetProcess = tmpProc;
    ctx->TargetCreateTime = targetCreateTime;
    ctx->DebuggerPid = caller;
    ctx->DebuggerCreateTime =
        (UINT64)PsGetProcessCreateTimeQuadPart(PsGetCurrentProcess());
    ctx->RequirePrivateDbgk = FALSE;
...
static HV_SYSCALL_CACHE g_SyscallCache[] = {
    ...
    // DriverHide must be acquired during DriverEntry, before a user-mode
    // resolver process exists.  Its exported Zw stub supplies this index.
    { "NtQuerySystemInformation", 0xFFFFFFFF, NULL },
    { "NtQueryDirectoryFile", 0xFFFFFFFF, NULL },
    { "NtEnumerateKey", 0xFFFFFFFF, NULL },
    { "NtDeviceIoControlFile", 0xFFFFFFFF, NULL },
...
static NTSTATUS
HvHookpFindUserModuleBase(
    _In_ ULONG CallerPid,
    _In_ PCWSTR ModuleName,
    _Out_ PUINT64 OutBase)
{
    const ULONG capacity = 32;
    const SIZE_T allocationSize = sizeof(HV_MODULE_INFO) * capacity;
    PHV_MODULE_INFO modules = NULL;
    ULONG count = 0;
    NTSTATUS status;
    UNICODE_STRING wanted;

    *OutBase = 0;
    modules = (PHV_MODULE_INFO)HvAllocateNonPagedZeroed(
        allocationSize, 'mVhH');
    if (!modules) return STATUS_INSUFFICIENT_RESOURCES;

    status = HvPhysEnumerateModules(
        CallerPid, modules, capacity, &count);
    if (!NT_SUCCESS(status) || count == 0) {
        ExFreePoolWithTag(modules, 'mVhH');
    ...
    for (ULONG index = 0; index < count; ++index) {
        ...
        SIZE_T nameLength = 0;
        while (nameLength < HV_MODULE_NAME_MAX &&
               modules[index].Name[nameLength]) {
            ++nameLength;
        }
        current.Buffer = modules[index].Name;
        current.Length = (USHORT)(nameLength * sizeof(WCHAR));
        current.MaximumLength = current.Length;
        if (RtlEqualUnicodeString(&current, &wanted, TRUE)) {
            *OutBase = modules[index].DllBase;
            break;
        }
    }

    ExFreePoolWithTag(modules, 'mVhH');
    return *OutBase != 0 ? STATUS_SUCCESS : STATUS_NOT_FOUND;
}
...
)
{
    *OutIndex = 0xFFFFFFFF;
    ...
    UINT64 dllBase = 0;
    NTSTATUS s = HvHookpFindUserModuleBase(
        CallerPid, DllName, &dllBase);
    if (!NT_SUCCESS(s)) return s;
...
{
    return HvHookpResolveSyscallIndexInDll(
        CallerPid, L"ntdll.dll", FunctionName, OutIndex);
#if 0
    *OutIndex = 0xFFFFFFFF;
    if (!CallerPid) return STATUS_INVALID_PARAMETER;
    ...
    UINT64 ntdllBase = 0;
    NTSTATUS s = HvHookpFindUserModuleBase(
        CallerPid, L"ntdll.dll", &ntdllBase);
    DbgPrint("[HvHook] M3.6 SyscallIdx step C: module resolve returned 0x%X\n", s);
    if (!NT_SUCCESS(s)) return s;
...
static HV_SYSCALL_CACHE*
HvHookpFindSyscallCache(_In_ PCWSTR Name)
{
    SIZE_T i;
    char asciiName[64];
...
static PVOID
HvHookpResolveSsdtIndex(
    _In_ PCWSTR Name,
    _In_ ULONG SyscallIndex
)
{
    NTSTATUS status;
    HV_SYSCALL_CACHE* cache;
    PULONG table;
    LONG encoded;
    PVOID realAddr;

    status = HvHookpResolveSsdtBase();
    if (!NT_SUCCESS(status)) {
        return NULL;
    }

    cache = HvHookpFindSyscallCache(Name);
    if (!cache) {
        DbgPrint("[HvHook] L6: %ws not registered in SSDT cache table\n", Name);
        return NULL;
    }
    if (cache->KernelAddr && cache->Index == SyscallIndex) {
        return cache->KernelAddr;
    }
    if (SyscallIndex >= g_NumberOfServices) {
        DbgPrint("[HvHook] L6: %ws syscall index %u exceeds service count %u\n",
                 Name, SyscallIndex, g_NumberOfServices);
        return NULL;
    }

    // SSDT[idx] decode: Win10/11 是 (int32 >> 4) + base,low 4 bits 是参数计数
    table = (PULONG)g_KiServiceTableBase;
    encoded = (LONG)table[SyscallIndex];
    realAddr = (PVOID)((ULONG_PTR)g_KiServiceTableBase + (encoded >> 4));
...
static PVOID HvHookpResolveViaSsdt(_In_ PCWSTR Name, _In_ ULONG CallerPid)
{
    ...
    status = HvHookpResolveSyscallIndex(
        CallerPid, asciiName, &syscallIndex);
    if (!NT_SUCCESS(status)) {
        DbgPrint("[HvHook] L6: %ws syscall index lookup failed: 0x%X\n",
                 Name, status);
        return NULL;
    }

    return HvHookpResolveSsdtIndex(Name, syscallIndex);
}
...
static PVOID
HvHookpResolveKernelSyscallCandidate(
    _In_ PCWSTR Name,
    _In_opt_ PVOID Candidate
)
{
    UCHAR head[8] = { 0 };
    ULONG syscallIndex;
    ...
    __try {
        RtlCopyMemory(head, Candidate, sizeof(head));
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        return NULL;
    }

    /* Never install an inline hook on a kernel Zw syscall stub. */
    if (head[0] == 0x4C && head[1] == 0x8B &&
        head[2] == 0xD1 && head[3] == 0xB8) {
        RtlCopyMemory(&syscallIndex, &head[4], sizeof(syscallIndex));
        Candidate = HvHookpResolveSsdtIndex(Name, syscallIndex);
        if (!Candidate) {
            DbgPrint("[HvHook] %ws kernel stub index 0x%X decode failed\n",
                     Name, syscallIndex);
        }
    }
...
{
    UNICODE_STRING us;
    UNICODE_STRING known;
    PVOID addr;
    SIZE_T i;

    RtlInitUnicodeString(&us, Name);
    RtlInitUnicodeString(&known, L"NtSetContextThread");
    if (g_SymbolNtSetContextThread &&
        RtlEqualUnicodeString(&us, &known, FALSE)) {
        return g_SymbolNtSetContextThread;
    }
    RtlInitUnicodeString(&known, L"NtReadVirtualMemory");
    if (g_SymbolNtReadVirtualMemory &&
        RtlEqualUnicodeString(&us, &known, FALSE)) {
        return g_SymbolNtReadVirtualMemory;
    }
    RtlInitUnicodeString(&known, L"NtWriteVirtualMemory");
    if (g_SymbolNtWriteVirtualMemory &&
        RtlEqualUnicodeString(&us, &known, FALSE)) {
        return g_SymbolNtWriteVirtualMemory;
    }

    // 路径 1: 标准 MmGetSystemRoutineAddress
    addr = MmGetSystemRoutineAddress(&us);
    if (addr) {
        return HvHookpResolveKernelSyscallCandidate(Name, addr);
    }
        ...
        if (addr) {
            ...
            if (head[0] == 0x4C && head[1] == 0x8B && head[2] == 0xD1 && head[3] == 0xB8) {
                ...
                __try {
                    ...
                    syscallIndex = 0xFFFFFFFF;
                }
                DbgPrint("[HvHook] %ws via Zw is syscall stub @ %p, skipping (will try SSDT)\n",
                         Name, addr);
                if (syscallIndex != 0xFFFFFFFF) {
                    serviceAddress = HvHookpResolveSsdtIndex(
                        Name, syscallIndex);
                    if (serviceAddress) {
                        DbgPrint("[HvHook] %ws resolved from kernel Zw stub index 0x%X -> %p\n",
                                 Name, syscallIndex, serviceAddress);
                        return serviceAddress;
                    }
                }
                // 不直接返回, 让代码继续到 EAT/user SSDT path
                addr = NULL;
            } else {
    ...
    // 路径 3: 手动 walk ntoskrnl PE EAT (MmGetSystemRoutineAddress 漏掉的名字)
    {
        ...
        if (ntosBase) {
            ...
            if (addr) {
                addr = HvHookpResolveKernelSyscallCandidate(Name, addr);
                if (addr) {
...
{
    if (!RoutineName || KeGetCurrentIrql() > APC_LEVEL) {
        return NULL;
    }
    return HvHookpGetNtRoutine(RoutineName, 0);
}
...
HvHookpEnableBridgeAntiDebugLocked(_In_ ULONG CallerPid)
{
    ...
    if (!HvHookIsDebuggerPid((HANDLE)(ULONG_PTR)CallerPid)) {
        return STATUS_NOT_FOUND;
    }
    if (g_BridgeAntiDebugEnabled &&
        g_HookNtQueryInformationProcess &&
        (g_HookOwnersNtQueryInformationProcess & HV_HOOK_OWNER_BRIDGE) != 0 &&
        (!HvPrivateDebugObjectIsEnabled() ||
         (g_HookNtQueryObject &&
          (g_HookOwnersNtQueryObject & HV_HOOK_OWNER_BRIDGE) != 0))) {
        return STATUS_SUCCESS;
    }
    ...
    g_BridgeAntiDebugEnabled = TRUE;
    DbgPrint("[HvHook] Bridge anti-debug owner acquired\n");
    return STATUS_SUCCESS;
}
...
NTSTATUS HvHookEnableBridgeAntiDebug(_In_ ULONG CallerPid)
{
    ...
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    status = HvHookpEnableBridgeAntiDebugLocked(CallerPid);
    ExReleasePushLockExclusive(&g_DebuggerMutationLock);
    KeLeaveCriticalRegion();
    return status;
}
...
NTSTATUS
HvHookConfigurePrivateDebugObjectSymbols(
    _In_ const HV_PRIVATE_DBGK_SYMBOLS* Symbols)
{
    NTSTATUS status;
    BOOLEAN enabled;

    if (!Symbols || !Symbols->NtDebugActiveProcess) {
        return STATUS_INVALID_PARAMETER;
    }
    if (!g_HvHookInitialized || !g_DebuggerMutationLockInitialized ||
        KeGetCurrentIrql() > APC_LEVEL) {
        return STATUS_INVALID_DEVICE_STATE;
    }
    ...
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    enabled = HvPrivateDebugObjectIsEnabled();
    if (enabled) {
        status = HvHookpAcquirePrivateDbgkIngressLocked(
    ...
    status = HvPrivateDebugObjectConfigureSymbols(Symbols);
    if (!NT_SUCCESS(status)) {
        if (enabled) {
            (void)HvPrivateDebugObjectSetEnabled(FALSE);
            (void)HvHookpReleasePrivateDbgkIngressLocked();
        }
        goto Exit;
    }
    g_PrivateDbgkIngressAddress = Symbols->NtDebugActiveProcess;
    g_SymbolNtSetContextThread = Symbols->NtSetContextThread;
    g_SymbolNtReadVirtualMemory = Symbols->NtReadVirtualMemory;
    g_SymbolNtWriteVirtualMemory = Symbols->NtWriteVirtualMemory;
...
{
    typedef struct _HV_PRIVATE_DEBUG_BINDING_SNAPSHOT {
        ULONG DebuggerPid;
        ULONG TargetPid;
    } HV_PRIVATE_DEBUG_BINDING_SNAPSHOT;

    HV_PRIVATE_DEBUG_BINDING_SNAPSHOT bindings[32];
    ULONG bindingCount = 0;
    BOOLEAN bindingOverflow = FALSE;
    BOOLEAN hadPrivateIngressOwner;
    ...
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    KeAcquireSpinLock(&g_ProtectedProcessLock, &irql);
    for (PLIST_ENTRY entry = g_ProtectedProcessList.Flink;
         entry != &g_ProtectedProcessList;
         entry = entry->Flink) {
        PPROTECTED_PROCESS process = CONTAINING_RECORD(
            entry, PROTECTED_PROCESS, ListEntry);
        if (!process->BridgeOwned) continue;
        if (bindingCount >= RTL_NUMBER_OF(bindings)) {
            bindingOverflow = TRUE;
            break;
        }
        bindings[bindingCount].DebuggerPid = process->DebuggerPid;
        bindings[bindingCount].TargetPid = process->ProcessId;
        ++bindingCount;
    }
    KeReleaseSpinLock(&g_ProtectedProcessLock, irql);
    ...
    currentlyEnabled = HvPrivateDebugObjectIsEnabled();
    if (currentlyEnabled != Enabled && bindingCount != 0) {
        status = STATUS_DEVICE_BUSY;
        goto Exit;
    }

    hadPrivateIngressOwner =
        (g_HookOwnersNtDebugActiveProcess &
         HV_HOOK_OWNER_PRIVATE_DBGK) != 0;
    if (Enabled && g_PrivateDbgkIngressAddress) {
        status = HvHookpAcquirePrivateDbgkIngressLocked(
            g_PrivateDbgkIngressAddress);
        if (!NT_SUCCESS(status)) goto Exit;
    }

    status = HvPrivateDebugObjectSetEnabled(Enabled);
    if (!NT_SUCCESS(status)) {
        if (Enabled && !hadPrivateIngressOwner) {
            (void)HvHookpReleasePrivateDbgkIngressLocked();
        }
        goto Exit;
    }
    ...
    if (Enabled) {
        ...
        for (ULONG index = 0; index < bindingCount; ++index) {
            status = HvPrivateDebugObjectBind(
                (HANDLE)(ULONG_PTR)bindings[index].DebuggerPid,
                (HANDLE)(ULONG_PTR)bindings[index].TargetPid);
            if (!NT_SUCCESS(status)) {
                for (ULONG rollback = 0; rollback < index; ++rollback) {
                    (void)HvPrivateDebugObjectUnbind(
                        (HANDLE)(ULONG_PTR)bindings[rollback].DebuggerPid,
                        (HANDLE)(ULONG_PTR)bindings[rollback].TargetPid);
                }
                (void)HvPrivateDebugObjectSetEnabled(FALSE);
                (void)HvHookpReleaseSharedHookLocked(
                    L"NtQueryObject",
        ...
        (void)HvHookpReleaseSharedHookLocked(
            ...
            &g_HookOwnersNtQueryObject,
            HV_HOOK_OWNER_PRIVATE_DBGK);
        status = HvHookpReleasePrivateDbgkIngressLocked();
    }
...
)
{
    ...
    PEPROCESS targetProcess = NULL;
    NTSTATUS status;
    BOOLEAN debuggerExisted = FALSE;
    BOOLEAN bridgeOwnerExisted = FALSE;
    BOOLEAN privateObjectBound = FALSE;
    KIRQL oldIrql;

    if (!Config || DebuggerPid == 0 || Config->ProcessId == 0 ||
        Config->DebuggerPid != DebuggerPid ||
        Config->ProcessId == DebuggerPid) {
        return STATUS_INVALID_PARAMETER;
    }
    if (!g_HvHookInitialized || !g_DebuggerMutationLockInitialized ||
        KeGetCurrentIrql() > APC_LEVEL) {
    ...
    status = PsLookupProcessByProcessId(
        (HANDLE)(ULONG_PTR)DebuggerPid, &debuggerProcess);
    if (!NT_SUCCESS(status)) return status;
    status = PsLookupProcessByProcessId(
        (HANDLE)(ULONG_PTR)Config->ProcessId, &targetProcess);
    if (!NT_SUCCESS(status)) {
    ...
    {
        ULONG expectedControlPlane =
            BindFlags & HV_BRIDGE_BIND_EXPECT_DEBUG_OBJECT_MASK;
        if (expectedControlPlane == HV_BRIDGE_BIND_EXPECT_DEBUG_OBJECT_MASK) {
            status = STATUS_INVALID_PARAMETER;
            goto ExitLocked;
        }
        if (expectedControlPlane != 0 &&
            ((expectedControlPlane == HV_BRIDGE_BIND_EXPECT_PRIVATE_DBGK) !=
             HvPrivateDebugObjectIsEnabled())) {
    ...
    if (PsGetProcessExitStatus(debuggerProcess) != STATUS_PENDING ||
        PsGetProcessExitStatus(targetProcess) != STATUS_PENDING) {
        status = STATUS_PROCESS_IS_TERMINATING;
        goto ExitLocked;
    }

    KeAcquireSpinLock(&g_ProtectedDebuggerLock, &oldIrql);
    for (PLIST_ENTRY entry = g_ProtectedDebuggerList.Flink;
         entry != &g_ProtectedDebuggerList;
         entry = entry->Flink)
    {
        PPROTECTED_DEBUGGER debugger =
            CONTAINING_RECORD(entry, PROTECTED_DEBUGGER, ListEntry);
        if (debugger->ProcessId != DebuggerPid) continue;
        if (debugger->ProcessObject != debuggerProcess ||
            debugger->ProcessCreateTime !=
                (UINT64)PsGetProcessCreateTimeQuadPart(debuggerProcess)) {
            status = STATUS_INVALID_CID;
        } else {
            debuggerExisted = TRUE;
            status = STATUS_SUCCESS;
        }
        break;
    }
    KeReleaseSpinLock(&g_ProtectedDebuggerLock, oldIrql);
    if (!NT_SUCCESS(status)) goto ExitLocked;

    if (!debuggerExisted) {
        /* Match Unreal's explicit ownership order: REGISTER only establishes
         * transport; debugger_add must publish the debugger identity before a
         * Bridge target can be bound.  BIND never creates policy implicitly. */
        status = STATUS_NOT_FOUND;
        goto ExitLocked;
    }

    bridgeOwnerExisted =
        g_BridgeAntiDebugEnabled &&
        (g_HookOwnersNtQueryInformationProcess & HV_HOOK_OWNER_BRIDGE) != 0 &&
        (g_HookOwnersNtQueryObject & HV_HOOK_OWNER_BRIDGE) != 0;
    status = HvHookpEnableBridgeAntiDebugLocked(DebuggerPid);
    if (!NT_SUCCESS(status)) goto Rollback;

    status = HvHookpProtectProcessLocked(
        Config, targetProcess, TRUE, BindFlags);
    if (NT_SUCCESS(status) && HvPrivateDebugObjectIsEnabled()) {
        status = HvPrivateDebugObjectBind(
            (HANDLE)(ULONG_PTR)DebuggerPid,
            (HANDLE)(ULONG_PTR)Config->ProcessId);
        if (NT_SUCCESS(status)) {
            privateObjectBound = TRUE;
        }
    }
    if (NT_SUCCESS(status)) goto ExitLocked;

Rollback:
    if (privateObjectBound) {
        (void)HvPrivateDebugObjectUnbind(
            (HANDLE)(ULONG_PTR)DebuggerPid,
            (HANDLE)(ULONG_PTR)Config->ProcessId);
    }
    (void)HvHookpUnbindBridgeTargetLocked(
        Config->ProcessId, DebuggerPid);
    if (!bridgeOwnerExisted) {
        HvHookpDisableBridgeAntiDebugLocked();
    }
...
)
{
    ...
    if (ProcessId == 0 || DebuggerPid == 0) return STATUS_INVALID_PARAMETER;
    if (!g_HvHookInitialized || !g_DebuggerMutationLockInitialized ||
        KeGetCurrentIrql() > APC_LEVEL) {
        return STATUS_INVALID_DEVICE_STATE;
    }
    ...
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    status = HvHookpUnbindBridgeTargetLocked(ProcessId, DebuggerPid);
    if ((NT_SUCCESS(status) || status == STATUS_NOT_FOUND) &&
        !HvHookpHasBridgeBindings()) {
        HvHookpDisableBridgeAntiDebugLocked();
    }
    ExReleasePushLockExclusive(&g_DebuggerMutationLock);
    KeLeaveCriticalRegion();
...
static NTSTATUS
HvHookpEnableDebuggerProxyLocked(_In_ ULONG CallerPid)
{
    NTSTATUS st = STATUS_SUCCESS;

    if (!g_HvHookInitialized) return STATUS_NOT_INITIALIZED;
    if (g_DebuggerProxyEnabled &&
        g_HookNtSetContextThread &&
        (g_HookOwnersNtSetContextThread & HV_HOOK_OWNER_DEBUGGER_PROXY) != 0 &&
        g_HookNtGetContextThread &&
        (g_HookOwnersNtGetContextThread & HV_HOOK_OWNER_DEBUGGER_PROXY) != 0 &&
        g_HookNtReadVirtualMemory &&
        g_HookNtWriteVirtualMemory) {
        return STATUS_SUCCESS;
    }

    // Recover any incomplete prior state before beginning a new transaction.
    // This is the self-heal path for enabled=TRUE with a missing core handle.
    HvHookpDisableDebuggerProxyLocked();

    DbgPrint("[HvHook] Enabling debugger proxy (Nt[Set/Get]ContextThread + Nt[R/W]VirtualMemory), CallerPid=%u\n",
             CallerPid);

    st = HvHookpAcquireSharedHookLocked(
        L"NtSetContextThread",
        CallerPid,
        HookedNtSetContextThread,
        &g_HookNtSetContextThread,
        &g_HookOwnersNtSetContextThread,
        HV_HOOK_OWNER_DEBUGGER_PROXY);
    if (!NT_SUCCESS(st)) goto RollbackCoreHooks;
    ...
    st = HvHookpInstallTrackedHookLocked(
    ...
    if (!g_HookNtSetContextThread ||
        ...
        goto RollbackCoreHooks;
    }

    // Core transaction commits here. Optional protection hooks remain
    // best-effort and cannot turn an operational core into enabled=FALSE.
    ...
    // 2026-06-18: 5 个 R3 保护补强 hook (允许部分失败, 单个失败不影响其它)
    {
        ...
        for (ULONG i = 0; i < RTL_NUMBER_OF(extraEntries); i++) {
            (void)HvHookpInstallTrackedHookLocked(
                extraEntries[i].Name,
                CallerPid,
                extraEntries[i].Hook,
                extraEntries[i].Handle);
        }
    }

    (void)HvHookpAcquireSharedHookLocked(
        L"NtDebugActiveProcess",
        CallerPid,
        HookedNtDebugActiveProcess,
        &g_HookNtDebugActiveProcess,
...
RollbackCoreHooks:
    ...
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtWriteVirtualMemory", &g_HookNtWriteVirtualMemory);
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtReadVirtualMemory", &g_HookNtReadVirtualMemory);
    (void)HvHookpReleaseSharedHookLocked(
        L"NtGetContextThread",
        &g_HookNtGetContextThread,
        &g_HookOwnersNtGetContextThread,
    ...
    DbgPrint("[HvHook] Debugger proxy core hook transaction failed: 0x%X\n", st);
    return st;
}

static VOID HvHookpDisableDebuggerProxyLocked(VOID)
{
    if (!g_HvHookInitialized) return;
    ...
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtReadVirtualMemoryEx", &g_HookNtReadVirtualMemoryEx);
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtCreateSection", &g_HookNtCreateSection);
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtMapViewOfSection", &g_HookNtMapViewOfSection);
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtMapViewOfSectionEx", &g_HookNtMapViewOfSectionEx);
    (void)HvHookpReleaseSharedHookLocked(
        L"NtCreateThreadEx",
        &g_HookNtCreateThreadEx,
        &g_HookOwnersNtCreateThreadEx,
    ...
    // P106 调试器线程操作 proxy
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtSuspendThread", &g_HookNtSuspendThread);
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtResumeThread", &g_HookNtResumeThread);
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtOpenThread", &g_HookNtOpenThread);
    ...
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtQueryVirtualMemory", &g_HookNtQueryVirtualMemory);
    // 2026-06-17: win32k window-enum spoofs
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtUserQueryWindow", &g_HookNtUserQueryWindow);
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtUserInternalGetWindowText", &g_HookNtUserInternalGetWindowText);
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtUserGetClassName", &g_HookNtUserGetClassName);
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtUserFindWindowEx", &g_HookNtUserFindWindowEx);
    // 2026-06-19: NtUserBuildHwndList hook (hwnd 列表过滤)
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtUserBuildHwndList", &g_HookNtUserBuildHwndList);
    // 2026-06-19: NtUserWindowFromPoint 族 (Spy++ 拖拽防护)
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtUserWindowFromPoint", &g_HookNtUserWindowFromPoint);
    (void)HvHookpRemoveTrackedHookLocked(
        L"NtUserChildWindowFromPointEx", &g_HookNtUserChildWindowFromPointEx);
}
...
static BOOLEAN
HvHookpDebuggerHideSharedHooksPublished(VOID)
{
    return g_DebuggerProxyEnabled &&
           g_AccessBypassEnabled &&
           g_HookNtQuerySystemInformation != NULL &&
           (g_HookOwnersNtQuerySystemInformation &
            HV_HOOK_OWNER_DEBUGGER_PROXY) != 0 &&
           g_HookNtOpenProcess != NULL &&
           g_HookNtGetNextProcess != NULL;
}

static BOOLEAN
HvHookpAnySharedHideHookPublished(VOID)
{
    return g_HookNtQuerySystemInformation != NULL ||
           g_HookNtOpenProcess != NULL ||
           g_HookNtGetNextProcess != NULL;
}

/* Legacy standalone EPT/NPT hiding still owns private auxiliary hooks when
 * no common hook is published. Keep it for compatibility; folding that
 * lifetime into the shared owner table remains explicit owner debt. */
...
// Preserve the former build-specific EPROCESS strategy as a compatibility
// fallback. It is intentionally not the primary path because the offsets are
// version dependent. A later build can enable it after validating the target
// Windows layout; the structured cleanup guarantees restoration on every exit.
#ifndef HV_ENABLE_LEGACY_EPROCESS_ACCESS_FALLBACK
#define HV_ENABLE_LEGACY_EPROCESS_ACCESS_FALLBACK 0
#endif

#if HV_ENABLE_LEGACY_EPROCESS_ACCESS_FALLBACK
#define HV_LEGACY_EPROCESS_PROTECTION_OFFSET 0x87A
#define HV_LEGACY_EPROCESS_TOKEN_OFFSET      0x4B8

static KMUTEX  g_LegacyAccessMutex;
static BOOLEAN g_LegacyAccessMutexInitialized = FALSE;

static NTSTATUS
HvHookpLegacyOpenProcess(
    _In_ PFN_NtOpenProcess Original,
    ...
    _In_ PCLIENT_ID ClientId)
{
    ...
    ULONG_PTR savedToken = 0;
    UCHAR savedProtection = 0;
    BOOLEAN tokenChanged = FALSE;
    BOOLEAN protectionChanged = FALSE;

    if (!g_LegacyAccessMutexInitialized) {
        KeInitializeMutex(&g_LegacyAccessMutex, 0);
        g_LegacyAccessMutexInitialized = TRUE;
    }

    KeWaitForSingleObject(&g_LegacyAccessMutex, Executive, KernelMode,
                          FALSE, NULL);
    __try {
        __try {
            PUCHAR protection = (PUCHAR)Target +
                HV_LEGACY_EPROCESS_PROTECTION_OFFSET;
            savedProtection = *protection;
            *protection = 0;
            protectionChanged = TRUE;

            if (callerProcess &&
                NT_SUCCESS(PsLookupProcessByProcessId(
                    (HANDLE)(ULONG_PTR)4, &systemProcess)) &&
                systemProcess) {
                PULONG_PTR callerToken = (PULONG_PTR)(
                    (PUCHAR)callerProcess + HV_LEGACY_EPROCESS_TOKEN_OFFSET);
                PULONG_PTR systemToken = (PULONG_PTR)(
                    (PUCHAR)systemProcess + HV_LEGACY_EPROCESS_TOKEN_OFFSET);
                savedToken = *callerToken;
                *callerToken = (*systemToken & ~0xFULL) |
                               (savedToken & 0xFULL);
                tokenChanged = TRUE;
            }

            status = Original(ProcessHandle, DesiredAccess,
        ...
        __finally {
            if (tokenChanged) {
                PULONG_PTR callerToken = (PULONG_PTR)(
                    (PUCHAR)callerProcess + HV_LEGACY_EPROCESS_TOKEN_OFFSET);
                *callerToken = savedToken;
            }
            if (protectionChanged) {
                *((PUCHAR)Target + HV_LEGACY_EPROCESS_PROTECTION_OFFSET) =
                    savedProtection;
            }
            if (systemProcess) {
                ObDereferenceObject(systemProcess);
            }
            KeReleaseMutex(&g_LegacyAccessMutex, FALSE);
        }
    }
    __except (EXCEPTION_EXECUTE_HANDLER) {
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtOpenProcess, STATUS_DELETE_PENDING)
    ...
    /* Preserve the native self-open contract.  A debugger frequently opens
     * itself while its plugin/CLR loader is still initializing.  Routing that
     * request through the privileged object-pointer path changes observable
     * handle semantics and contradicts the protection policy below, which is
     * intended only for cross-process access. */
    ...
    // 这里命中也等同于反向保护。
    if (targetPidFromCid && caller != targetPidFromCid &&
        HvHookIsProcessHidden((ULONG)(ULONG_PTR)targetPidFromCid))
    {
    ...
    // 不再临时改 EPROCESS.Protection/Token。旧路径依赖跨版本硬编码偏移，
    // 还会让 caller 的其它线程在替换窗口观察到 SYSTEM token。
    if (prevMode != UserMode) {
        return Original(ProcessHandle, DesiredAccess, ObjectAttributes, ClientId);
    }
    ...
    NTSTATUS openSt = ObOpenObjectByPointer(
        target,
        handleAttributes,
        NULL,
        DesiredAccess,
    ...
    if (!NT_SUCCESS(openSt)) {
        openSt = HvHookpLegacyOpenProcess(
            Original,
            target,
            ProcessHandle,
            DesiredAccess,
    ...
    if (!NT_SUCCESS(openSt)) {
        ...
        HvDbgEvtPost(HV_DBGEVT_SEV_ERROR, HV_DBGEVT_CAT_OPEN_PROCESS, openSt,
                     ...
                     (ULONG)(ULONG_PTR)targetPidFromCid,
                     DesiredAccess, 0,
                     "NtOpenProcess: object-pointer open failed");
        return openSt;
    }
    ...
    if (localHandle) {
        __try {
            ProbeForWrite(ProcessHandle, sizeof(HANDLE), sizeof(HANDLE));
            *ProcessHandle = localHandle;
        } __except (EXCEPTION_EXECUTE_HANDLER) {
            NTSTATUS copySt = GetExceptionCode();
            ZwClose(localHandle);
            return copySt;
        }
    }
    ...
    HvDbgEvtPost(HV_DBGEVT_SEV_INFO, HV_DBGEVT_CAT_OPEN_PROCESS, STATUS_SUCCESS,
                 ...
                 (ULONG)(ULONG_PTR)targetPidFromCid,
                 DesiredAccess, 0,
                 "NtOpenProcess bypass OK (object-pointer user handle)");
    return STATUS_SUCCESS;
    HV_HOOK_CALLBACK_END()
}
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtGetNextProcess, STATUS_DELETE_PENDING)
    ...
    /* The shared owner must cover both backend hidden state and debugger
     * records. Keep the returned hidden handle only long enough to use it as
     * the next enumeration anchor, then close it after Original consumed it. */
    ULONG skipCount = 0;
    while (NT_SUCCESS(st) && localOut) {
        HANDLE outPid = HvHookpProcessHandleToProcessId(localOut);
        BOOLEAN hidden = outPid
            ? HvHookIsProcessHidden((ULONG)(ULONG_PTR)outPid)
            : FALSE;
        BOOLEAN debugger = outPid ? HvHookIsDebuggerPid(outPid) : FALSE;
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtDuplicateObject, STATUS_DELETE_PENDING)
    ...
    // VMP 可用 DUPLICATE_CLOSE_SOURCE 绕过 NtClose，关闭带
    // OBJ_PROTECT_CLOSE 的句柄并观察异常。source handle 属于当前调用者时，
    // 预查属性并剥掉 CLOSE_SOURCE，duplicate 本身仍照常执行。
    if ((Options & 0x1UL) != 0 &&
        g_AntiAntiDebugConfig.HookNtClose &&
        HvHookpIsAntiAntiDebugCaller()) {
        BOOLEAN sourceIsCaller =
            SourceProcessHandle == (HANDLE)(LONG_PTR)-1;
        if (!sourceIsCaller && SourceProcessHandle) {
            sourceIsCaller =
                HvHookpProcessHandleToProcessId(SourceProcessHandle) == caller;
        }

        if (sourceIsCaller) {
            PVOID sourceObject = NULL;
            OBJECT_HANDLE_INFORMATION sourceInfo = { 0 };
            NTSTATUS sourceStatus = ObReferenceObjectByHandle(
                SourceHandle,
                0,
                NULL,
                ExGetPreviousMode(),
            ...
            if (NT_SUCCESS(sourceStatus)) {
                ...
                if (sourceInfo.HandleAttributes & 0x1UL) {
                    Options &= ~0x1UL;
                    HV_HOT_DBG("[HvHook-AAD] NtDuplicateObject CLOSE_SOURCE stripped\n");
                }
            }
        }
    }
...
/* Protected by g_DebuggerMutationLock.  The callback is shared by debugger
 * reverse protection and the optional positive AccessBypass policy. */
static LONG g_ObDebuggerProtectOwnerCount = 0;
static BOOLEAN g_ObAccessBypassOwner = FALSE;
    ...
    _In_ POB_PRE_OPERATION_INFORMATION Info)
{
    ...
    if (targetPid && HvHookIsDebuggerPid(targetPid)) {
        ...
        if (HvHookpShouldProtectDebuggerPid(targetPid) &&
            caller != targetPid && !HvHookIsDebuggerPid(caller) &&
            !HvHookIsTrustedSystemCaller(caller))
        {
            ULONG orig = *access;
    ...
    // 正向恢复只服务已注册 debugger，且最多恢复它原本请求的 access。
    if (HvHookIsAccessBypassEnabled() &&
        caller && HvHookIsDebuggerPid(caller) &&
        targetPid && caller != targetPid) {
        ACCESS_MASK before = *access;
        *access |= originalAccess;
        if (before != *access) {
            HV_HOT_DBG("[HvHook-Restore] caller=%u target=%u op=%u %s "
                       "before=0x%X requested=0x%X restored=0x%X\n",
                       (ULONG)(ULONG_PTR)caller,
                       (ULONG)(ULONG_PTR)targetPid,
                       Info->Operation,
                       isProcess ? "PROC" : "THREAD",
                       before,
                       originalAccess,
                       *access);
        }
...
static NTSTATUS HvHookpAcquireDebuggerProtectOwnerLocked(VOID)
{
    ...
    if (g_ObDebuggerProtectOwnerCount == MAXLONG) {
        return STATUS_INTEGER_OVERFLOW;
    }
    status = HvHookpRegisterObCallback();
    if (NT_SUCCESS(status)) {
        g_ObDebuggerProtectOwnerCount++;
    }
    return status;
}
...
static VOID HvHookpReleaseDebuggerProtectOwnerLocked(VOID)
{
    if (g_ObDebuggerProtectOwnerCount > 0) {
        g_ObDebuggerProtectOwnerCount--;
    }
    if (g_ObDebuggerProtectOwnerCount == 0 &&
        !g_ObAccessBypassOwner) {
        HvHookpUnregisterObCallback();
    }
}
...
static NTSTATUS HvHookpAcquireAccessBypassObOwnerLocked(VOID)
{
    ...
    if (g_ObAccessBypassOwner) {
        return STATUS_SUCCESS;
    }
    status = HvHookpRegisterObCallback();
    if (NT_SUCCESS(status)) {
        g_ObAccessBypassOwner = TRUE;
    }
    return status;
}
...
static VOID HvHookpUnregisterObCallback(VOID)
{
    ...
    g_ObDebuggerProtectOwnerCount = 0;
    g_ObAccessBypassOwner = FALSE;
}
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtCreateFile, STATUS_DELETE_PENDING)
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtOpenFile, STATUS_DELETE_PENDING)
...
{
    HV_HOOK_CALLBACK_BEGIN(g_HookNtReadFile, STATUS_DELETE_PENDING)
...
static NTSTATUS HvHookpEnableAccessBypassLocked(VOID)
{
    PVOID    t;
    NTSTATUS st;
    ...
    st = HvHookpAcquireAccessBypassObOwnerLocked();
    if (!NT_SUCCESS(st)) {
        return st;
    }

    DbgPrint("[HvHook] Enabling NtOpenProcess access bypass (object-pointer handle path)\n");
    ...
    if (!t) {
        ...
        HvHookpReleaseAccessBypassObOwnerLocked();
        return STATUS_NOT_FOUND;
    }
    ...
    // 2026-06-19: debugger image 文件防护. Create/Open 是主防线；ReadFile
    // 兜底保留在独立 gate 后，避免同步读取无限期持有全局 unload rundown。
...
#if HV_ENABLE_BLOCKING_NTREADFILE_IMAGE_FALLBACK
...
#if !HV_ENABLE_BLOCKING_NTREADFILE_IMAGE_FALLBACK
    DbgPrint("[HvHook] NtReadFile image fallback retained but not published; "
             "synchronous reads cannot hold unload rundown\n");
#endif
    ...
    if (!g_HookNtDuplicateObject) {
        ...
        if (dupT) {
            NTSTATUS stDup = HvHookInstall(
                dupT, HookedNtDuplicateObject, &g_HookNtDuplicateObject);
            if (NT_SUCCESS(stDup)) {
...
static NTSTATUS HvHookpDisableAccessBypassLocked(VOID)
{
    NTSTATUS finalStatus = STATUS_SUCCESS;
    NTSTATUS status;
    ...
    status = HvHookpRemoveTrackedHookLocked(
        L"NtOpenProcess", &g_HookNtOpenProcess);
    if (!NT_SUCCESS(status)) finalStatus = status;
    status = HvHookpRemoveTrackedHookLocked(
        L"NtGetNextProcess", &g_HookNtGetNextProcess);
    if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;

    // 2026-06-19: 卸 debugger image 文件防护
    status = HvHookpRemoveTrackedHookLocked(
        L"NtCreateFile", &g_HookNtCreateFile);
    if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;
    status = HvHookpRemoveTrackedHookLocked(
        L"NtOpenFile", &g_HookNtOpenFile);
    if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;
    status = HvHookpRemoveTrackedHookLocked(
        L"NtReadFile", &g_HookNtReadFile);
    if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;

    // 2026-06-19: 卸 NtDuplicateObject
    if (g_HookNtDuplicateObject &&
        !(g_AntiAntiDebugEnabled && g_AntiAntiDebugConfig.HookNtClose)) {
        status = HvHookpRemoveTrackedHookLocked(
            L"NtDuplicateObject", &g_HookNtDuplicateObject);
        if (!NT_SUCCESS(status) && NT_SUCCESS(finalStatus)) finalStatus = status;
    }
    ...
    if (!NT_SUCCESS(finalStatus)) {
        DbgPrint("[HvHook] Access bypass disable retained hook ownership: 0x%X\n",
                 finalStatus);
        return finalStatus;
    }
...
NTSTATUS HvHookDisableAccessBypass(VOID)
{
    ...
    KeEnterCriticalRegion();
    ExAcquirePushLockExclusive(&g_DebuggerMutationLock);
    status = HvHookpDisableAccessBypassLocked();
    ExReleasePushLockExclusive(&g_DebuggerMutationLock);
    KeLeaveCriticalRegion();
    return status;
}
...
HvHookIsDebuggerProxyEnabled(VOID)
{
    ...
    // 是否真的非 NULL。Context 与 R/W 核心 hook 都装上才算 enabled。
    return g_DebuggerProxyEnabled
...
#define HV_DSE_CI_CALLBACK_ANCHOR_SIZE       33u
#define HV_DSE_IMAGE_SCN_MEM_DISCARDABLE     0x02000000UL
#define HV_DSE_IMAGE_SCN_MEM_EXECUTE         0x20000000UL

static BOOLEAN
HvDsepAsciiEqualsInsensitive(
    _In_reads_(LeftCapacity) PCSTR Left,
    _In_ SIZE_T LeftCapacity,
    _In_z_ PCSTR Right
)
{
    SIZE_T index;

    if (!Left || !Right || LeftCapacity == 0) {
        return FALSE;
    }

    for (index = 0; index < LeftCapacity; index++) {
...
static NTSTATUS
HvDsepQueryLoadedModule(
    _In_z_ PCSTR ModuleName,
    _Out_ PVOID* OutImageBase,
    _Out_ PULONG OutImageSize
)
{
    const ULONG poolTag = 'ICsD';
    NTSTATUS status;
    ULONG required = 0;
    ULONG bufferSize;
    ULONG returned = 0;
    ...
    if (!ModuleName || !OutImageBase || !OutImageSize) {
        return STATUS_INVALID_PARAMETER;
    }
    *OutImageBase = NULL;
    *OutImageSize = 0;

    status = ZwQuerySystemInformation(11, NULL, 0, &required);
    if (required == 0) {
        return NT_SUCCESS(status) ? STATUS_NOT_FOUND : status;
    }
    if (required > MAXULONG - 0x1000) {
        return STATUS_INTEGER_OVERFLOW;
    }

    bufferSize = required + 0x1000;
    buffer = HvAllocateNonPagedZeroed(bufferSize, poolTag);
    if (!buffer) {
        return STATUS_INSUFFICIENT_RESOURCES;
    }

    status = ZwQuerySystemInformation(
        11, buffer, bufferSize, &returned);
    if (!NT_SUCCESS(status)) {
        ExFreePoolWithTag(buffer, poolTag);
        return status;
    }

    if (returned == 0 || returned > bufferSize) {
        returned = bufferSize;
    }
    if (returned < (ULONG)FIELD_OFFSET(HV_RTL_PROCESS_MODULES, Modules)) {
        ExFreePoolWithTag(buffer, poolTag);
        return STATUS_INFO_LENGTH_MISMATCH;
    }

    modules = (PHV_RTL_PROCESS_MODULES)buffer;
    maxModules = (returned - (ULONG)FIELD_OFFSET(HV_RTL_PROCESS_MODULES, Modules)) /
                 sizeof(HV_RTL_PROCESS_MODULE_INFORMATION);
    if (modules->NumberOfModules > maxModules) {
        ExFreePoolWithTag(buffer, poolTag);
        return STATUS_INFO_LENGTH_MISMATCH;
    }
    ...
    for (index = 0; index < modules->NumberOfModules; index++) {
        PHV_RTL_PROCESS_MODULE_INFORMATION module = &modules->Modules[index];
        ULONG fileNameOffset = module->OffsetToFileName;

        if (fileNameOffset >= (ULONG)sizeof(module->FullPathName)) {
            continue;
        }
        if (!HvDsepAsciiEqualsInsensitive(
                (PCSTR)module->FullPathName + fileNameOffset,
                (SIZE_T)sizeof(module->FullPathName) - fileNameOffset,
                ModuleName)) {
            continue;
        }

        *OutImageBase = module->ImageBase;
        *OutImageSize = module->ImageSize;
        status = STATUS_SUCCESS;
        break;
    }
...
static BOOLEAN
HvDsepRangeIsExecutable(
    _In_ PUCHAR ImageBase,
    _In_ ULONG ImageSize,
    _In_reads_(SectionCount) PHV_IMAGE_SECTION_HEADER Sections,
...
)
{
    ...
    ULONG_PTR imageValue;
    ULONG_PTR rva;
    USHORT index;

    if (!ImageBase || !Sections || !Address || Length == 0) {
        return FALSE;
    }

    imageValue = (ULONG_PTR)ImageBase;
    addressValue = (ULONG_PTR)Address;
    if (addressValue < imageValue) {
        return FALSE;
    }
    rva = addressValue - imageValue;
    if (rva >= ImageSize || Length > (SIZE_T)(ImageSize - rva)) {
        return FALSE;
    }

    for (index = 0; index < SectionCount; index++) {
        ULONG sectionRva = Sections[index].VirtualAddress;
        ULONG sectionSize = Sections[index].Misc.VirtualSize;
        ULONG_PTR sectionOffset;

        if ((Sections[index].Characteristics & HV_DSE_IMAGE_SCN_MEM_EXECUTE) == 0 ||
            (Sections[index].Characteristics & HV_DSE_IMAGE_SCN_MEM_DISCARDABLE) != 0) {
            continue;
        }
        if (sectionSize == 0) {
            sectionSize = Sections[index].SizeOfRawData;
        }
        ...
        sectionOffset = rva - sectionRva;
        if (sectionOffset <= sectionSize &&
            Length <= (SIZE_T)(sectionSize - sectionOffset)) {
            return TRUE;
        }
    }
...
static BOOLEAN
HvDsepMatchesCiCallbackAnchor(
    _In_reads_(HV_DSE_CI_CALLBACK_ANCHOR_SIZE) const UCHAR* Code
)
{
    UCHAR storeModRm;

    if (Code[0] != 0x48 || Code[1] != 0x8D || Code[2] != 0x05 ||
        Code[7] != 0x48 || Code[8] != 0x89 || Code[10] != 0x20 ||
        Code[11] != 0x48 || Code[12] != 0x8D || Code[13] != 0x05 ||
        Code[18] != 0x48 || Code[19] != 0x89 || Code[21] != 0x28 ||
        Code[22] != 0x48 || Code[23] != 0x8D || Code[24] != 0x05 ||
        Code[29] != 0x48 || Code[30] != 0x89 || Code[32] != 0x18) {
        return FALSE;
    }

    storeModRm = Code[9];
    if ((storeModRm & 0xF8) != 0x40 || (storeModRm & 0x07) == 0x04) {
        return FALSE;
    }

    return Code[20] == storeModRm && Code[31] == storeModRm;
}

static PUCHAR
HvDsepResolveRipRelativeTarget(
    _In_reads_(7) const UCHAR* Instruction
)
{
    LONG displacement;

    RtlCopyMemory(&displacement, Instruction + 3, sizeof(displacement));
    return (PUCHAR)((LONG_PTR)(Instruction + 7) + displacement);
}

static NTSTATUS
HvDsepResolveCiValidateImageHeader(
    _Out_ PVOID* OutTarget
)
{
    NTSTATUS status;
    PVOID imageBaseValue;
    ULONG imageSize;
    PUCHAR imageBase;
    PHV_IMAGE_DOS_HEADER dosHeader;
    PHV_IMAGE_NT_HEADERS64 ntHeaders;
    PHV_IMAGE_SECTION_HEADER sections;
    ULONG ntOffset;
    ...
    *OutTarget = NULL;
    ...
    status = HvDsepQueryLoadedModule(
        "ci.dll", &imageBaseValue, &imageSize);
    if (!NT_SUCCESS(status)) {
        return status;
    }

    imageBase = (PUCHAR)imageBaseValue;
    __try {
        if (imageSize < sizeof(HV_IMAGE_DOS_HEADER)) {
            return STATUS_INVALID_IMAGE_FORMAT;
        }

        dosHeader = (PHV_IMAGE_DOS_HEADER)imageBase;
        if (dosHeader->e_magic != HV_IMAGE_DOS_SIGNATURE ||
        ...
        if (ntOffset > imageSize -
            (ULONG)FIELD_OFFSET(HV_IMAGE_NT_HEADERS64, OptionalHeader)) {
            return STATUS_INVALID_IMAGE_FORMAT;
        }

        ntHeaders = (PHV_IMAGE_NT_HEADERS64)(imageBase + ntOffset);
        if (ntHeaders->Signature != HV_IMAGE_NT_SIGNATURE ||
            ntHeaders->OptionalHeader.Magic != HV_IMAGE_NT_OPTIONAL_HDR64_MAGIC ||
        ...
        sectionTableSize =
            ...
            sizeof(HV_IMAGE_SECTION_HEADER);
        if (sectionTableOffset > imageSize ||
            sectionTableSize > imageSize - sectionTableOffset) {
            return STATUS_INVALID_IMAGE_FORMAT;
        }

        sections = (PHV_IMAGE_SECTION_HEADER)(imageBase + sectionTableOffset);
        for (sectionIndex = 0;
             sectionIndex < ntHeaders->FileHeader.NumberOfSections;
             sectionIndex++) {
            ULONG sectionRva = sections[sectionIndex].VirtualAddress;
            ULONG sectionSize = sections[sectionIndex].Misc.VirtualSize;
            ULONG offset;

            if ((sections[sectionIndex].Characteristics &
                 HV_DSE_IMAGE_SCN_MEM_EXECUTE) == 0 ||
                (sections[sectionIndex].Characteristics &
                 HV_DSE_IMAGE_SCN_MEM_DISCARDABLE) != 0) {
                continue;
            }
            if (sectionSize == 0) {
                sectionSize = sections[sectionIndex].SizeOfRawData;
            }
            if (sectionRva >= imageSize || sectionSize > imageSize - sectionRva ||
                sectionSize < HV_DSE_CI_CALLBACK_ANCHOR_SIZE) {
                continue;
            }

            for (offset = 0;
                 offset <= sectionSize - HV_DSE_CI_CALLBACK_ANCHOR_SIZE;
                 offset++) {
                PUCHAR anchor = imageBase + sectionRva + offset;
                PUCHAR validateHeader;
                PUCHAR validateData;
                PUCHAR queryInformation;

                if (!HvDsepMatchesCiCallbackAnchor(anchor)) {
                    continue;
                }

                validateHeader = HvDsepResolveRipRelativeTarget(anchor);
                validateData = HvDsepResolveRipRelativeTarget(anchor + 11);
                queryInformation = HvDsepResolveRipRelativeTarget(anchor + 22);
                if (!HvDsepRangeIsExecutable(
                        imageBase, imageSize, sections,
                        ntHeaders->FileHeader.NumberOfSections,
                        validateHeader, 32) ||
                    !HvDsepRangeIsExecutable(
                        imageBase, imageSize, sections,
                    ...
                    !HvDsepRangeIsExecutable(
                        ...
                        ntHeaders->FileHeader.NumberOfSections,
                        queryInformation, 16) ||
                    validateHeader == validateData ||
                    validateHeader == queryInformation ||
                    validateData == queryInformation) {
                    continue;
                }

                match = validateHeader;
                matchCount++;
                if (matchCount > 1) {
    ...
    *OutTarget = match;
    DbgPrint("[HvHook-DSE] Resolved private ci.dll!CiValidateImageHeader @ %p "
             "from callback publication anchor (base=%p size=0x%X)\n",
             match, imageBaseValue, imageSize);
    return STATUS_SUCCESS;
}
...
{
    HV_HOOK_CALLBACK_BEGIN(g_CiValidateImageHeaderHook, STATUS_DELETE_PENDING)
...
HvDseDisable(VOID)
{
    ...
    PVOID target = NULL;
    NTSTATUS status;
    NTSTATUS resolveStatus = STATUS_NOT_FOUND;
    HV_HOOK_HANDLE handle = NULL;
    ...
    if (!target) {
        resolveStatus = HvDsepResolveCiValidateImageHeader(&target);
        if (!NT_SUCCESS(resolveStatus)) {
            DbgPrint("[HvHook-DSE] Private CiValidateImageHeader resolver failed: 0x%X\n",
                     resolveStatus);
        }
    }
    if (!target) {
...
HvDseEnable(VOID)
{
    ...
    if (g_CiValidateImageHeaderHook) {
        ...
        if (!NT_SUCCESS(status)) {
            DbgPrint("[HvHook-DSE] HvHookRemove failed: 0x%X; ownership retained for retry\n",
                     status);
            return status;
        }
