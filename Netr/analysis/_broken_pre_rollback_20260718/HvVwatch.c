    ...
    _In_opt_ HANDLE ScopeThreadId)
{
    ...
    candidate.DirectMainEpt = ScopeThreadId ? TRUE : FALSE;
    candidate.MirrorOverlayEpt = ScopeThreadId ? TRUE : FALSE;
    ...
    _In_ BOOLEAN PrivateEvent)
{
    ...
    for (LONG index = (LONG)SlotIndex;
        ...
        if (!candidate->InUse && entryIdx < 0) entryIdx = index;
    }
    if (entryIdx < 0) {
        status = STATUS_INSUFFICIENT_RESOURCES;
        goto ExitLocked;
...
{
    return HvVwatchpClearOwnedInternal(NULL, TargetPid, SlotIndex);
}
    ...
    _In_ ULONG SlotIndex)
{
    ...
    return HvVwatchpClearOwnedInternal(DebuggerPid, TargetPid, SlotIndex);
}
    ...
    _Out_ PHANDLE HitTargetPid)
{
    ...
    *HitTargetPid = NULL;
    ...
    for (LONG i = 0; i < HV_VWATCH_MAX_ENTRIES; i++) {
        ...
        if (Gla < watchVa) continue;
        if (Gla >= watchVa + length) continue;
    ...
    _Inout_ PGUEST_CONTEXT Ctx)
{
    ...
    if (reason == HV_VWATCH_MTF_HIT) {
        ...
        if (hitPrivateEvent && hitDebuggerPid && hitTargetPid) {
            ...
            // External Windows debuggers own a DebugObject, so preserve their
            // standard #DB delivery.  Built-in private delivery never injects
            // an unhandled exception into the target.
            __vmx_vmwrite(
