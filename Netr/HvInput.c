...
BOOLEAN HvInputHandleIoExit(SIZE_T qualification, PGUEST_CONTEXT ctx)
{
    ...
    if (g_Backend != HV_INPUT_BACKEND_PS2) return FALSE;
    ...
    if (direction) {
        ...
        if (port == 0x60) {
            ...
            if (g_InputEnabled && g_NextByteSource == INPUT_SOURCE_MOUSE) {
            ...
            // 状态端口: 计算合成 status
            BOOLEAN kbdHas = !HvInputFifoIsEmpty(&g_KbdFifo);
            BOOLEAN mouseHas = !HvInputFifoIsEmpty(&g_MouseFifo);

            if (g_InputEnabled && (kbdHas || mouseHas)) {
