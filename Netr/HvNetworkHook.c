...
// 是否启用伪造
static volatile BOOLEAN g_FakeEnabled = TRUE;
...
HvNetHookInitialize(VOID)
{
    ...
    if (!HvHookIsInitialized()) {
        DbgPrint("[HvNetHook] HvHook not available, running in config-only mode\n");
    ...
    return g_HvNetHookManager.HookInstalled ? STATUS_SUCCESS : STATUS_UNSUCCESSFUL;
}
...
HvNetHookCleanup(VOID)
{
    ...
    if (g_NtDeviceIoControlFileHookHandle != NULL) {
        HvHookRemove(g_NtDeviceIoControlFileHookHandle);
...
{
    InterlockedExchange((PLONG)&g_FakeEnabled, FALSE);
...
{
    InterlockedExchange((PLONG)&g_FakeEnabled, TRUE);
