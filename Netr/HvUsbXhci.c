    ...
    _Out_ PSIZE_T  OutSize)
{
    ...
    *OutSize = 0;
    ...
    ULONG saveLo = bar0Lo, saveHi = bar0Hi;
    ULONG all1 = 0xFFFFFFFF;

    HvXhciCfgWrite(bus, dev, fn, PCI_BAR0_OFFSET, &all1, 4);
    HvXhciCfgRead (bus, dev, fn, PCI_BAR0_OFFSET, &sizeLo, 4);
    HvXhciCfgWrite(bus, dev, fn, PCI_BAR0_OFFSET, &saveLo, 4);

    if (bar64) {
        HvXhciCfgWrite(bus, dev, fn, PCI_BAR0_OFFSET + 4, &all1, 4);
        HvXhciCfgRead (bus, dev, fn, PCI_BAR0_OFFSET + 4, &sizeHi, 4);
        HvXhciCfgWrite(bus, dev, fn, PCI_BAR0_OFFSET + 4, &saveHi, 4);
    }

    // mask 高位 flag, 取 size
    ULONG sizeLoMasked = sizeLo & 0xFFFFFFF0;
    SIZE_T size;
    if (bar64) {
        ULONG64 sizeFull = ((ULONG64)sizeHi << 32) | sizeLoMasked;
        size = (SIZE_T)((~sizeFull) + 1);
    } else {
        size = (SIZE_T)((~sizeLoMasked) + 1);
    }
    ...
    *OutPhys = phys;
    *OutSize = size;
    XHCI_LOG("BAR0 phys=0x%llX size=0x%llX (%s)",
             phys, (ULONG64)size, bar64 ? "64-bit" : "32-bit");
    return STATUS_SUCCESS;
}
...
{
    if (!HvUsbXhciIsReady()) return;
    if (g_HvUsbXhci.MsiVector == 0) return;
