 ...
 * 读取 L1 EPT 条目（通过物理地址）
 */
static BOOLEAN NestedEptReadL1Entry(ULONG64 EntryPhysical, PULONG64 Entry)
{
    PVOID mapped;
    PHYSICAL_ADDRESS pa;
    
    pa.QuadPart = EntryPhysical & ~0xFFFULL;
    mapped = MmMapIoSpace(pa, PAGE_SIZE, MmNonCached);
    
    if (!mapped) {
        return FALSE;
    }
    
    ULONG offset = (ULONG)(EntryPhysical & 0xFFF);
    *Entry = *(PULONG64)((PUCHAR)mapped + offset);
...
NTSTATUS NestedEptInitialize(VOID)
{
    ...
    DbgPrint("[HV-NESTED-EPT] Nested EPT manager initialized\n");
...
PNESTED_EPT_CONTEXT NestedEptCreateContext(ULONG64 L1Eptp)
{
    KIRQL oldIrql;
...
PNESTED_EPT_CONTEXT NestedEptFindContext(ULONG64 L1Eptp)
{
    KIRQL oldIrql;
...
PNESTED_EPT_CONTEXT NestedEptGetOrCreateContext(ULONG64 L1Eptp)
{
    PNESTED_EPT_CONTEXT context;
    
    context = NestedEptFindContext(L1Eptp);
    if (context) {
        InterlockedIncrement(&context->RefCount);
        return context;
    }
    
    return NestedEptCreateContext(L1Eptp);
}
    ...
    PULONG PageSize)
{
    ...
    // 读取 PML4E
    if (!NestedEptReadL1Entry(pml4Pa + pml4Index * 8, &pml4e)) {
        return FALSE;
    }
    ...
    // 读取 PDPTE
    pdptPa = (pml4e >> 12) << 12;
    if (!NestedEptReadL1Entry(pdptPa + pdptIndex * 8, &pdpte)) {
        return FALSE;
    }
    ...
    // 读取 PDE
    pdPa = (pdpte >> 12) << 12;
    if (!NestedEptReadL1Entry(pdPa + pdIndex * 8, &pde)) {
        return FALSE;
    }
    ...
    // 读取 PTE
    ptPa = (pde >> 12) << 12;
    if (!NestedEptReadL1Entry(ptPa + ptIndex * 8, &pte)) {
        return FALSE;
    }
    ...
    PULONG64 Permissions)
{
    ...
    // 查找 PTE（需要遍历分割的页表）
    // 对于简单情况，假设大部分使用 2MB 大页
    // 如果需要 4KB 页，需要在 SplitPt 中查找
    ...
    PNESTED_EPT_TRANSLATION Result)
{
    ...
    // 构建 EPT02 映射
    NestedEptBuildMapping(Context, L2Gpa & ~(l1PageSize - 1), 
                          hpa & ~(l1PageSize - 1),
                          Result->MergedPermissions, Result->MemoryType, l1PageSize);
...
static PVOID NestedEptAllocateTablePageLocked(PNESTED_EPT_CONTEXT Context, PPHYSICAL_ADDRESS PhysicalAddress)
{
    ...
    page = NestedEptAllocatePage(PhysicalAddress);
    if (!page) {
        return NULL;
    }
...
cleanup:
    // 取消映射
    if (pt) MmUnmapIoSpace(pt, PAGE_SIZE);
    if (pd) MmUnmapIoSpace(pd, PAGE_SIZE);
    if (pdpt) MmUnmapIoSpace(pdpt, PAGE_SIZE);
    ...
    PBOOLEAN InjectToL1)
{
    ...
    *InjectToL1 = FALSE;
    ...
    // 获取嵌套 EPT 上下文
    context = NestedEptGetOrCreateContext(VcpuData->NestedVmx.L1Eptp);
    if (!context) {
        *InjectToL1 = TRUE;
        return TRUE;
    }
