...
/**
 * Anthropic 协议要求同一轮 tool_use 对应的所有 tool_result 合并到同一个 user 消息.
 * 如果历史里有旧版(每个 tool_result 一条 user 消息), 这里合并连续的 user(tool_result) 消息.
 * 同时去重 tool_use_id (同 id 出现两次会触发 "each tool_use must have a single result").
 */
function sanitizeForAnthropic(history: ChatMsg[], provider: string): ChatMsg[] {
  if (provider !== "anthropic") return history;
  const out: ChatMsg[] = [];
  let buf: Array<{ type: string; tool_use_id?: string; content?: unknown }> = [];
  const seenIds = new Set<string>();
  const flush = () => {
    if (buf.length === 0) return;
    out.push({ role: "user", content: buf });
    buf = [];
    seenIds.clear();
  };
  for (const m of history) {
    const isToolResultUser =
      ...
      (m.content as Array<{ type?: string }>).every((b) => b?.type === "tool_result");
    if (isToolResultUser) {
      for (const b of m.content as Array<{ type: string; tool_use_id?: string; content?: unknown }>) {
...
export function AiAssistantPanel() {
  const { pid, processName } = useSession();
  ...
  async function pickFilesViaDialog() {
    // 复用 project_pick_open 那种, 这里没专用 dialog command, 走 input file
    const inp = document.createElement("input");
    inp.type = "file";
    inp.multiple = true;
    inp.onchange = () => {
      // 浏览器 File 拿不到绝对路径, 此分支主要给 Mac/Linux 兼容. Windows 上必须用拖放.
      toast.info("浏览器无法读绝对路径, 请把文件拖进来");
    };
    inp.click();
  }
  ...
  async function send() {
    ...
    if (!cfg) setCfg(c);
    const cleaned = sanitizeForAnthropic(history, c.provider);
    // 拼附件为代码块
    const attachmentsText = attached.length === 0 ? "" : attached.map(formatAttachment).join("\n\n");
    const pidPrefix = pid ? `(当前会话: PID ${pid}${processName ? " " + processName : ""}) ` : "";
    const fullText = [
    ...
    const next: ChatMsg[] = [...cleaned, userMsg];
    setHistory(next);
    setInput("");
    setAttached([]);
    setBusy(true);
  ...
  async function loadOldSession(id: string) {
    try {
      const payloads = await dbgIpc.aiSessionLoad(id);
      const msgs = payloads as ChatMsg[];
      setHistory(msgs);
      setSessionId(id);
