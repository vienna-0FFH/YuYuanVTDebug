...
import { cancelActiveRunExecution, endStepExecution, useSession } from "./sessionStore";
...
export function BreakHitToast() {
  ...
  const handleHit = useCallback(async (hit: HitInput) => {
    ...
    const activeStep = initial.activeStep;
    let activeRunToCancel: { tid: number; kind: "over" | "out" } | null = null;
    if (activeStep?.tid === hit.tid && activeStep.kind !== "many") {
      if (
        (activeStep.kind === "over" || activeStep.kind === "out")
        && hit.transient !== true
      ) {
        activeRunToCancel = { tid: activeStep.tid, kind: activeStep.kind };
      } else {
        endStepExecution(activeStep.tid, activeStep.kind);
      }
    }
    ...
    if (hit.tid) {
      try {
        ...
        if (activeRunToCancel) {
          try {
            await cancelActiveRunExecution(
              hit.pid,
              activeRunToCancel.tid,
              activeRunToCancel.kind,
            );
          } catch (error) {
            toast.error(`提前停止后的步进回滚失败: ${errMsg(error)}`);
          }
        }
  ...
  useEffect(() => {
    ...
    void listen<NativeControlError>("dbg-native-error", (event) => {
      ...
      const current = useSession.getState();
      if (current.pid !== error.pid) return;
      const activeStep = current.activeStep;
      if (
        activeStep?.tid === error.tid
        && (activeStep.kind === "over" || activeStep.kind === "out")
      ) {
        void cancelActiveRunExecution(error.pid, error.tid, activeStep.kind).catch(
          (cleanupError) => toast.error(`控制错误后的步进回滚失败: ${errMsg(cleanupError)}`),
        );
      }
      toast.error(
