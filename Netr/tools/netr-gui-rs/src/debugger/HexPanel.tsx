...
export function HexPanel() {
  ...
  const address = session.followAddress ?? session.address;
  const setAddress = session.setAddress;
    ...
    return (
      <ContextMenu>
        ...
        <ContextMenuContent>
          ...
          <ContextMenuItem onClick={() => setAddress(fieldAddr)}>跳到该地址</ContextMenuItem>
          {ptrVal !== null && (
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex h-9 shrink-0 items-center gap-2 border-b border-border bg-card/60 px-3 text-xs">
        <span className="font-semibold uppercase tracking-wider text-muted-foreground">Hex 内存</span>
