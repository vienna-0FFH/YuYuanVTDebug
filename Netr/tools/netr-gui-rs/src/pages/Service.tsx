 ...
 * 内部:
 *   加载 = install + start + openDevice + 自动 license + 自我保护(由 driverStore 自动)
    ...
    <div className="space-y-6">
      ...
      <Card className="card-hover">
        ...
        <CardContent className="grid gap-4 sm:grid-cols-3">
          ...
          <StatBlock
            label="自我保护"
            value={selfProtected ? "已激活" : "未激活"}
            tone={selfProtected ? "success" : "muted"}
          />
        </CardContent>
      </Card>
          ...
          <CardContent className="flex items-center gap-3 p-4 text-sm">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs text-muted-foreground">
                ...
                {" · "}
                自我保护: <span className={selfProtected ? "text-success" : "text-warning"}>{selfProtected ? "已激活" : "等待中…"}</span>
              </div>
              {!driverLicensed && lastAuthError && (
                <div className="mt-1 text-xs text-warning">授权下发失败: {lastAuthError}</div>
              )}
              {driverLicensed && !selfProtected && selfProtectError && (
