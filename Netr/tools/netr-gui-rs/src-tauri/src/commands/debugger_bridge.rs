...
static INJECTION_LOG_LOCK: OnceLock<Mutex<()>> = OnceLock::new();

fn injection_log_candidates() -> Vec<PathBuf> {
    let mut candidates = Vec::new();
    if let Some(path) = std::env::var_os("NETR_INJECTION_LOG") {
        candidates.push(PathBuf::from(path));
    }
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            candidates.push(dir.join("GuardMeta-injection.log"));
        }
    }
    if let Some(program_data) = std::env::var_os("ProgramData") {
        candidates.push(
            PathBuf::from(program_data)
                .join("GuardMeta")
                .join("Logs")
                .join("GuardMeta-injection.log"),
        );
    }
    candidates.push(std::env::temp_dir().join("GuardMeta-injection.log"));
    candidates.dedup();
    candidates
}

pub(crate) fn injection_diag(message: impl AsRef<str>) -> Option<PathBuf> {
    injection_diag_lines(&[message.as_ref().to_owned()])
}

pub(crate) fn injection_diag_lines(messages: &[String]) -> Option<PathBuf> {
    if messages.is_empty() {
        return None;
    }
    let _guard = INJECTION_LOG_LOCK
    ...
    for message in messages {
        ...
        lines.push_str(&format!(
            "[unix_ms={unix_ms} gui_pid={} thread={:?}] {message}\r\n",
            std::process::id(),
            std::thread::current().id(),
        ));
    }

    for path in injection_log_candidates() {
...
fn resolve_injector(wow64: bool) -> AppResult<PathBuf> {
    let suffix = if wow64 { "32" } else { "64" };
    let injector_name = format!("NetrBridgeInjector{suffix}.exe");
    ...
    for dir in &dirs {
        let injector = dir.join(&injector_name);
        if injector.is_file() {
            return Ok(injector.canonicalize().unwrap_or(injector));
        }
    }
    ...
    let searched = dirs
        ...
        .join("; ");
    Err(AppError::Internal(format!(
        "loader injector not found ({injector_name}); searched: {searched}"
    )))
}
...
pub(crate) fn inject_dll_via_windows_loader(pid: u32, dll_path: &str) -> AppResult<()> {
    let initial_log = injection_diag(format!(
        "fallback.begin target_pid={pid} requested_dll={dll_path}"
    ));
    let result = (|| -> AppResult<()> {
        let process = open_process_for_query(pid)?;
        ensure_process_active(process.0, pid, "before loader fallback")?;
        let wow64 = target_is_wow64(process.0, pid)?;
        let injector = resolve_injector(wow64)?;
        let path = PathBuf::from(dll_path);
        if !path.is_file() {
            return Err(AppError::Io(format!(
                "DLL not found for loader fallback: {}",
                path.display()
            )));
        }
        let path = path.canonicalize().unwrap_or(path);
        injection_diag(format!(
            "fallback.exec target_pid={pid} architecture={} injector={} dll={}",
            if wow64 { "x86" } else { "x64" },
            injector.display(),
            path.display(),
        ));
        let output = Command::new(&injector)
            ...
            .arg(&path)
            .output()
            .map_err(|e| AppError::Io(format!("start {}: {e}", injector.display())))?;

        let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
        let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
        injection_diag(format!(
            "fallback.exit target_pid={pid} code={} success={} stdout={} stderr={}",
            output.status.code().unwrap_or(-1),
        ...
        if output.status.success() {
            ensure_process_active(process.0, pid, "after loader fallback")?;
            return Ok(());
        }

        let detail = if !stderr.is_empty() { stderr } else { stdout };
        Err(AppError::Internal(format!(
            "loader injector failed (exit={}): {}",
            output.status.code().unwrap_or(-1),
            if detail.is_empty() {
    ...
    match result {
        Ok(()) => {
            injection_diag(format!("fallback.success target_pid={pid}"));
            Ok(())
        }
        Err(error) => {
            let log_path = injection_diag(format!(
                "fallback.failure target_pid={pid} error={error}"
            ))
            .or(initial_log);
            ...
            Err(AppError::Internal(format!("{error}{diagnostic}")))
        }
    }
}
