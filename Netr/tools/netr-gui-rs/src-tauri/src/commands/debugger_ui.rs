...
use windows::Win32::Foundation::{
    CloseHandle, ERROR_INVALID_HANDLE, FILETIME, HANDLE, STILL_ACTIVE, WAIT_OBJECT_0,
};
...
pub fn spawn_private_event_poller(
    ...
    std::thread::spawn(move || loop {
        ...
        if result.event.kind == 1
            && (result.event.dr6 & 0xFFFF_FFFF_0000_0000) == 0x5354_4550_0000_0000
        {
            let stage = (result.event.dr6 & 0xFF) as u8;
            let cpu = ((result.event.dr6 >> 8) & 0xFF) as u8;
            let mtf_active = ((result.event.dr6 >> 16) & 0xFF) as u8;
            let mtf_reason = ((result.event.dr6 >> 24) & 0xFF) as u8;
            ...
            super::native_debug::private_dbgk_entry_diag(
                result.event.process_id,
                "private_swbp.step_root_failure",
                format!(
                    "sequence={} rip=0x{:X} stage={stage_name}({stage}) cpu={cpu} mtf_active={mtf_active} mtf_reason={mtf_reason}",
                    result.event.sequence, result.event.rip,
                ),
            );
        }
        ...
        let defer_transient_cleanup = transient && !entry;
        if transient && !defer_transient_cleanup {
        ...
        if defer_transient_cleanup {
            super::native_debug::private_dbgk_entry_diag(
                result.event.process_id,
                "private_swbp.transient_deferred",
                format!(
                    "sequence={} tid={tid} rip=0x{:X}",
                    result.event.sequence, result.event.rip
                ),
            );
        }
        ...
        if app.emit("dbg-private-hit", payload).is_err() {
            ...
            if transient {
                let _ = consume_transient_private_swbp(
                    &handles,
                    device.inner(),
                    result.event.process_id,
                    result.event.rip,
...
fn builtin_bridge_capabilities(private_debug_object: bool) -> u32 {
    let capabilities = BRIDGE_CAP_DEBUG_EVENTS
    ...
    if private_debug_object {
        capabilities | BRIDGE_CAP_PRIVATE_DEBUG_OBJECT
    } else {
        capabilities
    }
}

fn validate_private_debug_object_registration(
    device: &DeviceState,
    requested: bool,
    previously_configured: bool,
    flags: u32,
) -> AppResult<()> {
    let enabled = (flags & BRIDGE_STATE_PRIVATE_DEBUG_OBJECT) != 0;
    if enabled != requested {
        return Err(AppError::Internal(
            "VT private DebugObject setting changed during debugger registration; retry the operation"
                .into(),
        ));
    }
    if requested && (flags & BRIDGE_CAP_PRIVATE_DEBUG_OBJECT) == 0 {
        return Err(AppError::Internal(
            "driver did not grant the private DebugObject capability".into(),
        ));
    }
    if requested && !previously_configured {
        configure_private_dbgk_symbols(device)?;
    }
    Ok(())
}
...
fn bridge_register_builtin(
    ...
    let private_debug_object = super::antivmp::private_debug_object_enabled(device)?;
    let requested_capabilities = builtin_bridge_capabilities(private_debug_object);
    let (already_registered, previously_configured) = {
        let lifecycle = state.lifecycle.lock();
        (
            lifecycle.registered,
            (lifecycle.granted_capabilities & BRIDGE_CAP_PRIVATE_DEBUG_OBJECT) != 0,
        )
    };
    ...
    if already_registered {
        ...
        validate_private_debug_object_registration(
            device,
            private_debug_object,
    ...
    if let Err(error) = validate_private_debug_object_registration(
        device,
        private_debug_object,
        false,
        result.flags,
...
fn cached_process_exit_code(state: &Arc<DebuggerHandles>, pid: u32) -> Option<u32> {
    let raw = state.procs.lock().get(&pid).copied()?;
    let mut exit_code = STILL_ACTIVE.0 as u32;
    unsafe { GetExitCodeProcess(HANDLE(raw as _), &mut exit_code) }.ok()?;
    (exit_code != STILL_ACTIVE.0 as u32).then_some(exit_code)
}
...
fn resume_and_close_pending_private_events(
    ...
    for mut event in pending {
        ...
        if !event.thread_resumed {
            if handle_is_gone {
                if cached_process_exit_code(state, event.process_id).is_some() {
                ...
                if previous != u32::MAX {
                    ...
                    handle_is_gone = resume_error == ERROR_INVALID_HANDLE;
                    let mut thread_exit_code = STILL_ACTIVE.0 as u32;
                    let thread_exited = !handle_is_gone
                        && unsafe { GetExitCodeThread(handle, &mut thread_exit_code) }.is_ok()
                        && thread_exit_code != STILL_ACTIVE.0 as u32;
                    let process_exit_code =
                        cached_process_exit_code(state, event.process_id);
                    if thread_exited || process_exit_code.is_some() {
                        event.thread_resumed = true;
                        super::native_debug::private_dbgk_entry_diag(
                            event.process_id,
                            "lifecycle.detach.suspension_retired",
                            format!(
                                "tid={} reason={} resume_error=0x{:X}",
                                event.thread_id,
                                process_exit_code
                                    .map(|exit_code| format!("process_exit_0x{exit_code:08X}"))
                                    .unwrap_or_else(|| {
                                        format!("thread_exit_0x{thread_exit_code:08X}")
                                    }),
                                resume_error.0,
                            ),
                        );
        ...
        if !handle_is_gone && unsafe { CloseHandle(handle) }.is_err() {
            failures.push(format!(
...
fn detach_last_target(
    ...
    if !driver_detached {
        ...
        let (resume_retry, resume_failures) =
            resume_and_close_pending_private_events(state, continued);
    ...
    let (resume_retry, resume_failures) =
        resume_and_close_pending_private_events(state, pending);
...
#[tauri::command]
pub async fn dbg_suspend_thread(
    state: State<'_, Arc<DebuggerHandles>>,
    tid: u32,
) -> AppResult<u32> {
    let pending = state.pending_private.lock().get(&tid).map(|event| {
        (event.process_id, event.sequence, event.rip)
    });
    if let Some((pid, sequence, rip)) = pending {
        super::native_debug::private_dbgk_entry_diag(
            pid,
            "private_swbp.suspend_thread.skipped",
            format!(
                "sequence={sequence} tid={tid} rip=0x{rip:X} reason=already-pending-private"
            ),
        );
        return Ok(0);
    }
...
fn pending_private_transient_sequence_for_step(
    state: &Arc<DebuggerHandles>,
    pid: u32,
    tid: u32,
    rip: u64,
    ...
    let sequence = {
        let pending_events = state.pending_private.lock();
        let pending = pending_events.get(&tid)?;
        if pending.process_id != pid || pending.rip != rip || pending.kind == 0 {
            return None;
        }
        pending.sequence
    };
    state
        ...
        .lock()
        .get(&pid)
        .and_then(|items| items.get(&rip))
        .is_some_and(|target| target.owner_tid == tid)
        .then_some(sequence)
}

fn consume_pending_private_transient_for_thread(
    state: &Arc<DebuggerHandles>,
    device: &DeviceState,
    tid: u32,
    reason: &str,
    ...
    let pending = {
        let pending_events = state.pending_private.lock();
        pending_events.get(&tid).and_then(|event| {
            (event.kind != 0).then_some((event.process_id, event.rip, event.sequence))
        })
    };
    let Some((pid, address, sequence)) = pending else {
        return Ok(false);
    };
    let is_transient = state
        .transient_bps
        .lock()
        .get(&pid)
    ...
    let consumed = consume_transient_private_swbp(state, device, pid, address)?;
    if consumed {
        super::native_debug::private_dbgk_entry_diag(
            pid,
            "private_swbp.transient_consumed",
            format!(
                "sequence={sequence} tid={tid} rip=0x{address:X} reason={reason}"
            ),
        );
    }
    Ok(consumed)
...
pub async fn dbg_resume_thread(
    ...
    consume_pending_private_transient_for_thread(
        state.inner(),
        device.inner(),
        tid,
        "resume",
...
fn ensure_no_private_run_target(
    state: &Arc<DebuggerHandles>,
    pid: u32,
    tid: u32,
) -> AppResult<()> {
    let pending_hit_address = {
        let pending_events = state.pending_private.lock();
        pending_events.get(&tid).and_then(|pending| {
            (pending.process_id == pid && pending.kind != 0).then_some(pending.rip)
        })
    };
    let active_target = state
        ...
        .get(&pid)
        .and_then(|items| {
            items.iter().find_map(|(address, target)| {
                (target.owner_tid == tid && Some(*address) != pending_hit_address)
                    .then_some(*address)
            })
        });
    if let Some(address) = active_target {
        return Err(AppError::Internal(format!(
            "thread {tid} is still running to one-shot breakpoint 0x{address:X}"
        )));
    }
    Ok(())
}
...
pub(crate) async fn wait_for_builtin_thread_pause(
    ...
    loop {
        ...
        if private_pending {
            if require_native_event {
                return Err(AppError::Internal(format!(
                    "VT synthetic step for PID {pid} thread {tid} re-entered its private breakpoint gate before completing MTF"
                )));
            }
            return Ok(());
        }
...
async fn wait_for_builtin_thread_pause_unbounded(
    state: &Arc<DebuggerHandles>,
    pid: u32,
    tid: u32,
) -> AppResult<()> {
    loop {
        ...
        let private_pending = state
            ...
            .get(&tid)
            .is_some_and(|event| event.process_id == pid);
        if private_pending || super::native_debug::owns_pending_event(pid, tid) {
            return Ok(());
        }
        match thread_owner_pid(tid) {
            Ok(owner_pid) if owner_pid == pid => {}
            Ok(owner_pid) => {
                return Err(AppError::Internal(format!(
                    "thread {tid} moved from PID {pid} to PID {owner_pid} while stepping"
                )))
            }
            Err(error) => {
                return Err(AppError::Internal(format!(
                    "thread {tid} exited while stepping: {error}"
                )))
            }
        }
        tokio::time::sleep(std::time::Duration::from_millis(10)).await;
    ...
    .await
    {
        ...
        Err(wait_error) => {
            ...
            let already_paused = state
                ...
                .get(&tid)
                .is_some_and(|event| event.process_id == pid)
                || super::native_debug::owns_pending_event(pid, tid);
            if !already_paused {
...
async fn start_vt_single_step(
    ...
    let pending_transient_sequence =
        pending_private_transient_sequence_for_step(state, pid, tid, rip);
    let reuses_pending_transient = pending_transient_sequence.is_some();
    let pending_private_sequence = if reuses_pending_transient {
        super::native_debug::private_dbgk_entry_diag(
            pid,
            "vt_step.pending_reuse",
            format!(
                "sequence={} tid={tid} rip=0x{rip:X} source=transient-swbp",
                pending_transient_sequence.unwrap_or_default()
            ),
        );
        pending_transient_sequence
    } else {
    ...
    let installed_swbp_gate = uses_swbp_gate && !reuses_pending_transient;
    if uses_swbp_gate {
        super::native_debug::private_dbgk_entry_diag(
            pid,
            "vt_step.gate.begin",
            format!(
                "tid={tid} address=0x{rip:X} source={}",
                if reuses_pending_transient {
                    "pending-transient"
                } else {
                    "new-transient"
                }
    ...
    let private_event_resumed = if pending_private_sequence.is_some() {
        match resume_pending_private_event(state, device, tid) {
...
pub async fn dbg_step_into(
    ...
    let state = state.inner().clone();
    let _step = state.step_mutation.try_lock().map_err(|_| {
        AppError::Internal("another debugger step command is still being armed".into())
    })?;
    let pid = thread_owner_pid(tid)?;
    ensure_no_private_run_target(&state, pid, tid)?;
    match builtin_target_mode(&state, pid) {
        Some(BuiltinDebugMode::Native) =>
...
pub async fn dbg_step_many(
    ...
    for completed in 0..count {
        let async_run = match kind.as_str() {
            "into" => dbg_step_into(
            ...
            .await
            .map(|result| result.kind == "run-over"),
            _ => unreachable!(),
        };
        let async_run = async_run.map_err(|error| {
            AppError::Internal(format!(
                "{kind} failed after {completed}/{count} completed steps: {error}"
            ))
        })?;
        if async_run {
            if let Err(error) = wait_for_builtin_thread_pause_unbounded(&handles, pid, tid).await {
            return Err(AppError::Internal(format!(
...
#[tauri::command]
pub async fn dbg_cancel_active_run(
    state: State<'_, Arc<DebuggerHandles>>,
    device: State<'_, DeviceState>,
    pid: u32,
    tid: u32,
    ...
    match cancel_transient_private_swbps_for_thread(&state, device.inner(), pid, tid) {
        Ok(value) => cancelled |= value,
        Err(error) => failures.push(format!("private VT one-shot cleanup: {error}")),
    }
    if super::native_debug::is_session(pid) {
        match super::native_debug::cancel_step_over(pid, tid) {
            Ok(value) => cancelled |= value,
            Err(error) => failures.push(format!("native one-shot cleanup: {error}")),
        }
    }
...
pub async fn dbg_consume_transient_bp(
    ...
    let owner_tid = state
        ...
        .get(&pid)
        .and_then(|items| items.get(&address))
        .map(|target| target.owner_tid);
    let pending = owner_tid.and_then(|tid| {
        state
            .pending_private
            .lock()
            .get(&tid)
            .filter(|event| event.process_id == pid && event.rip == address)
            .map(|event| (event.sequence, tid))
    });
    if let Some((sequence, tid)) = pending {
        super::native_debug::private_dbgk_entry_diag(
            pid,
            "private_swbp.transient_consume.deferred",
            format!(
                "sequence={sequence} tid={tid} address=0x{address:X} reason=public-legacy-command"
            ),
        );
        return Ok(true);
    }
...
fn consume_transient_private_swbp(
    ...
    super::native_debug::private_dbgk_entry_diag(
        pid,
        "private_swbp.transient_consume.begin",
        format!(
            "address=0x{address:X} caller={}:{}",
            caller.file(),
            caller.line(),
        ),
    );
    ...
    let Some(target) = target else {
        super::native_debug::private_dbgk_entry_diag(
            pid,
            "private_swbp.transient_consume.result",
            format!("address=0x{address:X} result=not-found"),
        );
        return Ok(false);
    };
    if target.owns_registration {
        if let Err(error) = remove_private_swbp(state, device, pid, address) {
            ...
            super::native_debug::private_dbgk_entry_diag(
                pid,
                "private_swbp.transient_consume.error",
                format!("address=0x{address:X} error={error}"),
            );
            return Err(error);
        }
    }
    super::native_debug::private_dbgk_entry_diag(
        ...
        "private_swbp.transient_consume.result",
        format!(
            "address=0x{address:X} result=ok owns_registration={}",
            target.owns_registration,
        ),
    );
    Ok(true)
}

fn cancel_transient_private_swbps_for_thread(
    state: &Arc<DebuggerHandles>,
    device: &DeviceState,
    pid: u32,
    tid: u32,
) -> AppResult<bool> {
    let addresses: Vec<u64> = state
        .transient_bps
        .lock()
        .get(&pid)
        .map(|items| {
            items
                .iter()
                .filter_map(|(address, target)| (target.owner_tid == tid).then_some(*address))
                .collect()
        })
        .unwrap_or_default();
    let mut cancelled = false;
    ...
    for address in addresses {
        if let Some(sequence) =
            pending_private_transient_sequence_for_step(state, pid, tid, address)
        {
            super::native_debug::private_dbgk_entry_diag(
                pid,
                "private_swbp.transient_cancel.deferred",
                format!(
                    "sequence={sequence} tid={tid} address=0x{address:X} reason=pending-private-hit"
                ),
            );
            continue;
        }
        match consume_transient_private_swbp(state, device, pid, address) {
            Ok(value) => cancelled |= value,
            Err(error) => failures.push(format!("0x{address:X}: {error}")),
        }
    }
    if failures.is_empty() {
        Ok(cancelled)
