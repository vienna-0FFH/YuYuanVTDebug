...
- writes to committed `MEM_PRIVATE` pages through VT first;
- writes to shared `MEM_IMAGE` or write-copy `MEM_MAPPED` pages by reading the
  original bytes through VT, writing those same bytes through the OS-COW
  helper (driver IOCTL with the original `WriteProcessMemory` as a safe
  fallback), verifying with `QueryWorkingSetEx` that the target page is no
  longer shared, and only then applying the requested bytes through VT;
...
original target byte is never damaged. Ordinary private-page writes remain
VT-first. A shared image page is never sent to the raw VT physical writer until
OS-COW ownership has been materialized and independently verified; failure is
reported without applying the requested bytes to the shared page.
