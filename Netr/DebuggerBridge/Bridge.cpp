...
struct PendingPrivateDbgkEvent {
    std::uint64_t Sequence{};
    std::uint64_t VirtualDr6{};
    ULONG State{};
    HANDLE Process = nullptr;
    HANDLE Thread = nullptr;
};

struct PendingWindowsDebugEvent {
    std::uint64_t VirtualDr6{};
...
SRWLOCK g_hwbp_commit_lock = SRWLOCK_INIT;
...
std::unordered_map<std::uint64_t, VirtualDebugState> g_debug_states;
...
std::unordered_map<std::uint64_t, PendingWindowsDebugEvent> g_pending_windows_debug_events;
...
std::atomic<unsigned long long> g_write_sequence{0};
...
BOOL WINAPI BridgeSetThreadContextRealTf(HANDLE thread, const CONTEXT* context);
...
void LatchWindowsDebugEventDr6(const DEBUG_EVENT* event);
...
BOOL WINAPI BridgeWow64SetThreadContextRealTf(HANDLE thread, const WOW64_CONTEXT* context);
#endif
...
std::uint64_t LookupPendingDebugEventDr6(DWORD pid, DWORD tid)
{
    std::uint64_t dr6 = 0;
    AcquireSRWLockShared(&g_private_event_lock);
    const auto private_pending =
        g_pending_private_dbgk_events.find(PrivateEventKey(pid, tid));
    if (private_pending != g_pending_private_dbgk_events.end()) {
        dr6 |= private_pending->second.VirtualDr6;
    }
    const auto windows_pending =
        g_pending_windows_debug_events.find(PrivateEventKey(pid, tid));
    if (windows_pending != g_pending_windows_debug_events.end()) {
        dr6 |= windows_pending->second.VirtualDr6;
    }
    ReleaseSRWLockShared(&g_private_event_lock);
    return dr6;
}

void UpdatePendingDebugEventDr6(DWORD pid, DWORD tid, std::uint64_t dr6)
{
    AcquireSRWLockExclusive(&g_private_event_lock);
    const auto private_pending =
        g_pending_private_dbgk_events.find(PrivateEventKey(pid, tid));
    if (private_pending != g_pending_private_dbgk_events.end()) {
        private_pending->second.VirtualDr6 &= dr6 & 0xFULL;
    }
    const auto windows_pending =
        g_pending_windows_debug_events.find(PrivateEventKey(pid, tid));
    if (windows_pending != g_pending_windows_debug_events.end()) {
        windows_pending->second.VirtualDr6 &= dr6 & 0xFULL;
    }
    ReleaseSRWLockExclusive(&g_private_event_lock);
}

bool RetirePendingWindowsDebugEvent(
    DWORD pid,
    DWORD tid,
    PendingWindowsDebugEvent* retired_event)
{
    bool retired = false;
    PendingWindowsDebugEvent value = {};
    AcquireSRWLockExclusive(&g_private_event_lock);
    const auto pending =
        g_pending_windows_debug_events.find(PrivateEventKey(pid, tid));
    if (pending != g_pending_windows_debug_events.end()) {
        value = pending->second;
        g_pending_windows_debug_events.erase(pending);
        retired = true;
    }
    ReleaseSRWLockExclusive(&g_private_event_lock);
    if (retired_event) *retired_event = value;
    return retired;
}

bool SendPendingHardwareHitOperation(
    DWORD operation,
    DWORD pid,
    DWORD tid,
    std::uint64_t generation,
    std::uint64_t* dr6_mask,
    std::uint64_t* returned_generation)
{
    if (g_device == INVALID_HANDLE_VALUE || pid == 0 || tid == 0 ||
        (operation != HV_BRIDGE_PENDING_HWBP_QUERY &&
         operation != HV_BRIDGE_PENDING_HWBP_RETIRE)) {
    ...
    HV_BRIDGE_PENDING_HWBP_RESULT result = {};
    DWORD returned = 0;
    const BOOL ok = DeviceIoControl(
        g_device,
    ...
    if (operation == HV_BRIDGE_PENDING_HWBP_QUERY &&
        ((result.Dr6Mask & 0xFULL) == 0 || result.Generation == 0)) {
        SetLastError(ERROR_INVALID_DATA);
        return false;
    }

    if (dr6_mask) *dr6_mask = result.Dr6Mask & 0xFULL;
    if (returned_generation) *returned_generation = result.Generation;
    return true;
}
    ...
    DWORD timeout)
{
    ...
    if ((source.Flags & HV_BRIDGE_DBGK_EVENT_VT_HWBP) != 0) {
        pending.VirtualDr6 =
            source.ExceptionInformation[HV_BRIDGE_DBGK_VT_DR6_INDEX] & 0xFULL;
    }
    ...
    ReleaseSRWLockExclusive(&g_private_event_lock);
    Trace(L"[DBGK-WAIT] delivered seq=%llu state=%lu pid=%lu tid=%lu process=%p thread=%p virtual_dr6=0x%llX\n",
          ...
          static_cast<unsigned long long>(pending.VirtualDr6));
...
void ReleasePendingWindowsDebugEvents(DWORD pid)
{
    AcquireSRWLockExclusive(&g_private_event_lock);
    for (auto pending = g_pending_windows_debug_events.begin();
         pending != g_pending_windows_debug_events.end();) {
        const DWORD pending_pid = static_cast<DWORD>(pending->first >> 32);
        if (pid != 0 && pending_pid != pid) {
            ++pending;
            continue;
        }
        pending = g_pending_windows_debug_events.erase(pending);
    }
    ReleaseSRWLockExclusive(&g_private_event_lock);
}
    ...
    UCHAR type)
{
    ...
    if (target_tid == 0 &&
        (capabilities & HV_BRIDGE_CAP_DR_HWBP_FALLBACK)) {
    ...
    if (!ok || returned < sizeof(result)) {
        ...
        Trace(L"[HWBP] ioctl=0x%08X target=%lu tid=%lu slot=%lu address=0x%llX length=%u type=%u failed win32=%lu returned=%lu\n",
              code, target_pid, target_tid, slot,
              static_cast<unsigned long long>(address), length, type,
              error, returned);
        SetLastError(error);
        return false;
    }
    if (result.Status == HV_BRIDGE_STATUS_SUCCESS) {
        Trace(L"[HWBP] ioctl=0x%08X target=%lu tid=%lu slot=%lu address=0x%llX length=%u type=%u policy=0x%X mode=%llu\n",
              code, target_pid, target_tid, slot,
              static_cast<unsigned long long>(address), length, type, policy,
    ...
    const DWORD error = result.Status == HV_BRIDGE_STATUS_NOT_FOUND
        ? ERROR_NOT_FOUND : ERROR_INVALID_PARAMETER;
    Trace(L"[HWBP] ioctl=0x%08X target=%lu tid=%lu slot=%lu address=0x%llX length=%u type=%u rejected status=0x%08lX info=%llu\n",
          code, target_pid, target_tid, slot,
          static_cast<unsigned long long>(address), length, type,
          result.Status, static_cast<unsigned long long>(result.Info));
    SetLastError(error);
    return false;
}
...
bool HardwareSlotEnabled(const VirtualDebugState& state, ULONG slot)
{
    return ((state.Dr7 >> (slot * 2)) & 3ULL) != 0;
}

bool HardwareSlotMaterializable(
    const VirtualDebugState& state,
    ULONG slot)
{
    if (!HardwareSlotEnabled(state, slot)) return false;

    const UCHAR type = static_cast<UCHAR>(
        (state.Dr7 >> (16 + slot * 4)) & 3ULL);
    const UCHAR length = DecodeBreakpointLength(state.Dr7, slot, type);
    const std::uint64_t address = state.Dr[slot];
    if (address < 0x10000ULL || address >= 0x0000800000000000ULL ||
        address + length < address) {
        return false;
    }
    if (type == 0) return length == 1;
    if (type != 1 && type != 3) return false;
    return (address & (length - 1)) == 0 &&
           ((address & 0xFFFULL) + length) <= 0x1000ULL;
}

bool HardwareSlotsEquivalent(
    const VirtualDebugState& left,
    const VirtualDebugState& right,
    ULONG slot)
{
    const bool left_materialized = HardwareSlotMaterializable(left, slot);
    const bool right_materialized = HardwareSlotMaterializable(right, slot);
    if (left_materialized != right_materialized) return false;
    if (!left_materialized) return true;

    const UCHAR left_type = static_cast<UCHAR>(
        (left.Dr7 >> (16 + slot * 4)) & 3ULL);
    const UCHAR right_type = static_cast<UCHAR>(
        (right.Dr7 >> (16 + slot * 4)) & 3ULL);
    return left.Dr[slot] == right.Dr[slot] &&
           left_type == right_type &&
           DecodeBreakpointLength(left.Dr7, slot, left_type) ==
               DecodeBreakpointLength(right.Dr7, slot, right_type);
}

bool ApplyHardwareSlot(
    DWORD pid,
    DWORD tid,
    const VirtualDebugState& state,
    ULONG slot)
{
    if (!HardwareSlotMaterializable(state, slot)) {
        return SendHardwareBreakpoint(
            IOCTL_HV_BRIDGE_CLEAR_HWBP, pid, tid, slot, 0, 0, 0);
    }
...
bool ApplyHardwareStateDelta(
    DWORD pid,
    DWORD tid,
    const VirtualDebugState* previous,
    const VirtualDebugState& state)
{
    const VirtualDebugState empty = {};
    const VirtualDebugState& baseline = previous ? *previous : empty;
    bool applied[4] = {};

    for (ULONG slot = 0; slot < 4; slot++) {
        if (previous && HardwareSlotsEquivalent(baseline, state, slot)) {
            continue;
        }
        if (!ApplyHardwareSlot(pid, tid, state, slot)) {
            const DWORD error = GetLastError();
            for (ULONG rollback_slot = 0; rollback_slot < 4; rollback_slot++) {
                if (applied[rollback_slot]) {
                    (void)ApplyHardwareSlot(
                        pid, tid, baseline, rollback_slot);
                }
            }
            SetLastError(error);
            return false;
...
{
    const DWORD saved_error = GetLastError();
    AcquireSRWLockExclusive(&g_hwbp_commit_lock);
    AcquireSRWLockExclusive(&g_debug_state_lock);
    for (auto state = g_debug_states.begin(); state != g_debug_states.end();) {
        if (static_cast<DWORD>(state->first >> 32) == pid) {
            state = g_debug_states.erase(state);
        } else {
            ++state;
        }
    }
    ...
    for (ULONG slot = 0; slot < 4; slot++) {
        (void)SendHardwareBreakpoint(
            IOCTL_HV_BRIDGE_CLEAR_HWBP, pid, 0, slot, 0, 0, 0);
    }
    ReleaseSRWLockExclusive(&g_hwbp_commit_lock);
    ClearTargetSoftwareBreakpoints(pid);
    SetLastError(saved_error);
}

void ClearThreadHardwareBreakpoints(DWORD pid, DWORD tid)
{
    if (!pid || !tid) return;
    const DWORD saved_error = GetLastError();
    AcquireSRWLockExclusive(&g_hwbp_commit_lock);
    AcquireSRWLockExclusive(&g_debug_state_lock);
    g_debug_states.erase(PrivateEventKey(pid, tid));
    ReleaseSRWLockExclusive(&g_debug_state_lock);
    for (ULONG slot = 0; slot < 4; slot++) {
        (void)SendHardwareBreakpoint(
            IOCTL_HV_BRIDGE_CLEAR_HWBP, pid, tid, slot, 0, 0, 0);
...
bool UnbindTarget(DWORD pid)
{
    ...
    ReleasePendingPrivateEvents(pid);
    ReleasePendingPrivateDbgkEvents(pid);
    ReleasePendingWindowsDebugEvents(pid);
...
void CleanupBridgeState()
{
    ...
    ReleasePendingPrivateEvents(0);
    ReleasePendingPrivateDbgkEvents(0);
    ReleasePendingWindowsDebugEvents(0);
    ReleaseSRWLockExclusive(&g_target_lock);
...
void ObserveDebugEvent(const DEBUG_EVENT* event)
{
    ...
    if (event->dwDebugEventCode == EXIT_THREAD_DEBUG_EVENT) {
        ClearThreadHardwareBreakpoints(
            event->dwProcessId,
            event->dwThreadId);
    }
    ...
    DWORD timeout)
{
    ...
    for (;;) {
        ...
        if (native_ok) {
            LatchWindowsDebugEventDr6(event);
            ObserveDebugEvent(event);
            return TRUE;
        }
...
enum class TargetWriteRoute {
    VtPrivate,
    CowMaterialize,
    Reject
};

const wchar_t* TargetWriteRouteName(TargetWriteRoute route)
{
    switch (route) {
    case TargetWriteRoute::VtPrivate: return L"vt-private";
    case TargetWriteRoute::CowMaterialize: return L"cow-materialize";
    default: return L"reject";
    }
}

const wchar_t* TargetMemoryTypeName(DWORD type)
{
    switch (type) {
...
bool IsWriteCopyProtection(DWORD protection)
{
    const DWORD base = protection & 0xFFu;
    return base == PAGE_WRITECOPY || base == PAGE_EXECUTE_WRITECOPY;
}

bool QueryTargetPageShared(
    HANDLE process,
    ULONG_PTR address,
    bool* known,
    bool* shared,
    DWORD* query_error)
{
    ...
    if (query_error) *query_error = ERROR_SUCCESS;

    PSAPI_WORKING_SET_EX_INFORMATION working_set = {};
    working_set.VirtualAddress = reinterpret_cast<PVOID>(address & ~0xFFFULL);
    if (!QueryWorkingSetEx(process, &working_set, sizeof(working_set))) {
        if (query_error) *query_error = GetLastError();
        return false;
    }
    if (!working_set.VirtualAttributes.Valid) {
        if (query_error) *query_error = ERROR_NOT_FOUND;
        return true;
    }

    if (known) *known = true;
    if (shared) *shared = working_set.VirtualAttributes.Shared != 0;
    return true;
}

TargetWriteRoute ClassifyTargetWritePage(
    HANDLE process,
    ULONG_PTR address,
    SIZE_T size,
...
{
    if (memory) *memory = {};
    if (working_set_known) *working_set_known = false;
    if (shared) *shared = true;
    if (query_error) *query_error = ERROR_SUCCESS;
    ...
    if (VirtualQueryEx(
            ...
            &local_memory, sizeof(local_memory)) != sizeof(local_memory)) {
        if (query_error) *query_error = GetLastError();
        return TargetWriteRoute::Reject;
    }
    if (memory) *memory = local_memory;

    const ULONG_PTR region_start =
        reinterpret_cast<ULONG_PTR>(local_memory.BaseAddress);
    const bool valid_range = address >= region_start &&
        size <= static_cast<SIZE_T>(UINTPTR_MAX - address) &&
        local_memory.RegionSize <=
            static_cast<SIZE_T>(UINTPTR_MAX - region_start) &&
        address + size <= region_start + local_memory.RegionSize;
    if (!valid_range || local_memory.State != MEM_COMMIT) {
        if (query_error) *query_error = ERROR_INVALID_ADDRESS;
        return TargetWriteRoute::Reject;
    }
    if (local_memory.Type == MEM_PRIVATE) {
        return TargetWriteRoute::VtPrivate;
    }
    if (local_memory.Type != MEM_IMAGE && local_memory.Type != MEM_MAPPED) {
        if (query_error) *query_error = ERROR_NOT_SUPPORTED;
        return TargetWriteRoute::Reject;
    }

    bool local_known = false;
    bool local_shared = true;
    DWORD working_set_error = ERROR_SUCCESS;
    (void)QueryTargetPageShared(
        process, address, &local_known, &local_shared, &working_set_error);
    if (working_set_known) *working_set_known = local_known;
    if (shared) *shared = local_shared;
    if (query_error) *query_error = working_set_error;
    if (local_known && !local_shared) {
        return TargetWriteRoute::VtPrivate;
    }
    if (local_memory.Type == MEM_MAPPED &&
        !IsWriteCopyProtection(local_memory.Protect)) {
        if (query_error) *query_error = ERROR_SHARING_VIOLATION;
        return TargetWriteRoute::Reject;
    }
    return TargetWriteRoute::CowMaterialize;
}

bool VerifyTargetPagePrivate(
    HANDLE process,
    ULONG_PTR address,
    DWORD* query_error)
{
    ...
    const bool queried = QueryTargetPageShared(
        process, address, &known, &shared, &error);
    if (!queried || !known || shared) {
        if (query_error) {
            *query_error = error != ERROR_SUCCESS
                ? error
...
bool DriverWriteMemory(
    ...
    SIZE_T* transferred)
{
    const auto sequence = g_write_sequence.fetch_add(1) + 1;
    SIZE_T total = 0;
    const ULONG capabilities = g_granted_capabilities.load();
    ...
    while (total < size) {
        const ULONG_PTR current_address =
            reinterpret_cast<ULONG_PTR>(address) + total;
        const SIZE_T page_remaining = 0x1000u - (current_address & 0xFFFu);
        const ULONG chunk = static_cast<ULONG>(std::min<SIZE_T>(
            std::min<SIZE_T>(size - total, HV_BRIDGE_MEMORY_MAX_PAYLOAD),
            page_remaining));

        MEMORY_BASIC_INFORMATION memory = {};
        bool working_set_known = false;
        bool shared = true;
        DWORD classify_error = ERROR_SUCCESS;
        const TargetWriteRoute route = ClassifyTargetWritePage(
            process, current_address, chunk, &memory,
            &working_set_known, &shared, &classify_error);
        Trace(L"[WRITE #%llu] classify pid=%lu addr=%p chunk=%lu total=%llu route=%s type=%s protect=0x%08lX ws_known=%d shared=%d win32=%lu\n",
              sequence, pid, reinterpret_cast<PVOID>(current_address), chunk,
              static_cast<unsigned long long>(total),
              TargetWriteRouteName(route), TargetMemoryTypeName(memory.Type),
              memory.Protect, working_set_known, shared, classify_error);
        if (route == TargetWriteRoute::Reject) {
            if (transferred) *transferred = total;
            SetLastError(classify_error != ERROR_SUCCESS
                ? classify_error
                : ERROR_NOT_SUPPORTED);
            return false;
        }
        ...
        std::vector<unsigned char> input(
            sizeof(HV_BRIDGE_MEMORY_REQUEST) + chunk);
        auto* request =
            reinterpret_cast<HV_BRIDGE_MEMORY_REQUEST*>(input.data());
        request->ProcessId = pid;
        request->Address = current_address;
        request->Size = chunk;

        const auto send_write = [&](DWORD code, LPCVOID payload,
                                    HV_BRIDGE_MEMORY_RESULT* result,
                                    DWORD* returned, DWORD* ioctl_error) {
            std::memcpy(input.data() + sizeof(*request), payload, chunk);
            *result = {};
            *returned = 0;
            const BOOL ioctl_ok = DeviceIoControl(
                g_device, code,
                ...
                result, sizeof(*result), returned, nullptr);
            *ioctl_error = ioctl_ok ? ERROR_SUCCESS : GetLastError();
            return ioctl_ok && *returned >= sizeof(*result);
        };
        const auto result_complete = [&](bool request_ok,
                                         const HV_BRIDGE_MEMORY_RESULT& result) {
            return request_ok && IsNtSuccess(result.Status) &&
                result.BytesTransferred == chunk;
        };
        const auto trace_result = [&](const wchar_t* phase, bool request_ok,
                                      const HV_BRIDGE_MEMORY_RESULT& result,
                                      DWORD returned, DWORD ioctl_error) {
            Trace(L"[WRITE #%llu] %s pid=%lu addr=%p chunk=%lu ok=%d status=0x%08X copied=%lu stage=%lu(%s) detail=0x%08X returned=%lu win32=%lu\n",
                  sequence, phase, pid,
                  reinterpret_cast<PVOID>(current_address), chunk, request_ok,
                  static_cast<unsigned long>(result.Status),
                  std::min(result.BytesTransferred, chunk),
                  result.Reserved0, CopyStageName(result.Reserved0),
                  result.Reserved1, returned, ioctl_error);
        };
        const auto send_native_write = [&](const wchar_t* phase, LPCVOID payload,
                                           HV_BRIDGE_MEMORY_RESULT* result,
                                           bool* request_ok, ULONG* copied,
                                           DWORD* ioctl_error) {
            SIZE_T native_written = 0;
            const BOOL native_ok = g_write_process_memory &&
                g_write_process_memory(
                    process, reinterpret_cast<LPVOID>(current_address),
                    payload, chunk, &native_written);
            const DWORD native_error = native_ok ? ERROR_SUCCESS : GetLastError();
            Trace(L"[WRITE #%llu] %s pid=%lu addr=%p chunk=%lu ok=%d copied=%llu win32=%lu\n",
                  sequence, phase, pid,
                  reinterpret_cast<PVOID>(current_address), chunk, native_ok,
                  static_cast<unsigned long long>(native_written), native_error);
            if (native_ok && native_written == chunk) {
                *result = {};
                result->Status = 0;
                ...
                *request_ok = true;
                *copied = chunk;
                *ioctl_error = ERROR_SUCCESS;
                return true;
            }
            *request_ok = false;
            *copied = static_cast<ULONG>(std::min<SIZE_T>(native_written, chunk));
            *ioctl_error = native_error != ERROR_SUCCESS
                ? native_error
                : ERROR_PARTIAL_COPY;
            return false;
        };
        ...
        HV_BRIDGE_MEMORY_RESULT result = {};
        DWORD returned = 0;
        DWORD ioctl_error = ERROR_SUCCESS;
        bool request_ok = false;
        ULONG copied = 0;

        if (route == TargetWriteRoute::CowMaterialize) {
            if (!allow_cow || !allow_vt_write) {
                Trace(L"[WRITE #%llu] shared image page lacks required cow+vt capabilities pid=%lu addr=%p cow=%d vt=%d\n",
                      sequence, pid, reinterpret_cast<PVOID>(current_address),
                      allow_cow, allow_vt_write);
                if (transferred) *transferred = total;
                SetLastError(ERROR_NOT_SUPPORTED);
                return false;
            }

            std::vector<unsigned char> original(chunk);
            SIZE_T original_read = 0;
            if (!DriverReadMemory(
                    pid, reinterpret_cast<LPCVOID>(current_address),
                    original.data(), chunk, &original_read) ||
                original_read != chunk) {
                const DWORD error = GetLastError();
                Trace(L"[WRITE #%llu] materialize read failed pid=%lu addr=%p chunk=%lu copied=%llu win32=%lu\n",
                      sequence, pid, reinterpret_cast<PVOID>(current_address),
                      chunk, static_cast<unsigned long long>(original_read), error);
                if (transferred) *transferred = total;
                SetLastError(error != ERROR_SUCCESS ? error : ERROR_PARTIAL_COPY);
                return false;
            }

            request_ok = send_write(
                IOCTL_HV_BRIDGE_MEMORY_WRITE_COW, original.data(),
                &result, &returned, &ioctl_error);
            trace_result(L"cow-materialize", request_ok, result,
                         returned, ioctl_error);
            if (!result_complete(request_ok, result)) {
                ULONG materialized = 0;
                if (!send_native_write(
                        L"native-cow-materialize", original.data(),
                        &result, &request_ok, &materialized, &ioctl_error)) {
                    if (transferred) *transferred = total;
                    SetLastError(ioctl_error != ERROR_SUCCESS
                        ? ioctl_error
                        : ERROR_PARTIAL_COPY);
            ...
            DWORD verify_error = ERROR_SUCCESS;
            if (!VerifyTargetPagePrivate(
                    process, current_address, &verify_error)) {
                Trace(L"[WRITE #%llu] cow verification rejected shared page pid=%lu addr=%p type=%s protect=0x%08lX win32=%lu\n",
                      sequence, pid, reinterpret_cast<PVOID>(current_address),
                      TargetMemoryTypeName(memory.Type), memory.Protect,
                      verify_error);
                if (transferred) *transferred = total;
                SetLastError(verify_error != ERROR_SUCCESS
                    ? verify_error
                    : ERROR_SHARING_VIOLATION);
                return false;
            }
            ...
            request_ok = send_write(
                IOCTL_HV_BRIDGE_MEMORY_WRITE,
                static_cast<const unsigned char*>(buffer) + total,
                &result, &returned, &ioctl_error);
            trace_result(L"vt-after-cow", request_ok, result,
                         returned, ioctl_error);
            copied = request_ok
                ? std::min(result.BytesTransferred, chunk)
                : 0;
            if (!result_complete(request_ok, result)) {
                request_ok = send_write(
                    IOCTL_HV_BRIDGE_MEMORY_WRITE_COW,
                    static_cast<const unsigned char*>(buffer) + total,
                    &result, &returned, &ioctl_error);
                trace_result(L"private-cow-fallback", request_ok, result,
                             returned, ioctl_error);
                copied = request_ok
                    ? std::min(result.BytesTransferred, chunk)
                    : copied;
            ...
            if (allow_vt_write) {
                request_ok = send_write(
                    ...
                    static_cast<const unsigned char*>(buffer) + total,
                    &result, &returned, &ioctl_error);
                trace_result(L"vt-private", request_ok, result,
                             returned, ioctl_error);
                copied = request_ok
                    ? std::min(result.BytesTransferred, chunk)
                    : 0;
            }
            if (!result_complete(request_ok, result) && allow_cow) {
                request_ok = send_write(
                    IOCTL_HV_BRIDGE_MEMORY_WRITE_COW,
                    static_cast<const unsigned char*>(buffer) + total,
                    &result, &returned, &ioctl_error);
        ...
        if (!result_complete(request_ok, result)) {
            (void)send_native_write(
                L"verified-private-native-fallback",
                static_cast<const unsigned char*>(buffer) + total,
                &result, &request_ok, &copied, &ioctl_error);
        }

        if (!result_complete(request_ok, result)) {
            if (transferred) *transferred = total + copied;
            if (!request_ok) {
                SetLastError(ioctl_error != ERROR_SUCCESS
                    ? ioctl_error
                    : ERROR_INVALID_DATA);
            } else if (!IsNtSuccess(result.Status)) {
                SetLastErrorFromNtStatus(result.Status);
            } else {
                SetLastError(ERROR_PARTIAL_COPY);
            }
    ...
    if (transferred) *transferred = total;
    Trace(L"[WRITE #%llu] complete pid=%lu addr=%p size=%llu transferred=%llu\n",
          sequence, pid, address,
          static_cast<unsigned long long>(size),
          static_cast<unsigned long long>(total));
    return true;
...
bool LookupDebugState(DWORD pid, DWORD tid, VirtualDebugState* state)
{
    AcquireSRWLockShared(&g_debug_state_lock);
    const auto found = g_debug_states.find(PrivateEventKey(pid, tid));
...
void LatchWindowsDebugEventDr6(const DEBUG_EVENT* event)
{
    if (!event || event->dwProcessId == 0 || event->dwThreadId == 0 ||
        event->dwDebugEventCode != EXCEPTION_DEBUG_EVENT ||
        event->u.Exception.ExceptionRecord.ExceptionCode !=
            EXCEPTION_SINGLE_STEP ||
        (g_granted_capabilities.load() & HV_BRIDGE_CAP_VT_HWBP) == 0 ||
        !IsBoundTarget(event->dwProcessId)) {
        return;
    }
    ...
    std::uint64_t configured_mask = 0;
    for (ULONG slot = 0; slot < 4; ++slot) {
        if (state.Dr[slot] != 0 &&
            ((state.Dr7 >> (slot * 2)) & 3ULL) != 0) {
            configured_mask |= 1ULL << slot;
        }
    }
    if (configured_mask == 0) return;

    std::uint64_t pending_dr6 = 0;
    std::uint64_t pending_generation = 0;
    const bool pending_ok = SendPendingHardwareHitOperation(
        ...
        &pending_dr6,
        &pending_generation);
    const DWORD pending_error = pending_ok ? ERROR_SUCCESS : GetLastError();
    ...
    const BOOL context_ok = thread && g_get_thread_context &&
        g_get_thread_context(thread, &context);
    const DWORD context_error = context_ok ? ERROR_SUCCESS : GetLastError();
    if (thread) CloseHandle(thread);

    const std::uint64_t raw_dr6 = context_ok
        ? static_cast<std::uint64_t>(context.Dr6) & 0xFULL
        : 0;
    const std::uint64_t virtual_dr6 =
        (pending_dr6 | raw_dr6) & configured_mask;
    if (virtual_dr6 != 0) {
        PendingWindowsDebugEvent pending = {};
        pending.VirtualDr6 = virtual_dr6;
        pending.Generation = pending_ok ? pending_generation : 0;
        AcquireSRWLockExclusive(&g_private_event_lock);
        g_pending_windows_debug_events[
            PrivateEventKey(event->dwProcessId, event->dwThreadId)] = pending;
        ReleaseSRWLockExclusive(&g_private_event_lock);
    }

    Trace(L"[NATIVE-WAIT] single-step pid=%lu tid=%lu pending_ok=%d pending_win32=%lu pending_dr6=0x%llX generation=%llu context_ok=%d context_win32=%lu raw_dr6=0x%llX configured=0x%llX virtual_dr6=0x%llX\n",
          event->dwProcessId, event->dwThreadId,
          pending_ok, pending_error,
          static_cast<unsigned long long>(pending_dr6),
          static_cast<unsigned long long>(pending_generation),
          context_ok, context_error,
          static_cast<unsigned long long>(raw_dr6),
          static_cast<unsigned long long>(configured_mask),
          static_cast<unsigned long long>(virtual_dr6));
}

bool CommitDebugState(
    DWORD pid,
    DWORD tid,
    const VirtualDebugState& state)
{
    ...
    const bool had_previous = LookupDebugState(pid, tid, &previous);
    if (!ApplyHardwareStateDelta(
            pid,
            tid,
    ...
    AcquireSRWLockExclusive(&g_debug_state_lock);
    g_debug_states[PrivateEventKey(pid, tid)] = state;
    ReleaseSRWLockExclusive(&g_debug_state_lock);
    ReleaseSRWLockExclusive(&g_hwbp_commit_lock);
    return true;
}
...
void* ReplacementForName(const char* name)
{
    ...
    if (strcmp(name, "SetThreadContext") == 0) return reinterpret_cast<void*>(&BridgeSetThreadContextRealTf);
#ifdef _WIN64
    if (strcmp(name, "Wow64GetThreadContext") == 0) return reinterpret_cast<void*>(&BridgeWow64GetThreadContext);
    if (strcmp(name, "Wow64SetThreadContext") == 0) return reinterpret_cast<void*>(&BridgeWow64SetThreadContextRealTf);
#endif
...
void* ReplacementForAddress(void* address)
{
    ...
    if (address == reinterpret_cast<void*>(g_set_thread_context)) return reinterpret_cast<void*>(&BridgeSetThreadContextRealTf);
#ifdef _WIN64
    if (address == reinterpret_cast<void*>(g_wow64_get_thread_context)) return reinterpret_cast<void*>(&BridgeWow64GetThreadContext);
    if (address == reinterpret_cast<void*>(g_wow64_set_thread_context)) return reinterpret_cast<void*>(&BridgeWow64SetThreadContextRealTf);
#endif
...
BOOL WINAPI BridgeContinueDebugEvent(DWORD pid, DWORD tid, DWORD status)
{
    ...
    if (private_dbgk != g_pending_private_dbgk_events.end()) {
        ...
        const bool exit_process =
            pending.State == HV_BRIDGE_DBGK_EXIT_PROCESS;
        const HV_BRIDGE_DBGK_EVENT event = [&]() {
        ...
        if (ok && exit_process) {
            const bool unbound = UnbindTarget(pid);
            Trace(L"[DBGK-EXIT] pid=%lu tid=%lu unbound=%d win32=%lu\n",
                  pid, tid, unbound,
                  unbound ? ERROR_SUCCESS : GetLastError());
        }
    ...
    const DWORD error = ok ? ERROR_SUCCESS : GetLastError();
    PendingWindowsDebugEvent retired_event = {};
    const bool retired_windows_event = ok &&
        RetirePendingWindowsDebugEvent(pid, tid, &retired_event);
    bool retired_driver_hit = false;
    DWORD retire_error = ERROR_SUCCESS;
    if (retired_windows_event && retired_event.Generation != 0) {
        retired_driver_hit = SendPendingHardwareHitOperation(
            HV_BRIDGE_PENDING_HWBP_RETIRE,
            pid,
            tid,
            retired_event.Generation,
            nullptr,
            nullptr);
        retire_error = retired_driver_hit ? ERROR_SUCCESS : GetLastError();
    }
    const auto sequence = g_continue_sequence.fetch_add(1) + 1;
    if (sequence <= 512 || !ok) {
    ...
    if (retired_windows_event) {
        Trace(L"[NATIVE-CONTINUE] pid=%lu tid=%lu virtual_dr6=0x%llX generation=%llu retire_ok=%d retire_win32=%lu\n",
              pid, tid,
              static_cast<unsigned long long>(retired_event.VirtualDr6),
              static_cast<unsigned long long>(retired_event.Generation),
              retired_event.Generation == 0 || retired_driver_hit,
              retire_error);
    }
    if (!ok) SetLastError(error);
    return ok;
    ...
    if (bound) {
        ...
        if ((capabilities & (HV_BRIDGE_CAP_MEMORY_IO |
            ...
            const BOOL result = DriverWriteMemory(
                process, pid, address, buffer, size, transferred) ? TRUE : FALSE;
            ReleaseSRWLockShared(&g_target_lock);
            return result;
        }
    }
...
BOOL WINAPI BridgeGetThreadContext(HANDLE thread, LPCONTEXT context)
{
    ...
    if (ok &&
        (requested & CONTEXT_DEBUG_REGISTERS) == CONTEXT_DEBUG_REGISTERS &&
        pid && tid) {
        context->Dr6 |= static_cast<DWORD_PTR>(
            LookupPendingDebugEventDr6(pid, tid));
    }
    const auto sequence = g_context_sequence.fetch_add(1) + 1;
    if (sequence <= 256) {
#ifdef _WIN64
        Trace(L"[CONTEXT #%llu] get thread=%p pid=%lu flags=0x%08lX ok=%d win32=%lu ip=0x%llX sp=0x%llX dr6=0x%llX dr7=0x%llX\n",
              sequence, thread, pid, requested,
    ...
    VirtualDebugState state = {};
    if (pid && tid && IsBoundTarget(pid) &&
        LookupDebugState(pid, tid, &state)) {
...
BOOL WINAPI BridgeSetThreadContextRealTf(HANDLE thread, const CONTEXT* context)
{
    ...
    if (!bound || !has_debug_registers) {
        const BOOL ok = g_set_thread_context(thread, context);
        const DWORD error = ok ? ERROR_SUCCESS : GetLastError();
        ReleaseSRWLockShared(&g_target_lock);
        if (sequence <= 256 || !ok) {
            Trace(L"[CONTEXT #%llu] set-real-tf pid=%lu tid=%lu tf=%d ok=%d win32=%lu\n",
                  sequence, pid, tid, (context->EFlags & 0x100u) != 0,
                  ok, error);
        }
        if (!ok) SetLastError(error);
        return ok;
    ...
    if (!g_set_thread_context(thread, &sanitized)) {
        const DWORD error = GetLastError();
        ReleaseSRWLockShared(&g_target_lock);
        Trace(L"[CONTEXT #%llu] set-real-tf-sanitized failed pid=%lu tid=%lu win32=%lu\n",
    ...
    const BOOL committed = CommitDebugState(pid, tid, state) ? TRUE : FALSE;
    const DWORD error = committed ? ERROR_SUCCESS : GetLastError();
    if (committed && tid) {
        UpdatePendingDebugEventDr6(pid, tid, context->Dr6);
    }
    ReleaseSRWLockShared(&g_target_lock);
    Trace(L"[CONTEXT #%llu] set-real-tf-virtual pid=%lu tid=%lu tf=%d dr0=0x%llX dr1=0x%llX dr2=0x%llX dr3=0x%llX dr7=0x%llX committed=%d\n",
          sequence, pid, tid, (context->EFlags & 0x100u) != 0,
          static_cast<unsigned long long>(context->Dr0),
          static_cast<unsigned long long>(context->Dr1),
          static_cast<unsigned long long>(context->Dr2),
          static_cast<unsigned long long>(context->Dr3),
          static_cast<unsigned long long>(context->Dr7),
          committed);
...
BOOL WINAPI BridgeWow64GetThreadContext(HANDLE thread, PWOW64_CONTEXT context)
{
    ...
    const DWORD tid = GetThreadId(thread);
    if (ok &&
        (requested & WOW64_CONTEXT_DEBUG_REGISTERS) ==
            WOW64_CONTEXT_DEBUG_REGISTERS &&
        pid && tid) {
        context->Dr6 |= static_cast<DWORD>(
            LookupPendingDebugEventDr6(pid, tid));
    }
    if (ok &&
...
BOOL WINAPI BridgeWow64SetThreadContextRealTf(
    HANDLE thread,
    const WOW64_CONTEXT* context)
{
    ...
    if (!bound || !has_debug_registers) {
        const BOOL ok = g_wow64_set_thread_context(thread, context);
        const DWORD error = ok ? ERROR_SUCCESS : GetLastError();
        ReleaseSRWLockShared(&g_target_lock);
        if (sequence <= 256 || !ok) {
            Trace(L"[CONTEXT #%llu] wow64-set-real-tf pid=%lu tid=%lu tf=%d ok=%d win32=%lu\n",
                  sequence, pid, tid, (context->EFlags & 0x100u) != 0,
                  ok, error);
        }
        if (!ok) SetLastError(error);
    ...
    if (!g_wow64_set_thread_context(thread, &sanitized)) {
        const DWORD error = GetLastError();
        ReleaseSRWLockShared(&g_target_lock);
        Trace(L"[CONTEXT #%llu] wow64-set-real-tf-sanitized failed pid=%lu tid=%lu win32=%lu\n",
              sequence, pid, tid, error);
        SetLastError(error);
        return FALSE;
    }
    ...
    ReleaseSRWLockShared(&g_target_lock);
    Trace(L"[CONTEXT #%llu] wow64-set-real-tf-virtual pid=%lu tid=%lu tf=%d dr0=0x%08lX dr1=0x%08lX dr2=0x%08lX dr3=0x%08lX dr7=0x%08lX committed=%d\n",
          sequence, pid, tid, (context->EFlags & 0x100u) != 0,
          context->Dr0, context->Dr1, context->Dr2, context->Dr3,
          context->Dr7, committed);
    if (!committed) SetLastError(error);
    return committed;
}
