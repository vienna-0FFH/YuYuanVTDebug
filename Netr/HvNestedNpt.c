 ...
 * 读取 L1 NPT 条目
 */
static BOOLEAN NestedNptReadL1Entry(ULONG64 EntryPhysical, PULONG64 Entry)
{
    PVOID mapped;
    PHYSICAL_ADDRESS pa;
...
NTSTATUS NestedNptInitialize(VOID)
{
    ...
    DbgPrint("[HV-NESTED-NPT] Nested NPT manager initialized\n");
...
PNESTED_NPT_CONTEXT NestedNptGetOrCreateContext(ULONG64 L1NCr3)
{
    ...
    context = NestedNptFindContext(L1NCr3);
    if (context) {
        InterlockedIncrement(&context->RefCount);
        return context;
    }
    
    return NestedNptCreateContext(L1NCr3);
}
    ...
    PULONG PageSize)
{
    ...
    // 读取 PML4E
    if (!NestedNptReadL1Entry(pml4Pa + pml4Index * 8, &pml4e)) {
        return FALSE;
    }
    ...
    if (!NestedNptReadL1Entry(pdptPa + pdptIndex * 8, &pdpte)) {
        return FALSE;
    }
    ...
    if (!NestedNptReadL1Entry(pdPa + pdIndex * 8, &pde)) {
        return FALSE;
    }
    ...
    if (!NestedNptReadL1Entry(ptPa + ptIndex * 8, &pte)) {
        return FALSE;
    }
    ...
    PNESTED_NPT_TRANSLATION Result)
{
    ...
    // 构建 NPT02 映射
    NestedNptBuildMapping(Context, L2Gpa & ~((ULONG64)l1PageSize - 1), 
                          hpa & ~((ULONG64)l1PageSize - 1),
                          Result->MergedPermissions, l1PageSize);
...
static PVOID NestedNptAllocateTablePageLocked(PNESTED_NPT_CONTEXT Context, PPHYSICAL_ADDRESS PhysicalAddress)
{
    ...
    page = NestedNptAllocatePage(PhysicalAddress);
    if (!page) {
        return NULL;
    }
...
cleanup:
    // 取消映射已映射的页表页
    if (ptMapped && pt) MmUnmapIoSpace(pt, PAGE_SIZE);
    if (pdMapped && pd) MmUnmapIoSpace(pd, PAGE_SIZE);
    if (pdptMapped && pdpt) MmUnmapIoSpace(pdpt, PAGE_SIZE);
    ...
    PBOOLEAN InjectToL1)
{
    ...
    *InjectToL1 = FALSE;
    ...
    context = NestedNptGetOrCreateContext(VcpuData->NestedSvm.L1NCr3);
    if (!context) {
        *InjectToL1 = TRUE;
        return TRUE;
    }
