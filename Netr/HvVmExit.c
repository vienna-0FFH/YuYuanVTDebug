...
// EPT VMX root 模式标志
extern VOID EptSetVmxRootMode(BOOLEAN InRootMode);
...
static BOOLEAN HvHandleVmcall(PGUEST_CONTEXT ctx)
{
    ...
    case VMCALL_PHYS_COPY: {
        // Phase C: 真·VT 无痕物理 R/W (by PID) + GetPeb 辅助服务
        //   ctx->Rdx = target PID (低 32 位有效)
        //   ctx->R8  = target GVA      (mode=R/W) | 0 (mode=GetPeb)
        //   ctx->R9  = kernel buf      (mode=R/W) | 0
        //   ctx->R10 = size            (mode=R/W) | 0  → 写回 bytesDone 或 peb_va
        //   ctx->R11 = mode:           0=read, 1=write, 2=GetPeb
        //
        // 与旧路径区别:RDX 现在传 PID 而不是 CR3。EPROCESS 链表遍历 +
        // CR3 候选收集在 root mode 内完成, hot path 不调任何 Mm*/Ps*。
        PVCPU_DATA vcpu = HvNestedGetCurrentVcpu();
        ULONG mode = (ULONG)(ctx->R11 & 0xFFFFFFFFULL);
        ULONG pid  = (ULONG)(ctx->Rdx & 0xFFFFFFFFULL);
        NTSTATUS s;

        if (mode == HV_VTROOT_MODE_GET_PEB) {
            UINT64 pebVa = 0;
            s = HvVtRootRootGetPebByPid(pid, &pebVa);
            ctx->R10 = pebVa;          // 写回 PEB user VA (经 BytesDone 出口)
        } else if (mode == HV_VTROOT_MODE_RESOLVE_CR3) {
            // R10 出 = 真 user CR3
            UINT64 realCr3 = 0;
            s = HvVtRootRootResolveUserCr3(pid, &realCr3);
            ctx->R10 = realCr3;
        } else if (mode == HV_VTROOT_MODE_GVA_TO_HPA) {
            // R8 = GVA, R10 出 = HPA (页大小编码在 NTSTATUS 低位: 见 Cloak)
            // 实际上 cloak 只关心 4KB; 2MB/1GB 时直接以 STATUS_NOT_SUPPORTED 返回。
            UINT64 hpa = 0, pageSize = 0;
            s = HvVtRootRootWalkGvaToHpa(vcpu, pid, ctx->R8, &hpa, &pageSize);
            if (NT_SUCCESS(s)) {
                if (pageSize == PAGE_SIZE) {
                    ctx->R10 = hpa;
                } else {
                    // 2MB/1GB - 不参与 cloak (返特殊 status, R10 = pageSize)
                    ctx->R10 = pageSize;
    ...
    default:
        // P118+: HWBP 透明路径已废 + #DB 拦截全局关闭. 这个 VMCALL 现在是 no-op.
        //   保留 case 不让 caller (HvDbgEnableInterceptsAllCpus) 报错.
        if ((ctx->Rcx & ~1ULL) == VMCALL_TOGGLE_HWBP_INTERCEPTS) {
            ctx->Rax = 0;
            break;
        }
        // 未知 VMCALL
...
static VOID HvHandleCrAccess(PGUEST_CONTEXT ctx)
{
    ...
    if (accessType == 0) {
        ...
        case 0:
            ...
            break;
        case 3:
            __vmx_vmwrite(GUEST_CR3, *gprPtr);
            // P118: HWBP 透明路径已废, 不在 CR3 hot path 装载 DR;
            //       GUI 通过 SetThreadContext 直接给目标线程下发, 由 Windows 调度器装载
            break;
        case 4: {
 ...
 */
static VOID HvHandleMovDr(PGUEST_CONTEXT ctx)
{
    SIZE_T qual = 0;
    ...
    BOOLEAN isProtectedTarget = HvHookIsProtectedProcessPid(pid);
    ...
    if (isRead) {
        ULONG64 value = isProtectedTarget ? 0ULL : HvDbgReadRealDr(drNum);
        switch (gprIdx) {
        case 0:  ctx->Rax = value; break;
        case 1:  ctx->Rcx = value; break;
        case 2:  ctx->Rdx = value; break;
 ...
 */
static VOID HvHandleExternalInterrupt(VOID)
{
    SIZE_T intrInfo;
    ULONG vector;
    ULONG cpuIndex;
 ...
 */
static VOID HvHandleInterruptWindow(VOID)
{
    ULONG cpuIndex;
    ULONG64 pendingIntr;
    ULONG vector;
 ...
 * 处理异常/NMI
 */
static BOOLEAN HvHandleExceptionNmi(VOID)
{
    ...
    if (vector == 3) {
        ...
        HANDLE debuggerPid = HvDbgLookupSwBpOwner(pid, (UINT64)rip);
        if (debuggerPid == NULL) {
        ...
        // 不重新注入 → guest 不会进自己 #BP handler。
        // 这里要做的是: 阻止下一条指令跑 (让线程挂起在断点位置)。
        // 方案: 用 NtSuspendThread 兼容? 不能,要从 root 调 — 不安全。
        // 简化: 让 vmexit 后 guest 继续 (RIP 不前进,sw bp 字节仍是 0xCC),
        // 下一轮调度到该 thread 会立刻再 vmexit。GUI 收到事件后 R3 调
        // SuspendThread + 在事件回调里清掉 0xCC (临时), 改成"运行到断点
        // 后是否继续"由用户在 GUI 决定。
        // 当前: 让线程自旋一会儿等 GUI 反应 (~ms 级,GUI 收到事件马上发
        // SuspendThread + clear breakpoint 即可)。
        return TRUE;
    }

    // P118+: #DB (vector=1) — 完全透传给 guest, 不在 hypervisor 里解析 / 拦截.
    //
    //   以前: 读 host DR6, 查 g_HwbpList 看是不是我们的断点, 是 → 上报 ring 吞掉,
    //   否 → vm-entry inject 还原。
    //
    //   问题:
    //     1) __readdr(6) 是 host DR6, vmexit 后可能被 clobber, 跟 guest 触发时不一致
    //     2) vm-entry inject #DB 时如果不设 GUEST_PENDING_DEBUG_EXCEPTIONS,
    //        guest IDT[1] handler 读 DR6 拿到的可能是错的 (Intel SDM 27.6.4)
    //     3) 用户场景: 外部 debugger (CE) 自己用 user-mode VEH 处理 #DB,
    //        我们参与解析只会搞乱
    //
    //   正确做法 (Intel SDM 26.2.1.3 + 27.6.4):
    //     - 从 VM_EXIT_QUALIFICATION 拿 #DB 命中位 (硬件已保存, 不动)
    //     - 写 GUEST_PENDING_DEBUG_EXCEPTIONS = 该 qualification 值
    //     - 设 entry interruption info 注入 #DB (vector=1, type=3, valid=1)
    //     - vm-entry 后硬件按"pending debug exceptions"模式派发, guest 进自己 IDT[1],
    //       第一条指令前抛 #DB, DR6 准确反映命中位
    if (type == 3 && vector == 1) {
        SIZE_T qual = 0;
        __vmx_vmread(VM_EXIT_QUALIFICATION, &qual);

        // 写 pending debug exceptions — Intel SDM 24.4.2 / 27.6.4
        //   bits 3:0 = B0..B3 (HWBP 命中)
        //   bit 12   = enabled breakpoint
        //   bit 14   = BS (single step)
        //   其余位必须 0
        __vmx_vmwrite(GUEST_PENDING_DEBUG_EXCEPTIONS, qual);
...
static BOOLEAN HvHandleEptViolation(PGUEST_CONTEXT GuestContext)
{
    ...
    // 尝试用 EPT Hook 处理
    if (EptHookHandleViolation((ULONG64)gpa, (ULONG64)qualification)) {
        return TRUE;
    }

    // P125 (2026-06-25): PEB 字段级 spoof — 在 EptPebSpoof 上 PEB 页 R=0,
    // target 自己读 PEB 触发 violation → 装补丁页 (3 字段被 0) + R=1 + MTF.
    if (HvPebCloakHandleEptViolation((ULONG64)gpa, (ULONG64)qualification, GuestContext)) {
        return TRUE;
    }
    ...
    if (HvVwatchHandleEptViolation((ULONG64)gpa, (ULONG64)qualification, GuestContext)) {
        return TRUE;
    }
        ...
        if (vcpuData && vcpuData->EptTables) {
            ...
            if (pml4Idx == 0 && pdptIdx < 512 && pdIdx < 512) {
                ...
                if (pde->Value & (1ULL << 7)) {
                    ...
                    PEPT_PTE pt = NULL;
                    for (ULONG i = 0; i < lookupTables->SplitPtCount; i++) {
                        if ((ULONG64)(lookupTables->SplitPtPhysical[i].QuadPart >> 12) == ptPfn) {
                            pt = lookupTables->SplitPt[i];
                            break;
                        }
                    }
                    if (pt) {
...
BOOLEAN HvVmExitDispatch(PGUEST_CONTEXT GuestContext)
{
    ...
    case EXIT_REASON_EXTERNAL_INTERRUPT:  // 1
        HvHandleExternalInterrupt();
        break;

    case EXIT_REASON_EXCEPTION_NMI:  // 0
        shouldContinue = HvHandleExceptionNmi();
        break;

    case 7:  // INTERRUPT_WINDOW
        HvHandleInterruptWindow();
        break;
    ...
    case EXIT_REASON_MONITOR_TRAP_FLAG:  // 37
        // Item 2: xHCI USBSTS/IMAN trap 也用 MTF (单步过读指令拿到真值后 OR 进
        // 假位)。先让 xHCI trap 处理,handled 则跳过 EptHook 的 MTF 路径。
        // P125/P128: MTF 退出顺序 Xhci → EptHook → PebCloak → Vwatch.
        // 每个 handler 内部检查 per-CPU MTF context, 只处理自己 arm 的那一次.
        if (!HvXhciEptTrapHandleMtf(GuestContext)) {
            EptHookHandleMtfExit();
            HvPebCloakHandleMtfExit(GuestContext);
            HvVwatchHandleMtfExit(GuestContext);
        }
        break;
            ...
            if (!(entryInfo & (1ULL << 31))) {
                ...
                if (vecType == 0 && vecNum != 2) {
                    if (!HvCanInjectInterrupt()) {
                        ...
                        ULONG cpuIdx2 = KeGetCurrentProcessorNumber();
 ...
 * 返回：TRUE = 继续执行，FALSE = 终止虚拟化
 */
BOOLEAN SvmHandleVmmcall(PVMCB Vmcb, PGUEST_CONTEXT GuestContext)
{
    ...
    case VMCALL_PHYS_COPY: {
        // Phase C: 真·VT 无痕物理 R/W (SVM 平行路径, by PID) + GetPeb
        //   Rdx = target PID
        //   R8  = target GVA (mode=R/W) | 0 (mode=GetPeb)
        //   R9  = kernel buf
        //   R10 = size       →  返回 bytesDone 或 peb_va
        //   R11 = mode: 0=read, 1=write, 2=GetPeb
        PVCPU_DATA vcpu = HvNestedGetCurrentVcpu();
        ULONG mode = (ULONG)(GuestContext->R11 & 0xFFFFFFFFULL);
        ULONG pid  = (ULONG)(GuestContext->Rdx & 0xFFFFFFFFULL);
        NTSTATUS s;

        if (mode == HV_VTROOT_MODE_GET_PEB) {
            UINT64 pebVa = 0;
            s = HvVtRootRootGetPebByPid(pid, &pebVa);
            GuestContext->R10 = pebVa;
        } else if (mode == HV_VTROOT_MODE_RESOLVE_CR3) {
            UINT64 realCr3 = 0;
            s = HvVtRootRootResolveUserCr3(pid, &realCr3);
            GuestContext->R10 = realCr3;
        } else if (mode == HV_VTROOT_MODE_GVA_TO_HPA) {
            UINT64 hpa = 0, pageSize = 0;
            s = HvVtRootRootWalkGvaToHpa(vcpu, pid, GuestContext->R8, &hpa, &pageSize);
            if (NT_SUCCESS(s)) {
                if (pageSize == PAGE_SIZE) {
    ...
    default:
        // P118+: HWBP 透明路径已废 + #DB 拦截全局关闭. no-op.
        if ((command & ~1ULL) == VMCALL_TOGGLE_HWBP_INTERCEPTS) {
            Vmcb->StateSaveArea.Rax = 0;
            break;
        }
        // 未知 VMMCALL
 ...
 */
VOID SvmHandleMsr(PVMCB Vmcb, PGUEST_CONTEXT GuestContext, ULONG64 ExitInfo1)
{
 ...
 * 处理 CR 访问 (SVM)
 */
VOID SvmHandleCrAccess(PVMCB Vmcb, PGUEST_CONTEXT GuestContext, ULONG64 ExitCode, ULONG64 ExitInfo1)
{
    ULONG crNum;
    BOOLEAN isWrite;
    ULONG64 value;
    ...
    if (isWrite) {
        // MOV to CR
        // ExitInfo1 包含 GPR 索引和新值
        value = ExitInfo1;  // 简化处理
        switch (crNum) {
        case 0:
            Vmcb->StateSaveArea.Cr0 = value;
            break;
        case 3:
            Vmcb->StateSaveArea.Cr3 = value;
            // P118: 见 Intel 分支, HWBP 透明路径已废
            break;
        case 4:
...
static VOID SvmHandleMovDr(PVMCB Vmcb, PGUEST_CONTEXT GuestContext, ULONG64 ExitCode, ULONG64 ExitInfo1)
{
    ...
    if (!isWrite) {
        ...
        // 写回到对应 GPR
        // 注: SVM 的 RAX 由 VMCB.StateSaveArea.Rax 保存,其他 GPR 在 GuestContext 中
        switch (gprIdx) {
        ...
        if (!isProtectedTarget) {
            ULONG64 srcVal = 0;
            switch (gprIdx) {
            case 0:  srcVal = Vmcb->StateSaveArea.Rax; break;
            case 1:  srcVal = GuestContext->Rcx; break;
            case 2:  srcVal = GuestContext->Rdx; break;
            case 3:  srcVal = GuestContext->Rbx; break;
            case 4:  srcVal = Vmcb->StateSaveArea.Rsp; break;
            case 5:  srcVal = GuestContext->Rbp; break;
            case 6:  srcVal = GuestContext->Rsi; break;
            case 7:  srcVal = GuestContext->Rdi; break;
            case 8:  srcVal = GuestContext->R8;  break;
            case 9:  srcVal = GuestContext->R9;  break;
            case 10: srcVal = GuestContext->R10; break;
            case 11: srcVal = GuestContext->R11; break;
            case 12: srcVal = GuestContext->R12; break;
            case 13: srcVal = GuestContext->R13; break;
            case 14: srcVal = GuestContext->R14; break;
            case 15: srcVal = GuestContext->R15; break;
            }
            HvDbgWriteRealDr(drNum, srcVal);
        }
        // else: 静默吞,不让 guest 改我们的 HWBP 状态
    }
...
BOOLEAN SvmHandleException(PVCPU_DATA VcpuData, ULONG64 ExitCode)
{
    ...
    if (vector == 1) {
        ...
        // 阶段 7: 检查是否 HWBP 命中 (B0-B3 = bit 0-3)
        UINT64 dr6 = __readdr(6);
        BOOLEAN isBpHit = (dr6 & 0xF) != 0;

        if (isBpHit &&
            InterlockedCompareExchange(&g_GlobalHwbpRefCount, 0, 0) > 0) {
            HANDLE pid = PsGetCurrentProcessId();
            HANDLE tid = PsGetCurrentThreadId();
            PHV_PROCESS_HWBP hwbp = NULL;
            ULONG hitSlot = 0xFFFFFFFF;
            HANDLE debuggerPid = NULL;

            KIRQL dbIrql;
            KeAcquireSpinLock(&g_HwbpListLock, &dbIrql);
            hwbp = HvDbgFindByPidLocked(pid);
            if (hwbp && hwbp->ActiveCount > 0) {
                for (ULONG i = 0; i < 4; i++) {
                    if ((dr6 & (1ULL << i)) && hwbp->Slots[i].Active) {
                        hitSlot = i;
                        break;
                    }
                }
                debuggerPid = hwbp->DebuggerPid;
            }
            KeReleaseSpinLock(&g_HwbpListLock, dbIrql);

            if (hitSlot != 0xFFFFFFFF && debuggerPid != NULL) {
                HV_DEBUG_EVENT evt = { 0 };
                evt.Tid = tid;
                evt.Pid = pid;
                evt.HitSlot = hitSlot;
                evt.Dr6 = dr6;
                evt.Rip = Vmcb->StateSaveArea.Rip;
                evt.Rsp = Vmcb->StateSaveArea.Rsp;
                evt.Rflags = Vmcb->StateSaveArea.Rflags;
                evt.Cr3 = Vmcb->StateSaveArea.Cr3;
                // (GPR 留 0 — 调试器使用我们的 ContextThread hook 路径读真寄存器)

                // P113: SVM root mode 同 VMX, 用 NoSignal 避免 KeSetEvent BSOD
                HvDbgEnqueueEventNoSignal(debuggerPid, &evt);

                // 清 DR6 + 设 RFLAGS.RF=1
                __writedr(6, 0xFFFF0FF0ULL);
                Vmcb->StateSaveArea.Rflags |= 0x10000;
        ...
        // 不是我们的 HWBP,继续 fallthrough 走重注入路径
    }
...
BOOLEAN SvmVmExitDispatch(PVCPU_DATA VcpuData, PGUEST_CONTEXT GuestContext)
{
    ...
    case SVM_EXIT_VMMCALL:
        shouldContinue = SvmHandleVmmcall(vmcb, GuestContext);
        break;
        
    case SVM_EXIT_MSR:
        SvmHandleMsr(vmcb, GuestContext, exitInfo1);
        break;
    ...
    case SVM_EXIT_WRITE_CR4:
        SvmHandleCrAccess(vmcb, GuestContext, exitCode, exitInfo1);
        break;
