...
typedef enum _NPT_HOOK_STATE {
    NptHookStateInactive,   // 未激活
    NptHookStateActive,     // 已激活
    NptHookStatePaused,     // 暂停（用于读取原始代码）
} NPT_HOOK_STATE;
...
typedef struct _NPT_HOOK_ENTRY {
    ...
    // 目标页对应的 NPT PTE（缓存，避免 VM Exit 时动态遍历）
    PNPT_PTE TargetPte;
...
typedef struct _NPT_STEP_CONTEXT {
    volatile PNPT_HOOK_ENTRY PendingHookEntry;   // 需要恢复的 Hook 条目
    volatile LONG StepActive;                     // 单步是否激活 (0=否, 1=是)
