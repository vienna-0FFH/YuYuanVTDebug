    ...
    _In_ UINT64 Address)
{
    ...
    if (entry && entry->DebuggerPid != DebuggerPid) {
        ...
        if (stepState == HV_VWATCH_STEP_COMPLETING) {
            ...
            InterlockedExchange64(&entry->StepDiagnostic, 0);
            KeMemoryBarrier();
...
HvVwatchpArmSwBpStepOverRoot(
    ...
    _Inout_ PHV_PRIVATE_SWBP_ENTRY Entry)
{
#define HV_VWATCH_STEP_DIAG_TAG                 0x5354455000000000ULL
#define HV_VWATCH_STEP_DIAG_PAGE_INDEX          1u
#define HV_VWATCH_STEP_DIAG_CPU_INDEX           2u
#define HV_VWATCH_STEP_DIAG_PAGE_UNUSED         3u
#define HV_VWATCH_STEP_DIAG_PAGE_INVALIDATED    4u
#define HV_VWATCH_STEP_DIAG_THREAD_TOKEN        5u
#define HV_VWATCH_STEP_DIAG_PAGE_RUNDOWN        6u
#define HV_VWATCH_STEP_DIAG_PAGE_CHANGED        7u
#define HV_VWATCH_STEP_DIAG_ACTIVE_PTE          8u
#define HV_VWATCH_STEP_DIAG_MTF_RESERVE         9u
#define HV_VWATCH_STEP_DIAG_ENTRY_STATE         10u

    ULONG cpuIndex = HvVwatchpCpuIndex(VcpuData);
    PHV_VWATCH_MTF_CONTEXT mtf = cpuIndex < HV_VWATCH_MAX_CPUS
        ? &g_VwatchManager.MtfContext[cpuIndex]
        : NULL;
#define HV_VWATCH_RECORD_STEP_DIAG(Stage)                                      \
    do {                                                                        \
        if (InterlockedCompareExchange(                                         \
                &Entry->StepState, 0, 0) == HV_VWATCH_STEP_ARMED) {             \
            ULONG64 active = mtf                                                \
                ? (ULONG64)(ULONG)InterlockedCompareExchange(                   \
                    &mtf->Active, 0, 0)                                         \
                : 0;                                                           \
            ULONG64 reason = mtf ? (ULONG64)(ULONG)mtf->Reason : 0;             \
            ULONG64 diagnostic = HV_VWATCH_STEP_DIAG_TAG |                     \
                ((ULONG64)(Stage) & 0xFFULL) |                                  \
                (((ULONG64)cpuIndex & 0xFFULL) << 8) |                          \
                ((active & 0xFFULL) << 16) |                                   \
                ((reason & 0xFFULL) << 24);                                    \
            InterlockedExchange64(                                             \
                &Entry->StepDiagnostic,                                        \
                (LONG64)diagnostic);                                            \
        }                                                                       \
    } while (0)
    ...
    if (Entry->PageIndex < 0 ||
        Entry->PageIndex >= HV_PRIVATE_SWBP_MAX_PAGES) {
        HV_VWATCH_RECORD_STEP_DIAG(HV_VWATCH_STEP_DIAG_PAGE_INDEX);
        return FALSE;
    }
    ...
    if (cpuIndex >= HV_VWATCH_MAX_CPUS) {
        HV_VWATCH_RECORD_STEP_DIAG(HV_VWATCH_STEP_DIAG_CPU_INDEX);
        return FALSE;
    }
    if (InterlockedCompareExchange(&page->InUse, 0, 0) == 0) {
        HV_VWATCH_RECORD_STEP_DIAG(HV_VWATCH_STEP_DIAG_PAGE_UNUSED);
        return FALSE;
    }
    if (InterlockedCompareExchange(&page->Invalidated, 0, 0) != 0) {
        HV_VWATCH_RECORD_STEP_DIAG(HV_VWATCH_STEP_DIAG_PAGE_INVALIDATED);
        return FALSE;
    }
    if (Entry->HitThreadToken != HvVwatchpCurrentThreadToken()) {
        HV_VWATCH_RECORD_STEP_DIAG(HV_VWATCH_STEP_DIAG_THREAD_TOKEN);
        return FALSE;
    }
    if (!HvVwatchpAcquireRootRundown(&page->RootRundown)) {
        HV_VWATCH_RECORD_STEP_DIAG(HV_VWATCH_STEP_DIAG_PAGE_RUNDOWN);
        return FALSE;
    }
    ...
    if (InterlockedCompareExchange(&page->InUse, 0, 0) == 0 ||
        InterlockedCompareExchange(&page->Invalidated, 0, 0) != 0 ||
        Entry->PageIndex != pageIndex) {
        HV_VWATCH_RECORD_STEP_DIAG(HV_VWATCH_STEP_DIAG_PAGE_CHANGED);
        HvVwatchpReleaseRootRundown(&page->RootRundown);
        return FALSE;
    }
    ...
    PEPT_PTE_ENTRY slot = HvVwatchpGetActiveSwBpPte(
        ...
        cpuIndex);
    if (!slot) {
        HV_VWATCH_RECORD_STEP_DIAG(HV_VWATCH_STEP_DIAG_ACTIVE_PTE);
        HvVwatchpReleaseRootRundown(&page->RootRundown);
        return FALSE;
    }
    if (!HvVwatchpTryReserveMtf(mtf)) {
        HV_VWATCH_RECORD_STEP_DIAG(HV_VWATCH_STEP_DIAG_MTF_RESERVE);
        HvVwatchpReleaseRootRundown(&page->RootRundown);
        return FALSE;
    }
    if (InterlockedCompareExchange(
        ...
        HV_VWATCH_RECORD_STEP_DIAG(HV_VWATCH_STEP_DIAG_ENTRY_STATE);
...
#undef HV_VWATCH_RECORD_STEP_DIAG
    return TRUE;
}
    ...
    _In_ UINT64 Rip)
{
    ...
    if (state == HvPrivateSwBpArmed) {
        ...
        if (InterlockedCompareExchange(
            ...
            if (InterlockedCompareExchange(
                    &entry->StepState, 0, 0) == HV_VWATCH_STEP_ARMED) {
                event.Dr6 = (UINT64)InterlockedCompareExchange64(
                    &entry->StepDiagnostic, 0, 0);
            }
    ...
    _Inout_ PGUEST_CONTEXT Ctx)
{
    ...
    if (reason == HV_VWATCH_MTF_SWBP_DATA ||
        ...
        if (reason == HV_VWATCH_MTF_SWBP_REARM && swBpEntry &&
            ...
            InterlockedExchange64(&swBpEntry->StepDiagnostic, 0);
            KeMemoryBarrier();
            InterlockedCompareExchange(
                &swBpEntry->State,
                HvPrivateSwBpArmed,
