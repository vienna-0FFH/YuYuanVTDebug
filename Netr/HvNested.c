...
{
    g_EnableNestedVirtualization = Enable;
    DbgPrint("[HV-NESTED] Nested virtualization %s\n", Enable ? "enabled" : "disabled");
}
...
VOID HvNestedRestoreL1State(PVCPU_DATA VcpuData)
{
    ...
    // 数据段访问权限：可读写、存在
    __vmx_vmwrite(GUEST_SS_ACCESS_RIGHTS, 0xA093);  // 64-bit data, P=1, S=1, Type=3 (read/write)
    __vmx_vmwrite(GUEST_DS_ACCESS_RIGHTS, 0xA093);
    __vmx_vmwrite(GUEST_ES_ACCESS_RIGHTS, 0xA093);
    __vmx_vmwrite(GUEST_FS_ACCESS_RIGHTS, 0xA093);
    __vmx_vmwrite(GUEST_GS_ACCESS_RIGHTS, 0xA093);
...
NTSTATUS HvNestedPrepareVmcs02(PVCPU_DATA VcpuData)
{
    ...
    if ((l1SecondaryProc & (1ULL << 1)) && l1Eptp != 0) {
        ...
        PNESTED_EPT_CONTEXT eptContext = NestedEptGetOrCreateContext(l1Eptp);
        if (eptContext && eptContext->Valid) {
            ...
            // 这会导致地址翻译不正确，但至少不会崩溃
            DbgPrint("[HV-NESTED] Warning: Failed to create EPT02, using L0 EPT\n");
...
BOOLEAN HvNestedShouldL0HandleExit(PVCPU_DATA VcpuData, SIZE_T ExitReason, SIZE_T Qualification)
{
    ...
    case EXIT_REASON_EPT_VIOLATION:
        // 区分是 L0 Hook 还是应该传递给 L1
        {
            ...
            isL0Hook = EptHookFindByPhysicalAddress(guestPhysical) != NULL;
            ...
            if (nested->L1EptEnabled && nested->L1Eptp != 0) {
...
NTSTATUS HvNestedSetupEpt(PVCPU_DATA VcpuData, ULONG64 L1Eptp)
{
    ...
    // 使用新的嵌套 EPT 管理器创建上下文
    eptContext = NestedEptGetOrCreateContext(L1Eptp);
    if (!eptContext) {
...
VOID HvNestedCleanup(VOID)
{
    ...
    // 打印统计信息
    HvNestedPrintStats();
    NestedEptPrintGlobalStats();
    NestedNptPrintGlobalStats();
